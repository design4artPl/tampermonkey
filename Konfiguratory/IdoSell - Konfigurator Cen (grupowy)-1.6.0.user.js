...
// @version      1.6.0
// @description  Grupowe zarządzanie konfiguratorem cen: kopiowanie, tworzenie, eksport/import, aktualizacja modyfikatorów z listy produktów
// @author       SyncOffer
...
/* Value children (nesting) */
.kcfg-value-children { margin: 4px 0 8px 20px; padding: 8px 12px; background: #f8f9fa; border: 1px dashed #dadce0; border-radius: 6px; display: none; }
.kcfg-value-children.kcfg-visible { display: block; }
.kcfg-value-children label { display: block; padding: 4px 0; font-size: 12px; cursor: pointer; }
.kcfg-children-toggle { background: none; border: none; cursor: pointer; font-size: 14px; padding: 4px; color: #5f6368; transition: transform 0.15s; }
.kcfg-children-toggle.kcfg-open { transform: rotate(90deg); }
...
`;
  ...
  function injectImportButton(doc) {
    ...
    bar.appendChild(btn);
    btn.addEventListener('click', () => {
      showImportNewProductsModal(doc);
    });
  }
  ...
  // Load configurator data from source product
  function loadConfiguratorPreview(doc, productId, previewArea, onLoaded) {
    previewArea.innerHTML = '<div style="padding:16px;text-align:center;"><span class="kcfg-spinner"></span> Pobieram konfigurator...</div>';

    fetchConfiguratorData(productId).then(configData => {
      if (!configData) {
        previewArea.innerHTML = '<div class="kcfg-info kcfg-info--warning">Ten towar nie ma konfiguratora cen.</div>';
        return;
      }
      if (configData.parameters.length === 0) {
        previewArea.innerHTML = '<div class="kcfg-info kcfg-info--warning">Nie znaleziono parametr\u00f3w konfiguratora.</div>';
        return;
      }
      renderConfiguratorPreview(doc, previewArea, configData);
      onLoaded(configData);
    }).catch(err => {
      previewArea.innerHTML = '<div class="kcfg-info kcfg-info--error">B\u0142\u0105d pobierania konfiguratora: ' + escapeHtml(err.message) + '</div>';
    });
  }
  ...
  function renderConfiguratorPreview(doc, container, configData) {
    container.innerHTML = buildPreviewTableHtml(configData);
  }
  ...
  // Execute copy to target products
  async function executeCopy(doc, sourceConfigData, targetIds, opts, body, footer) {
    const abortController = new AbortController();
    const signal = abortController.signal;

    body.innerHTML = '<div style="padding:20px;">' +
      '<div class="kcfg-progress-text" id="kcfg-prog-text">Rozpoczynanie kopiowania...</div>' +
    ...
    const results = { success: 0, errors: [], total: targetIds.length, aborted: false };

    for (let i = 0; i < targetIds.length; i++) {
      if (signal.aborted) { results.aborted = true; break; }

      const targetId = targetIds[i];
      statusText.textContent = 'Kopiowanie (' + (i + 1) + '/' + targetIds.length + ') towar #' + targetId + '...';
      barInner.style.width = Math.round(((i + 1) / targetIds.length) * 100) + '%';

      try {
        await copyConfiguratorToProduct(sourceConfigData, targetId, opts);
        results.success++;
      } catch (e) {
        results.errors.push({ id: targetId, error: e.message });
      }

      if (i < targetIds.length - 1) await sleep(300);
    }

    showExecutionResults(doc, body, footer, results, 'Kopiowanie zako\u0144czone');
  }
  ...
  async function copyConfiguratorToProduct(configData, targetProductId, opts) {
    // If overwrite is enabled, remove existing configurator parameters first
    if (opts.overwrite) {
      await removeExistingConfigurator(targetProductId);
    }

    // Build payload for addPriceConfiguratorParameters
    for (const param of configData.parameters) {
      const values = {};
        ...
        parameters: {
          data: {
            [param.name]: {
              ...
              langData: {},
              priceModifier: {
                new: opts.copyPrices ? priceModifierNew : {},
                existing: {},
              },
    ...
    // Save the configurator tree (passes configData for potential nesting)
    await saveConfiguratorTree(targetProductId, configData);
  }
  ...
  function refreshChildrenPanel(doc, panel, currentBlock) {
    const allBlocks = Array.from(doc.querySelectorAll('.kcfg-param-block'));
    const otherParams = [];
    allBlocks.forEach((b, idx) => {
      if (b === currentBlock) return;
      const name = b.querySelector('.kcfg-param-name').value.trim();
      otherParams.push({ idx, name: name || '(parametr ' + (idx + 1) + ')' });
    });

    // Preserve existing checked state
    const prevChecked = new Set();
    panel.querySelectorAll('input[type="checkbox"]').forEach(cb => {
      if (cb.checked) prevChecked.add(cb.dataset.paramIdx);
    });

    panel.innerHTML = '';

    if (otherParams.length === 0) {
      panel.innerHTML = '<div style="font-size:11px;color:#999;">Dodaj wi\u0119cej parametr\u00f3w, aby m\u00f3c je zagnie\u017cd\u017ca\u0107.</div>';
      return;
    }

    const title = doc.createElement('div');
    title.style.cssText = 'font-size:11px;font-weight:600;color:#5f6368;margin-bottom:4px;';
    title.textContent = 'Zagnie\u017cd\u017cone parametry:';
    ...
    otherParams.forEach(p => {
      ...
      cb.type = 'checkbox';
      cb.dataset.paramIdx = String(p.idx);
      cb.checked = prevChecked.has(String(p.idx));
      cb.style.marginRight = '6px';
      label.appendChild(cb);
      label.appendChild(doc.createTextNode(p.name));
      panel.appendChild(label);
    });
  }

  function refreshAllChildrenPanels(doc) {
    doc.querySelectorAll('.kcfg-value-children.kcfg-visible').forEach(panel => {
      const paramBlock = panel.closest('.kcfg-param-block');
      if (paramBlock) refreshChildrenPanel(doc, panel, paramBlock);
    });
  }
  ...
  function parseCsvConfigurator(text) {
    ...
    for (const line of lines) {
      // Skip header
      if (line.toLowerCase().startsWith('parametr;') || line.toLowerCase().startsWith('parametr,')) continue;

      const sep = line.includes(';') ? ';' : ',';
      const parts = line.split(sep).map(p => p.trim());
      if (parts.length < 4) continue;

      const [paramName, paramType, required, valueName, modTypRaw, modValue, nestedRaw] = parts;
      ...
      if (!paramsMap[paramName]) {
        paramsMap[paramName] = {
          name: paramName,
          type: normalizeParamType(paramType),
      ...
      const modType = normalizeModifierType(modTypRaw || '+');
      const childrenNames = nestedRaw ? nestedRaw.split('|').map(s => s.trim()).filter(Boolean) : [];

      paramsMap[paramName].values.push({
        id: '',
        name: valueName,
        modifierType: modType,
        modifierValue: modValue || '0',
        _childrenNames: childrenNames,
      });
    }

    const result = { parameters: paramOrder.map(n => paramsMap[n]) };
    convertNamesToChildren(result);
    return result;
  }
  ...
  async function executeExport(doc, selectedProductIds, format, body, footer, includeUnused, includeLangData) {
    ...
    for (let i = 0; i < selectedProductIds.length; i++) {
      ...
      try {
        ...
        if (!configData || configData.parameters.length === 0) {
          results.errors.push({ id: productId, error: 'Brak konfiguratora' });
          continue;
        }
  ...
  function parseImportJson(text) {
    ...
    const products = data.products.map(p => {
      ...
      return { productId: String(p.productId || ''), configData };
    });
  ...
  function parseImportXml(text) {
    ...
    productEls.forEach(prodEl => {
      ...
      products.push({ productId, configData });
    });
  ...
  function showImportNewProductsModal(doc) {
    const { modal, body, footer } = createModal(doc, 'Importuj konfigurator \u2014 utw\u00f3rz nowe towary', 0);
    modal.classList.add('kcfg-modal--wide');
    ...
    function handleFile(file) {
      ...
      reader.onload = () => {
        try {
          ...
          html += '<b>' + file.name + '</b> \u2014 ' + parsedProducts.length + ' produkt' + (parsedProducts.length > 1 ? '\u00f3w' : '') + '<br>';
          parsedProducts.forEach((p, i) => {
            if (i < 10) html += '<div style="margin-top:4px;font-size:12px;color:#5f6368;">ID: ' + escapeHtml(String(p.productId)) + ' \u2014 ' + p.configData.parameters.length + ' parametr\u00f3w</div>';
          });
          ...
          // Show create button
          if (!footer.querySelector('#kcfg-create-btn')) {
            const createBtn = doc.createElement('button');
            createBtn.id = 'kcfg-create-btn';
            createBtn.className = 'kcfg-btn-primary';
            createBtn.textContent = 'Utw\u00f3rz ' + parsedProducts.length + ' nowy' + (parsedProducts.length > 1 ? 'ch towar\u00f3w' : ' towar');
            createBtn.addEventListener('click', () => {
              if (!parsedProducts) return;
              const hideParams = body.querySelector('#kcfg-imp-hide-params').checked;
              const importItems = parsedProducts.map(p => ({
                name: 'Konfigurator #' + p.productId,
                configData: p.configData,
              }));
              executeCreateNewProducts(doc, importItems, parsedProducts[0].configData, true, hideParams, body, footer);
            });
  ...
  function renderImportPreview(doc, container, importData, selectedProductIds) {
    let html = '';

    if (importData.products.length > 0) {
      const firstProduct = importData.products[0];
      html += '<div style="font-size:12px;color:#5f6368;margin-bottom:4px;">Podgl\u0105d: towar #' + escapeHtml(String(firstProduct.productId)) + '</div>';
      html += buildPreviewTableHtml(firstProduct.configData);

      if (importData.products.length > 1) {
        html += '<div style="font-size:12px;color:#5f6368;margin-top:8px;">...i ' + (importData.products.length - 1) + ' wi\u0119cej produkt\u00f3w w pliku</div>';
      }
    }

    html += '<div class="kcfg-options" style="margin-top:16px;">';
    html += '<div class="kcfg-option-row"><label><input type="radio" name="kcfg-import-mode" value="selected" checked> Zastosuj do zaznaczonych towar\u00f3w (' + selectedProductIds.length + ')</label></div>';
    html += '<div class="kcfg-option-row"><label><input type="radio" name="kcfg-import-mode" value="match"> Dopasuj po ID towaru z pliku</label></div>';

    if (importData.products.length > 1) {
      html += '<div id="kcfg-import-select-wrap" style="margin-top:8px;">';
      html += '<label style="font-size:12px;color:#5f6368;">Konfigurator do zastosowania: </label>';
      html += '<select id="kcfg-import-select" style="padding:4px 8px;border:1px solid #dadce0;border-radius:4px;font-size:12px;">';
      importData.products.forEach((p, idx) => {
        html += '<option value="' + idx + '">Towar #' + escapeHtml(String(p.productId)) + ' (' + p.configData.parameters.length + ' param.)</option>';
      });
      html += '</select></div>';
    }

    html += '<div class="kcfg-option-row" style="margin-top:8px;"><label><input type="checkbox" id="kcfg-import-overwrite" checked> Nadpisz istniej\u0105cy konfigurator</label></div>';
    html += '</div>';

    container.innerHTML = html;

    const modeRadios = container.querySelectorAll('input[name="kcfg-import-mode"]');
    const selectWrap = container.querySelector('#kcfg-import-select-wrap');
    modeRadios.forEach(radio => {
      radio.addEventListener('change', () => {
        if (selectWrap) selectWrap.style.display = radio.value === 'selected' ? '' : 'none';
      });
    });
  }

  async function executeImport(doc, targetIds, importData, configDataToApply, overwrite, mode, body, footer) {
    const abortController = new AbortController();
    const signal = abortController.signal;

    body.innerHTML = '<div style="padding:20px;">' +
      '<div class="kcfg-progress-text" id="kcfg-prog-text">Rozpoczynanie importu...</div>' +
      '<div class="kcfg-progress" style="height:20px;margin-top:12px;"><div class="kcfg-progress-fill" id="kcfg-prog-bar"></div></div>' +
      '</div>';
    ...
    if (mode === 'match') {
      importData.products.forEach(p => { productConfigMap[p.productId] = p.configData; });
    }
    ...
    for (let i = 0; i < targetIds.length; i++) {
      ...
      statusText.textContent = 'Import (' + (i + 1) + '/' + targetIds.length + ') towar #' + productId + '...';
      barInner.style.width = Math.round(((i + 1) / targetIds.length) * 100) + '%';

      const dataForProduct = mode === 'match' ? productConfigMap[productId] : configDataToApply;
      if (!dataForProduct) {
        results.errors.push({ id: productId, error: 'Brak danych w pliku dla tego towaru' });
        continue;
      }

      try {
        if (overwrite) {
          await createConfiguratorOnProduct(dataForProduct, productId);
        } else {
          await updateConfiguratorOnProduct(dataForProduct, productId);
        }
        results.success++;
      } catch (e) {
        results.errors.push({ id: productId, error: e.message });
    ...
    showExecutionResults(doc, body, footer, results, 'Import zako\u0144czony');
  }
  ...
  async function executeCreateNewProducts(doc, importItems, fallbackConfigData, autoType, hideParams, body, footer) {
    const abortController = new AbortController();
    const signal = abortController.signal;

    body.innerHTML = '<div style="padding:20px;">' +
      '<div class="kcfg-progress-text" id="kcfg-prog-text">Tworzenie nowych towar\u00f3w...</div>' +
      '<div class="kcfg-progress" style="height:20px;margin-top:12px;"><div class="kcfg-progress-fill" id="kcfg-prog-bar"></div></div>' +
      '</div>';
    ...
    const results = { success: 0, errors: [], total: count, aborted: false, createdIds: [] };

    for (let i = 0; i < count; i++) {
      if (signal.aborted) { results.aborted = true; break; }

      const item = importItems[i];
      const itemConfigData = item.configData || fallbackConfigData;
      statusText.textContent = 'Tworzenie towaru (' + (i + 1) + '/' + count + ') ' + item.name + '...';
      barInner.style.width = Math.round(((i + 1) / count) * 100) + '%';

      try {
        const newId = await createNewProduct(item.name);
        results.createdIds.push(newId);

        statusText.textContent = 'Konfigurator dla towaru #' + newId + '...';
        await createConfiguratorOnProduct(itemConfigData, newId);

        if (hideParams) {
          await hideConfiguratorParameters(newId);
        }

        results.success++;
      } catch (e) {
        results.errors.push({ id: item.name, error: e.message });
      }

      if (i < count - 1) await sleep(500);
    }

    // Show results with created IDs
    let html = '<div style="padding:20px;">';
    html += '<div style="font-size:16px;font-weight:600;margin-bottom:12px;">Tworzenie zako\u0144czone</div>';
    html += '<div style="color:#34a853;margin-bottom:6px;">Utworzono: <b>' + results.success + '</b> / ' + results.total + '</div>';
    if (results.createdIds.length > 0) {
      html += '<div style="font-size:12px;color:#5f6368;margin-top:8px;">Nowe ID towar\u00f3w: ' + results.createdIds.join(', ') + '</div>';
    }
    if (results.errors.length > 0) {
      html += '<div style="color:#e74c3c;margin-bottom:6px;">B\u0142\u0119dy: <b>' + results.errors.length + '</b></div>';
      html += '<div style="max-height:120px;overflow-y:auto;font-size:12px;border:1px solid #eee;padding:8px;border-radius:4px;">';
      results.errors.forEach(e => { html += '<div>' + escapeHtml(String(e.id)) + ': ' + escapeHtml(e.error) + '</div>'; });
      html += '</div>';
    }
    html += '</div>';
    body.innerHTML = html;
  ...
  async function executeCreate(doc, selectedProductIds, configData, updateExisting, autoType, hideParams, body, footer) {
    const abortController = new AbortController();
    const signal = abortController.signal;
    ...
    for (let i = 0; i < selectedProductIds.length; i++) {
      ...
      const productId = selectedProductIds[i];
      statusText.textContent = (updateExisting ? 'Aktualizowanie' : 'Tworzenie') +
        ' (' + (i + 1) + '/' + selectedProductIds.length + ') towar #' + productId + '...';
      barInner.style.width = Math.round(((i + 1) / selectedProductIds.length) * 100) + '%';

      try {
        if (autoType) {
          await ensureProductTypeConfigurable(productId, (msg) => { statusText.textContent = msg; });
        }
        if (updateExisting) {
    ...
    showExecutionResults(doc, body, footer, results, updateExisting ? 'Aktualizacja zako\u0144czona' : 'Tworzenie zako\u0144czone');
  }
  ...
  function showExecutionResults(doc, body, footer, results, title) {
    ...
    if (results.errors.length > 0) {
      ...
      results.errors.forEach(e => {
        html += '<div>Towar #' + escapeHtml(String(e.id)) + ': ' + escapeHtml(e.error) + '</div>';
      });
      html += '</div>';
    }
