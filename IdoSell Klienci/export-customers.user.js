...
// @version      1.5
// @description  Export klientow z panelu zrodlowego (API lub CSV) i porownanie z panelem docelowym — generuje brakujacy_klienci.csv i adresy_do_dodania.csv
...
(function () {
  ...
  function diffSourceTarget(srcRows, tgtRows) {
    const tgtByLogin = new Map();
    for (const r of tgtRows) {
      const k = String(r.login || '').trim().toLowerCase();
      if (k) tgtByLogin.set(k, r);
    }
    const missing = []; // klient w src, brak w tgt
    const updates = []; // klient w obu, ale src ma adresy ktorych nie ma tgt
    for (const src of srcRows) {
      const k = String(src.login || '').trim().toLowerCase();
      if (!k) continue;
      const tgt = tgtByLogin.get(k);
      if (!tgt) { missing.push(src); continue; }
  ...
  function openModal() {
    ...
    const ts = GM_getValue('tgtShopId', '1');
    ...
    overlay.innerHTML = `
      <div id="idosell-cust-modal">
        ...
        <div class="section hide" id="cust-tgt-section">
          ...
          <label>shopId</label>
          <input id="cust-tgt-shop" type="text" placeholder="1" value="${ts}">
        </div>
