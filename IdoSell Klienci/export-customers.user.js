...
// @version      1.9
// @description  Export klientow z panelu zrodlowego (API lub CSV) i porownanie z panelem docelowym — pairing adresow target↔source z ID adresu i regionu (gotowe do PUT update)
...
(function () {
  ...
  // KLUCZOWE: pomijamy kraj. Bywa wpisany blednie (np. "Niemcy" przy adresie
  // w Warszawie z PL kodem pocztowym), wiec polegamy tylko na trojce
  // street+city+zipcode (znormalizowanej, z foldingiem diakrytykow).
  const _ADDR_PL_DIACRITIC = { 'ą':'a','ć':'c','ę':'e','ł':'l','ń':'n','ó':'o','ś':'s','ż':'z','ź':'z',
                                'Ą':'a','Ć':'c','Ę':'e','Ł':'l','Ń':'n','Ó':'o','Ś':'s','Ż':'z','Ź':'z' };
  function _normAddrField(v) {
    return String(v || '')
      .trim()
      .toLowerCase()
      .replace(/[ąćęłńóśżźĄĆĘŁŃÓŚŻŹ]/g, ch => _ADDR_PL_DIACRITIC[ch] || ch)
      .replace(/\s+/g, ' ');
  }
  function normAddr(a) {
    return [
      _normAddrField(a.street),
      _normAddrField(a.city),
      _normAddrField(a.zipcode).replace(/[\s-]/g, '')
    ].join('|');
  }
  ...
  function mapClient(c, addrs, shopIdNum, note) {
    ...
    // Raw API data (do diff/PUT) — non-enumerable, nie idzie do CSV
    Object.defineProperty(row, '_clientId', { value: c.clientId || '', enumerable: false });
    Object.defineProperty(row, '_rawAddresses', { value: Array.isArray(addrs) ? addrs : [], enumerable: false });
    return row;
  }
  ...
  function diffSourceTarget(srcRows, tgtRows, log) {
    ...
    const missing = [];
    const updates = [];
    const countryMismatches = []; // adresy ktore matchuja po street+city+zip ale roznia sie krajem
    ...
    for (const src of srcRows) {
      ...
      // Raw API addresses target (z ID, regionId) — z _rawAddresses jak target z API
      const tgtRaw = Array.isArray(tgt._rawAddresses) ? tgt._rawAddresses : [];
      const tgtRawByKey = new Map();
      for (const t of tgtRaw) {
        const key = normAddr({
          street: t.clientDeliveryAddressStreet,
          city: t.clientDeliveryAddressCity,
          zipcode: t.clientDeliveryAddressZipCode
        });
        tgtRawByKey.set(key, t);
      }
      // Source raw (jak source z API) — do source regionId
      const srcRaw = Array.isArray(src._rawAddresses) ? src._rawAddresses : [];
      const srcRawByKey = new Map();
      for (const s of srcRaw) {
        const key = normAddr({
          street: s.clientDeliveryAddressStreet,
          city: s.clientDeliveryAddressCity,
          zipcode: s.clientDeliveryAddressZipCode
        });
        srcRawByKey.set(key, s);
      }

      const tgtByKey = new Map();
      for (const a of tgtAddrs) tgtByKey.set(normAddr(a), a);

      const newAddrs = [];
      for (const a of srcAddrs) {
        const key = normAddr(a);
        const tgtMatch = tgtByKey.get(key);
        if (!tgtMatch) {
          newAddrs.push(a);
        } else {
          // matched by content — sprawdz czy kraj sie zgadza
          const sc = String(a.country || '').trim().toLowerCase();
          const tc = String(tgtMatch.country || '').trim().toLowerCase();
          if (sc && tc && sc !== tc) {
            const tgtRawMatch = tgtRawByKey.get(key) || {};
            const srcRawMatch = srcRawByKey.get(key) || {};
            countryMismatches.push({
              target_client_login: tgt.login || '',
              target_client_id: tgt._clientId || '',
              target_address_id: tgtRawMatch.clientDeliveryAddressId || '',
              target_region_id: tgtRawMatch.clientDeliveryAddressRegionId || '',
              street: a.street, city: a.city, zipcode: a.zipcode,
              source_country: a.country,
              target_country: tgtMatch.country,
              source_region_id: srcRawMatch.clientDeliveryAddressRegionId || ''
            });
          }
        }
      }
    ...
    if (log) {
      ...
      if (countryMismatches.length) {
        log(`▸ Adresy z roznym krajem (matchuja po street+city+zip): ${countryMismatches.length}`);
      }
    }
    return { missing, updates, countryMismatches };
  }
    ...
    overlay.querySelector('#cust-go').onclick = async () => {
      ...
      try {
        ...
        if (mode === 'export') {
          ...
          const { missing, updates, countryMismatches } = diffSourceTarget(srcRows, tgtRows, log);
          ...
          if (countryMismatches && countryMismatches.length) {
            // Pairing: dane z target (login, clientId, addressId, regionId) +
            // source (country, regionId) — gotowe do PUT update
            const mhdr = [
              'target_client_login','target_client_id',
              'target_address_id','target_region_id',
              'street','city','zipcode',
              'source_country','target_country','source_region_id'
            ];
            const mlines = [mhdr.map(csvEscape).join(',')];
            for (const m of countryMismatches) mlines.push(mhdr.map(h => csvEscape(m[h])).join(','));
            const mcsv = '﻿' + mlines.join('\r\n');
            const mfname = `adresy_kraj_do_naprawy_${todayStamp()}.csv`;
            downloadFile(mfname, mcsv);
            log(`▸ Pobrano: ${mfname} (${countryMismatches.length} wierszy z parami target↔source)`);
          }
