...
// @version      1.8
// @description  Export klientow z panelu zrodlowego (API lub CSV) i porownanie z panelem docelowym — match adresow po street+city+zipcode (kraj pomijany jako niewiarygodny), z fold diakrytykow
        ...
        if (!tgtMatch) {
          ...
          if (sc && tc && sc !== tc) {
            countryMismatches.push({
              login: src.login || '', email: src.email || '',
              street: a.street, city: a.city, zipcode: a.zipcode,
              source_country: a.country, target_country: tgtMatch.country,
              target_address_id: tgtMatch.id || ''
            });
          }
        }
      }
      ...
      try {
        ...
        if (mode === 'export') {
          ...
          if (countryMismatches && countryMismatches.length) {
            // Generuj informacyjny raport (tylko gdy sa)
            const mhdr = ['login','email','street','city','zipcode','source_country','target_country'];
            ...
            log(`▸ Pobrano: ${mfname} (${countryMismatches.length} wierszy)`);
          }
