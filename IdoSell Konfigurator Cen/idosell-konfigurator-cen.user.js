...
(function () {
  ...
  // Stan przerwanych operacji — przeżywa zamknięcie karty i restart przeglądarki
  const STORE_KEY_BATCH = 'kcfgPendingBatch';
  const STORE_KEY_STOCK = 'kcfgStockSnapshot';
  const STORE_KEY_NESTING = 'kcfgNestingPresets';
  ...
  function storeSet(key, value) {
    ...
    catch (_e) { return false; } // quota / tryb prywatny
  }
  ...
  function fetchAjaxRaw(url, body) {
    return new Promise((resolve, reject) => {
      // Fallback na okno główne: gdy iframe się przeładuje w trakcie operacji,
      // żądania lecą dalej (ta sama domena panelu, ta sama sesja).
      const win = getIframeWin() || window;
...
/* Zadanie w tle — pływająca karta postępu */
.kcfg-bg-task {
  position: fixed;
  right: 16px;
  bottom: 16px;
  ...
  z-index: 99999;
  font-family: 'Segoe UI', Roboto, Arial, sans-serif;
  animation: kcfgSlideUp 0.2s ease;
}
.kcfg-bg-task-head {
  display: flex;
  align-items: center;
...
.kcfg-bg-task-body { padding: 10px 16px 0; }
.kcfg-bg-task-foot {
  display: flex;
  justify-content: flex-end;
  gap: 8px;
...
`;
  ...
  // Reguła zagnieżdżenia: pod wartością `parentValue` parametru `parentParam`
  // pokaż parametr `childParam`. Dopasowanie po NAZWACH — dzięki temu jedna
  // reguła działa na każdym towarze, który ma te parametry u siebie.
  // Mutuje przekazany config (podawać kopię).
  function applyNestingRules(config, rules) {
    const applied = [];
    const skipped = [];

    const idxByName = {};
    config.parameters.forEach((p, idx) => { idxByName[(p.name || '').toLowerCase()] = idx; });

    (rules || []).forEach(rule => {
      const parentIdx = idxByName[(rule.parentParam || '').toLowerCase()];
      const childIdx = idxByName[(rule.childParam || '').toLowerCase()];
      if (parentIdx === undefined || childIdx === undefined || parentIdx === childIdx) {
        skipped.push(rule);
        return;
      }
      const val = config.parameters[parentIdx].values.find(
        v => (v.name || '').toLowerCase() === (rule.parentValue || '').toLowerCase()
      );
      if (!val) { skipped.push(rule); return; }

      if (!val.children) val.children = [];
      if (!val.children.includes(childIdx)) val.children.push(childIdx);
      applied.push(rule);
    });
  ...
  // Zapisane zestawy reguł — przeżywają zamknięcie przeglądarki
  function getNestingPresets() { return storeGet(STORE_KEY_NESTING) || {}; }

  function saveNestingPreset(name, rules) {
    const all = getNestingPresets();
    all[name] = rules;
    return storeSet(STORE_KEY_NESTING, all);
  }

  function deleteNestingPreset(name) {
    const all = getNestingPresets();
    delete all[name];
    storeSet(STORE_KEY_NESTING, all);
  }

  // Pierwszy towar z listy, który FAKTYCZNIE ma parametry z wartościami — towary
  // bez parametrów są pomijane, nie blokują wzorca. Skanuje najwyżej `limit`
  // pozycji, żeby nie zasypać panelu żądaniami przy dużym zaznaczeniu.
  async function findFirstProductWithParameters(productIds, onProgress, limit) {
    const max = Math.min(productIds.length, limit || 25);
    const skipped = [];

    for (let i = 0; i < max; i++) {
      const id = productIds[i];
      if (onProgress) onProgress(id, i + 1, max);
      try {
        const config = await fetchProductParametersAsConfig(id);
        if (config.parameters.length > 0) {
          return { productId: id, config, skipped, scanned: i + 1 };
        }
      } catch (_e) { /* nieczytelny towar traktujemy jak pusty */ }
      skipped.push(id);
      await sleep(120);
    }

    return { productId: null, config: null, skipped, scanned: max, exhausted: max < productIds.length };
  }
  ...
  function showFromProductParamsMode(doc, selectedProductIds) {
    ...
    body.innerHTML =
      ...
      '<div id="kcfg-fp-preview" style="padding:16px;text-align:center;color:#5f6368;font-size:13px;">' +
        '<span class="kcfg-spinner"></span> Szukam towaru wzorcowego...' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:10px;padding:10px 12px;background:#fff;border:1px solid #e8eaed;border-radius:8px;">' +
        '<span style="font-size:12px;color:#5f6368;font-weight:500;white-space:nowrap;">Towar wzorcowy:</span>' +
        '<input type="text" id="kcfg-fp-template-id" inputmode="numeric" placeholder="puste = pierwszy zaznaczony towar, który ma parametry" ' +
          'style="flex:1;min-width:200px;height:34px;padding:0 10px;border:1px solid #dadce0;border-radius:6px;font-size:13px;font-family:inherit;outline:none;background:#fff;box-sizing:border-box;">' +
        '<button id="kcfg-fp-template-load" class="kcfg-btn-secondary" style="height:34px;padding:0 16px;border-radius:6px;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;">Wczytaj</button>' +
      '</div>' +
      '<div class="kcfg-options" style="margin-top:14px;">' +
        '<div class="kcfg-option-row" style="flex-direction:column;align-items:flex-start;gap:5px;">' +
          '<label style="display:flex;align-items:center;gap:10px;cursor:pointer;"><span class="kcfg-switch"><input type="checkbox" id="kcfg-fp-per-product" checked><span class="kcfg-switch-track"><span class="kcfg-switch-thumb"></span></span></span> Każdy towar ze swoich parametrów</label>' +
          '<div id="kcfg-fp-per-product-hint" style="font-size:11px;color:#5f6368;line-height:1.5;padding-left:46px;"></div>' +
        '</div>' +
        ...
        '<div class="kcfg-option-row" style="flex-direction:column;align-items:flex-start;gap:5px;">' +
          '<label style="display:flex;align-items:center;gap:10px;cursor:pointer;"><span class="kcfg-switch"><input type="checkbox" id="kcfg-fp-background"><span class="kcfg-switch-track"><span class="kcfg-switch-thumb"></span></span></span> Uruchom jako zadanie w tle</label>' +
          '<div style="font-size:11px;color:#5f6368;line-height:1.5;padding-left:46px;">Okno się zamknie, postęp pokaże się w karcie w rogu i możesz dalej pracować w panelu. <b>Karty przeglądarki nie wolno zamknąć</b> — operacja przerwie się w połowie.</div>' +
        '</div>' +
      '</div>' +
      '<div style="margin-top:14px;padding:12px 14px;background:#fff;border:1px solid #e8eaed;border-radius:8px;">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px;">' +
          '<div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#5f6368;">Zagnieżdżenia</div>' +
          '<div style="font-size:11px;color:#9aa0a6;">Wybór wartości odsłania dodatkowy parametr</div>' +
        '</div>' +
        '<div id="kcfg-fp-rules"></div>' +
        '<div id="kcfg-fp-rules-hint" style="font-size:11px;color:#9aa0a6;">Najpierw wczytaj towar wzorcowy — z niego biorą się nazwy parametrów i wartości.</div>' +
        '<button id="kcfg-fp-add-rule" class="kcfg-add-value-btn" style="margin-top:8px;" disabled>+ Dodaj regułę</button>' +
        '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:10px;padding-top:10px;border-top:1px solid #f1f3f4;">' +
          '<span style="font-size:11px;color:#5f6368;">Zestawy:</span>' +
          '<select id="kcfg-fp-preset" style="height:32px;padding:0 8px;border:1px solid #dadce0;border-radius:6px;font-size:12px;font-family:inherit;background:#fff;outline:none;min-width:150px;"></select>' +
          '<button id="kcfg-fp-preset-load" class="kcfg-btn-secondary" style="height:32px;padding:0 12px;border-radius:6px;font-size:12px;cursor:pointer;font-family:inherit;">Wczytaj</button>' +
          '<button id="kcfg-fp-preset-del" class="kcfg-btn-secondary" style="height:32px;padding:0 12px;border-radius:6px;font-size:12px;cursor:pointer;font-family:inherit;">Usuń</button>' +
          '<input id="kcfg-fp-preset-name" placeholder="nazwa nowego zestawu" style="flex:1;min-width:140px;height:32px;padding:0 10px;border:1px solid #dadce0;border-radius:6px;font-size:12px;font-family:inherit;outline:none;background:#fff;box-sizing:border-box;">' +
          '<button id="kcfg-fp-preset-save" class="kcfg-btn-secondary" style="height:32px;padding:0 12px;border-radius:6px;font-size:12px;cursor:pointer;font-family:inherit;">Zapisz</button>' +
        '</div>' +
        '<div id="kcfg-fp-preset-msg" style="font-size:11px;color:#9aa0a6;margin-top:6px;"></div>' +
      '</div>';

    const previewArea = body.querySelector('#kcfg-fp-preview');
    const templateIdInput = body.querySelector('#kcfg-fp-template-id');
    const templateLoadBtn = body.querySelector('#kcfg-fp-template-load');
    const perProductCb = body.querySelector('#kcfg-fp-per-product');
    const perProductHint = body.querySelector('#kcfg-fp-per-product-hint');

    let templateConfig = null;  // parametry towaru wzorcowego
    let templateId = null;      // ID towaru, z którego pochodzi wzorzec

    // --- Reguły zagnieżdżeń ---------------------------------------------
    const rulesList = body.querySelector('#kcfg-fp-rules');
    const rulesHint = body.querySelector('#kcfg-fp-rules-hint');
    const addRuleBtn = body.querySelector('#kcfg-fp-add-rule');
    const presetSel = body.querySelector('#kcfg-fp-preset');
    const presetName = body.querySelector('#kcfg-fp-preset-name');
    const presetMsg = body.querySelector('#kcfg-fp-preset-msg');

    const RULE_SEL_STYLE = 'height:32px;padding:0 8px;border:1px solid #dadce0;border-radius:6px;font-size:12px;' +
      'font-family:inherit;background:#fff;outline:none;max-width:180px;';

    function paramNames() {
      return templateConfig ? templateConfig.parameters.map(p => p.name) : [];
    }

    function valuesOf(paramName) {
      const p = templateConfig && templateConfig.parameters.find(x => x.name === paramName);
      return p ? p.values.map(v => v.name) : [];
    }

    // Wartość z zapisanego zestawu może nie istnieć w tym wzorcu — pokazujemy ją
    // mimo to, żeby reguła nie zniknęła po cichu przy wczytaniu.
    function fillSelect(sel, options, selected) {
      const list = (selected && !options.includes(selected)) ? [selected].concat(options) : options;
    ...
    function addRuleRow(rule) {
      ...
      if (!names.length && !rule) return;

      const row = doc.createElement('div');
      row.className = 'kcfg-fp-rule';
      row.style.cssText = 'display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:6px;';
      row.innerHTML =
        '<span style="font-size:11px;color:#5f6368;">Pod wartością</span>' +
        '<select class="kcfg-rule-value" style="' + RULE_SEL_STYLE + '"></select>' +
        '<span style="font-size:11px;color:#5f6368;">parametru</span>' +
        '<select class="kcfg-rule-parent" style="' + RULE_SEL_STYLE + '"></select>' +
        '<span style="font-size:11px;color:#5f6368;">pokaż</span>' +
        '<select class="kcfg-rule-child" style="' + RULE_SEL_STYLE + '"></select>' +
        '<button class="kcfg-value-remove" title="Usuń regułę">✕</button>';
      rulesList.appendChild(row);

      const parentSel = row.querySelector('.kcfg-rule-parent');
      const valueSel = row.querySelector('.kcfg-rule-value');
      const childSel = row.querySelector('.kcfg-rule-child');

      fillSelect(parentSel, names, rule && rule.parentParam);
      fillSelect(childSel, names, rule && rule.childParam);
      fillSelect(valueSel, valuesOf(parentSel.value), rule && rule.parentValue);

      parentSel.addEventListener('change', () => fillSelect(valueSel, valuesOf(parentSel.value)));
      row.querySelector('.kcfg-value-remove').addEventListener('click', () => {
        row.remove();
        updateRulesHint();
      });
    ...
    function updateRulesHint() {
      const count = rulesList.querySelectorAll('.kcfg-fp-rule').length;
      if (!templateConfig) {
        rulesHint.textContent = 'Najpierw wczytaj towar wzorcowy — z niego biorą się nazwy parametrów i wartości.';
        rulesHint.style.display = '';
      } else if (count === 0) {
        rulesHint.textContent = 'Brak reguł — konfigurator powstanie płasko, wszystkie parametry na jednym poziomie.';
        rulesHint.style.display = '';
      } else {
    ...
    function collectRules() {
      return Array.from(rulesList.querySelectorAll('.kcfg-fp-rule')).map(row => ({
        parentParam: row.querySelector('.kcfg-rule-parent').value,
        parentValue: row.querySelector('.kcfg-rule-value').value,
        childParam: row.querySelector('.kcfg-rule-child').value,
      })).filter(r => r.parentParam && r.parentValue && r.childParam && r.parentParam !== r.childParam);
    }

    function refreshPresetSelect() {
      const names = Object.keys(getNestingPresets());
      presetSel.innerHTML = names.length
        ? names.map(n => '<option value="' + escapeHtml(n) + '">' + escapeHtml(n) + '</option>').join('')
        : '<option value="">— brak zapisanych —</option>';
    }

    addRuleBtn.addEventListener('click', () => addRuleRow(null));

    body.querySelector('#kcfg-fp-preset-load').addEventListener('click', () => {
      const name = presetSel.value;
      const preset = getNestingPresets()[name];
      if (!preset) { presetMsg.textContent = 'Nie ma czego wczytać.'; return; }
      rulesList.innerHTML = '';
      preset.forEach(r => addRuleRow(r));
      presetName.value = name;
      presetMsg.textContent = 'Wczytano zestaw „' + name + '" (' + preset.length + ' reguł).';
      updateRulesHint();
    });

    body.querySelector('#kcfg-fp-preset-save').addEventListener('click', () => {
      const name = (presetName.value || '').trim();
      if (!name) { presetMsg.textContent = 'Podaj nazwę zestawu.'; return; }
      const rules = collectRules();
      if (!rules.length) { presetMsg.textContent = 'Brak reguł do zapisania.'; return; }
      const ok = saveNestingPreset(name, rules);
      refreshPresetSelect();
      presetSel.value = name;
      presetMsg.textContent = ok
        ? 'Zapisano zestaw „' + name + '" (' + rules.length + ' reguł).'
        : 'Nie udało się zapisać — pamięć przeglądarki niedostępna.';
    });

    body.querySelector('#kcfg-fp-preset-del').addEventListener('click', () => {
      const name = presetSel.value;
      if (!name) return;
      deleteNestingPreset(name);
      refreshPresetSelect();
      presetMsg.textContent = 'Usunięto zestaw „' + name + '".';
    });
    ...
    const editorBtn = footer.querySelector('#kcfg-fp-editor');
    const runBtn = footer.querySelector('#kcfg-fp-run');

    footer.querySelector('#kcfg-back').addEventListener('click', () => showConfiguratorModal(doc, selectedProductIds));

    function updatePerProductHint() {
      if (perProductCb.checked) {
        perProductHint.textContent = 'Każdy zaznaczony towar dostanie konfigurator zbudowany ze swoich własnych parametrów.';
      } else if (templateId) {
        perProductHint.innerHTML = 'Wszystkie zaznaczone towary dostaną <b>ten sam</b> konfigurator — z parametrów towaru <b>#' +
          escapeHtml(String(templateId)) + '</b>. Ich własne parametry zostaną zignorowane.';
      } else {
        perProductHint.textContent = 'Wszystkie zaznaczone towary dostaną ten sam konfigurator — najpierw trzeba ustalić towar wzorcowy.';
      }
    }

    // Wzorzec: wskazane ID albo pierwszy zaznaczony towar, który ma parametry
    async function loadTemplate(explicitId) {
      templateConfig = null;
      templateId = null;
      editorBtn.disabled = true;
      runBtn.disabled = true;
      updatePerProductHint();

      previewArea.style.textAlign = 'center';
      previewArea.innerHTML = '<span class="kcfg-spinner"></span> ' + (explicitId
        ? 'Odczytuję parametry towaru #' + escapeHtml(explicitId) + '...'
        : 'Szukam pierwszego zaznaczonego towaru z parametrami...');

      try {
        let config, foundId, note = '';

        if (explicitId) {
          config = await fetchProductParametersAsConfig(explicitId);
          foundId = explicitId;
          if (!config.parameters.length) {
            previewArea.innerHTML = '<div class="kcfg-info kcfg-info--warning">Towar #' + escapeHtml(explicitId) +
              ' nie ma przypisanych parametrów z wartościami. Wskaż inne ID albo wyczyść pole i kliknij „Wczytaj", ' +
              'żeby wybrać wzorzec automatycznie.</div>';
            runBtn.disabled = false;
            updatePerProductHint();
            return;
          }
        } else {
          const found = await findFirstProductWithParameters(selectedProductIds, (id, n, total) => {
            previewArea.innerHTML = '<span class="kcfg-spinner"></span> Szukam towaru z parametrami — sprawdzam #' +
              escapeHtml(String(id)) + ' (' + n + '/' + total + ')...';
          });

          if (!found.productId) {
            previewArea.innerHTML = '<div class="kcfg-info kcfg-info--warning">Żaden ze sprawdzonych ' + found.scanned +
              ' towarów nie ma przypisanych parametrów z wartościami' +
              (found.exhausted ? ' (sprawdzono pierwszych ' + found.scanned + ' z ' + selectedProductIds.length + ' zaznaczonych)' : '') +
              '. Możesz wskazać ID towaru wzorcowego ręcznie — nie musi być zaznaczony na liście.</div>';
            runBtn.disabled = false;
            updatePerProductHint();
            return;
          }
          ...
          config = found.config;
          foundId = found.productId;
          if (found.skipped.length > 0) {
            note = '<div style="font-size:11px;color:#9aa0a6;text-align:left;margin-bottom:6px;">Pominięto (brak parametrów): #' +
              found.skipped.slice(0, 8).map(id => escapeHtml(String(id))).join(', #') +
              (found.skipped.length > 8 ? ' i ' + (found.skipped.length - 8) + ' więcej' : '') + '</div>';
          }
        }

        templateConfig = config;
        templateId = foundId;
        templateIdInput.placeholder = 'wybrano automatycznie: #' + foundId;

        previewArea.style.textAlign = 'left';
        previewArea.innerHTML = note +
          '<div style="font-size:12px;color:#5f6368;margin-bottom:4px;">Towar wzorcowy <b>#' + escapeHtml(String(foundId)) +
          '</b> — ' + config.parameters.length + ' parametrów:</div>' +
    ...
    templateLoadBtn.addEventListener('click', () => {
      const raw = templateIdInput.value.trim();
      if (raw && !/^\d+$/.test(raw)) {
        previewArea.style.textAlign = 'center';
        previewArea.innerHTML = '<div class="kcfg-info kcfg-info--error">ID towaru musi być liczbą.</div>';
        return;
      }
      loadTemplate(raw || null);
    });

    templateIdInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') { e.preventDefault(); templateLoadBtn.click(); }
    });

    perProductCb.addEventListener('change', updatePerProductHint);
    updatePerProductHint();
    loadTemplate(null);

    // Ręczna korekta przed zapisem — ta sama ścieżka co kopiowanie/import
    editorBtn.addEventListener('click', () => {
      if (templateConfig && templateConfig.parameters.length) showCreateMode(doc, selectedProductIds, templateConfig);
    });

    runBtn.addEventListener('click', () => {
      const perProduct = perProductCb.checked;
      const opts = {
        ...
        background: body.querySelector('#kcfg-fp-background').checked,
      };

      if (!perProduct && (!templateConfig || !templateConfig.parameters.length)) {
        previewArea.style.textAlign = 'center';
        previewArea.innerHTML = '<div class="kcfg-info kcfg-info--error">Brak towaru wzorcowego — wskaż ID towaru, ' +
          'z którego mają pochodzić parametry, albo włącz „Każdy towar ze swoich parametrów".</div>';
        return;
      }

      const nestingRules = collectRules();

      const items = selectedProductIds.map(id => ({
        productId: id,
        label: 'Towar #' + id,
        configData: perProduct ? null : templateConfig,
        fromProductParams: perProduct,
        nestingRules,
      }));
  ...
  function showCreateMode(doc, selectedProductIds, preloadedConfigData) {
    ...
    body.innerHTML =
      ...
      '<div class="kcfg-options" style="margin-top:12px;">' +
        ...
        '<div class="kcfg-option-row" style="flex-direction:column;align-items:flex-start;gap:5px;">' +
          '<label style="display:flex;align-items:center;gap:10px;cursor:pointer;"><span class="kcfg-switch"><input type="checkbox" id="kcfg-opt-background"><span class="kcfg-switch-track"><span class="kcfg-switch-thumb"></span></span></span> Uruchom jako zadanie w tle</label>' +
          '<div style="font-size:11px;color:#5f6368;line-height:1.5;padding-left:46px;">Okno si\u0119 zamknie, post\u0119p poka\u017ce si\u0119 w karcie w rogu. <b>Karty przegl\u0105darki nie wolno zamkn\u0105\u0107</b> \u2014 operacja przerwie si\u0119 w po\u0142owie.</div>' +
        '</div>' +
      '</div>';
    ...
    footer.querySelector('#kcfg-apply-btn').addEventListener('click', () => {
      ...
      const background = body.querySelector('#kcfg-opt-background').checked;
      executeCreate(doc, selectedProductIds, configData, updateExisting, autoType, hideParams, body, footer, background);
    });
  }
  ...
  function showImportFileModal(doc) {
    ...
    body.innerHTML =
      ...
      '<div id="kcfg-import-target" style="display:none;margin-top:16px;">' +
        ...
        '<div class="kcfg-options">' +
          ...
          '<div class="kcfg-option-row" style="flex-direction:column;align-items:flex-start;gap:5px;">' +
            '<label style="display:flex;align-items:center;gap:10px;cursor:pointer;"><span class="kcfg-switch"><input type="checkbox" id="kcfg-imp-background"><span class="kcfg-switch-track"><span class="kcfg-switch-thumb"></span></span></span> Uruchom jako zadanie w tle</label>' +
            '<div style="font-size:11px;color:#5f6368;line-height:1.5;padding-left:46px;">Okno si\u0119 zamknie, post\u0119p poka\u017ce si\u0119 w karcie w rogu. <b>Karty przegl\u0105darki nie wolno zamkn\u0105\u0107</b> \u2014 operacja przerwie si\u0119 w po\u0142owie.</div>' +
          '</div>' +
        '</div>' +
      '</div>';
    ...
    function runImport() {
      ...
      const background = body.querySelector('#kcfg-imp-background').checked;
      ...
      if (targetMode === 'existing') {
        ...
        const items = parsedProducts.map((p, i) => {
          ...
          return { label: name, configData: p.configData, createName: name };
        });
        // createNewProduct od razu zak\u0142ada towar typu konfigurator \u2014 autoType zb\u0119dne
        runConfiguratorBatch(doc, items, { autoType: false, updateExisting: false, hideParams, background },
  ...
  async function ensureProductTypeConfigurable(productId, statusCallback) {
    ...
    // Zrzut stan\u00f3w PRZED zerowaniem \u2014 jedyna kopia na czas, gdy towar jest "pusty".
    // Je\u015bli operacja padnie mi\u0119dzy zerowaniem a przywr\u00f3ceniem, skrypt odtworzy stany
    // z tego zapisu przy nast\u0119pnym wej\u015bciu na list\u0119 towar\u00f3w.
    storeSet(STORE_KEY_STOCK, { productId: String(productId), sizes, warehouseMap, savedAt: new Date().toISOString() });
    ...
    await restoreStocks(productId, sizes, warehouseMap);

    storeClear(STORE_KEY_STOCK);
    console.log('[PanelPRO] Zmieniono typ towaru #' + productId + ' na konfigurator (stany przywrocone)');
    return { changed: true, hadStock: true };
  }

  // Przywracanie stan\u00f3w \u2014 u\u017cywane po zmianie typu ORAZ przy odtwarzaniu
  // po przerwanej operacji (ten sam zapis, ta sama droga).
  async function restoreStocks(productId, sizes, warehouseMap, onStatus) {
    let done = 0;
    for (const size of sizes) {
      for (const [whName, qty] of Object.entries(size.stocks)) {
        const whId = warehouseMap[whName];
        if (!whId) continue;
        if (onStatus) onStatus('Przywracanie stan\u00f3w #' + productId + ' (' + (++done) + ')...');
        if (qty === 'jest') {
  ...
  // --- Raport post\u0119pu: to samo API, dwa wcielenia -------------------------

  const PROGRESS_HTML =
    '<div class="kcfg-progress-text" id="kcfg-prog-text">Rozpoczynanie...</div>' +
    '<div class="kcfg-progress" style="height:14px;margin-top:10px;"><div class="kcfg-progress-fill" id="kcfg-prog-bar"></div></div>';

  function createModalReporter(doc, body, footer, onStop) {
    body.innerHTML = '<div style="padding:20px;">' + PROGRESS_HTML + '</div>';
    footer.innerHTML = '<button class="kcfg-btn-stop" id="kcfg-stop">Stop</button>';
    footer.querySelector('#kcfg-stop').addEventListener('click', onStop);
    ...
    return {
      ...
      setProgress: (pct) => { barInner.style.width = pct + '%'; },
      done: (results, title) => showExecutionResults(doc, body, footer, results, title),
    };
  }

  // P\u0142ywaj\u0105ca karta \u2014 modal si\u0119 zamyka, operacja leci dalej, panel zostaje wolny.
  function createFloatingReporter(doc, title, onStop) {
    const card = doc.createElement('div');
    card.className = 'kcfg-bg-task';
    card.innerHTML =
      '<div class="kcfg-bg-task-head"><span class="kcfg-spinner"></span><span>' + escapeHtml(title) + '</span></div>' +
      '<div class="kcfg-bg-task-body">' + PROGRESS_HTML + '</div>' +
      '<div class="kcfg-bg-task-foot"><button class="kcfg-btn-stop">Stop</button></div>';
    doc.body.appendChild(card);

    const head = card.querySelector('.kcfg-bg-task-head');
    const cardBody = card.querySelector('.kcfg-bg-task-body');
    const cardFoot = card.querySelector('.kcfg-bg-task-foot');
    cardFoot.querySelector('button').addEventListener('click', onStop);

    const statusText = card.querySelector('#kcfg-prog-text');
    const barInner = card.querySelector('#kcfg-prog-bar');

    return {
      setStatus: (t) => { statusText.textContent = t; },
      setProgress: (pct) => { barInner.style.width = pct + '%'; },
      done: (results, doneTitle) => {
        const spinner = head.querySelector('.kcfg-spinner');
        if (spinner) spinner.remove();
        cardBody.style.padding = '0';
        showExecutionResults(doc, cardBody, cardFoot, results, doneTitle, () => card.remove());
      },
    };
  }

  // Ostrze\u017cenie przegl\u0105darki przy zamykaniu karty w trakcie operacji.
  // Nie da si\u0119 utrzyma\u0107 operacji po zamkni\u0119ciu karty \u2014 userscript ginie razem z ni\u0105.
  let _runningBatches = 0;
  let _unloadGuardAttached = false;

  function attachUnloadGuard() {
    if (_unloadGuardAttached) return;
    _unloadGuardAttached = true;
    const handler = (e) => {
      if (_runningBatches === 0) return;
      e.preventDefault();
      e.returnValue = '';
      return '';
    };
    try { window.addEventListener('beforeunload', handler); } catch (_e) { /* ignore */ }
    try {
      const win = getIframeWin();
      if (win && win !== window) win.addEventListener('beforeunload', handler);
    } catch (_e) { /* ignore */ }
  }

  // Wsp\u00f3lna p\u0119tla wsadowa: pasek post\u0119pu, przycisk Stop, zbieranie b\u0142\u0119d\u00f3w, podsumowanie.
  // items: [{ label, configData, productId, createName, fromProductParams }]
  //   productId         \u2014 towar docelowy (istniej\u0105cy)
  //   createName        \u2014 nazwa towaru, kt\u00f3ry ma dopiero powsta\u0107
  //   fromProductParams \u2014 configData odczyta\u0107 z parametr\u00f3w tego towaru
  // Pozycje s\u0105 celowo bez funkcji \u2014 dzi\u0119ki temu kolejk\u0119 da si\u0119 zapisa\u0107 i wznowi\u0107.
  // opts.background \u2014 true: modal si\u0119 zamyka, post\u0119p w p\u0142ywaj\u0105cej karcie
  async function runConfiguratorBatch(doc, items, opts, body, footer, title, resume) {
    const abortController = new AbortController();
    const signal = abortController.signal;
    ...
    if (opts.background) {
      closeModal(doc);
      reporter = createFloatingReporter(doc, title, onStop);
    } else {
      reporter = createModalReporter(doc, body, footer, onStop);
    }

    const statusText = { set textContent(v) { reporter.setStatus(v); } };
    const barInner = { style: { set width(v) { reporter.setProgress(parseInt(v, 10) || 0); } } };

    attachUnloadGuard();
    _runningBatches++;

    // Wspólny konfigurator zapisujemy raz, nie przy każdej pozycji
    const shared = items.length > 0 && items.every(it => it.configData === items[0].configData)
      ? items[0].configData : null;

    const results = resume && resume.results
      ? Object.assign({ success: 0, errors: [], createdIds: [] }, resume.results, { total: items.length, aborted: false })
      : { success: 0, errors: [], total: items.length, aborted: false, createdIds: [] };

    const saveProgress = (nextIndex) => storeSet(STORE_KEY_BATCH, {
      v: 1,
      title,
      opts: { autoType: opts.autoType, updateExisting: opts.updateExisting, hideParams: opts.hideParams },
      index: nextIndex,
      results: { success: results.success, errors: results.errors, createdIds: results.createdIds },
      sharedConfig: shared,
      items: items.map(it => ({
        productId: it.productId || null,
        label: it.label || null,
        createName: it.createName || null,
        fromProductParams: !!it.fromProductParams,
        nestingRules: it.nestingRules || null,
        configData: (shared && it.configData === shared) ? null : (it.configData || null),
      })),
    });

    const startIndex = (resume && resume.startIndex) || 0;
    let skippedRules = 0;   // reguły zagnieżdżeń niepasujące do konkretnych towarów
    ...
    for (let i = startIndex; i < items.length; i++) {
      ...
      try {
        // configData może być wyliczane dopiero teraz (odczyt parametrów towaru)
        let configData = item.configData;
        if (!configData && item.fromProductParams) {
          statusText.textContent = prefix + label + ' — odczyt parametrów...';
          configData = await fetchProductParametersAsConfig(item.productId);
        }
        ...
        // Zagnieżdżenia z reguł — po nazwach, na kopii, per towar.
        // Reguła niepasująca do tego towaru jest pomijana, nie przerywa operacji.
        if (item.nestingRules && item.nestingRules.length > 0) {
          configData = cloneConfig(configData);
          const nest = applyNestingRules(configData, item.nestingRules);
          if (nest.skipped.length > 0) skippedRules += nest.skipped.length;
        }

        let productId = item.productId;
        if (item.createName) {
          productId = await createNewProduct(item.createName);
      ...
      if (i < items.length - 1) await sleep(item.createName ? 500 : 300);
    }

    _runningBatches = Math.max(0, _runningBatches - 1);
    if (skippedRules > 0) results.skippedRules = skippedRules;
    // Przerwane zostaje do wznowienia, dokończone znika
    if (!results.aborted) storeClear(STORE_KEY_BATCH);
    reporter.done(results, title);
    return results;
  }

  // Wznowienie przerwanej kolejki — leci w tle, od pozycji, na której stanęła
  function resumePendingBatch(doc, state) {
    const items = (state.items || []).map(it => ({
      ...
      fromProductParams: it.fromProductParams,
      nestingRules: it.nestingRules || null,
      configData: it.configData || state.sharedConfig || null,
    }));
    const opts = Object.assign({}, state.opts, { background: true });
    return runConfiguratorBatch(doc, items, opts, null, null, state.title || 'Wznowiona operacja', {
  ...
  function executeCreate(doc, selectedProductIds, configData, updateExisting, autoType, hideParams, body, footer, background) {
    const items = selectedProductIds.map(id => ({ productId: id, configData, label: 'Towar #' + id }));
    return runConfiguratorBatch(
      doc, items, { autoType, updateExisting, hideParams, background }, body, footer,
  ...
  function showExecutionResults(doc, body, footer, results, title, onClose) {
    ...
    if (results.skippedRules > 0) {
      html += '<div style="font-size:12px;color:#856404;background:#fff3cd;border-radius:6px;padding:8px 10px;margin:8px 0;">' +
        'Pominięto <b>' + results.skippedRules + '</b> zastosowań reguł zagnieżdżenia — te towary nie miały ' +
        'pasującego parametru lub wartości. Konfigurator powstał u nich bez tego zagnieżdżenia.</div>';
    }
    ...
    closeBtn.addEventListener('click', () => (onClose ? onClose() : closeModal(doc)));
    footer.appendChild(closeBtn);
  }

  // =========================================================================
  // PO PRZERWANEJ OPERACJI
  // =========================================================================

  function showPendingWorkCard(doc) {
    if (_runningBatches > 0) return;              // operacja trwa teraz, nie jest przerwana
    if (doc.querySelector('#kcfg-pending-card')) return;

    const stock = storeGet(STORE_KEY_STOCK);
    const batch = storeGet(STORE_KEY_BATCH);
    if (!stock && !batch) return;

    injectStyles(doc);

    const card = doc.createElement('div');
    card.id = 'kcfg-pending-card';
    card.className = 'kcfg-bg-task';

    let html = '<div class="kcfg-bg-task-head"><span>⚠ Przerwana operacja</span></div><div class="kcfg-bg-task-body">';
    if (stock) {
      html += '<div class="kcfg-info kcfg-info--warning" style="margin-bottom:8px;font-size:12px;">' +
        'Towar <b>#' + escapeHtml(String(stock.productId)) + '</b> mógł zostać z wyzerowanymi stanami — ' +
        'zmiana typu przerwała się przed ich przywróceniem. Kopia stanów jest zapisana.</div>';
    }
    if (batch) {
      const total = (batch.items || []).length;
      const done = batch.index || 0;
      html += '<div style="font-size:12px;color:#3c4043;line-height:1.5;">' + escapeHtml(batch.title || 'Operacja') +
        '<br>Przerobione <b>' + done + '</b> z <b>' + total + '</b>, zostało <b>' + Math.max(0, total - done) + '</b>.</div>';
    }
    html += '</div><div class="kcfg-bg-task-foot"></div>';
    card.innerHTML = html;
    doc.body.appendChild(card);

    const foot = card.querySelector('.kcfg-bg-task-foot');
    const cardBody = card.querySelector('.kcfg-bg-task-body');
    ...
    addBtn('Odrzuć', 'kcfg-btn-secondary', () => {
      storeClear(STORE_KEY_STOCK);
      storeClear(STORE_KEY_BATCH);
      card.remove();
    });

    if (stock) {
      addBtn('Przywróć stany', 'kcfg-btn-primary', async () => {
        foot.innerHTML = '';
        cardBody.innerHTML = '<div style="font-size:12px;color:#5f6368;"><span class="kcfg-spinner"></span> ' +
          '<span id="kcfg-restore-status">Przywracanie stanów...</span></div>';
        const st = cardBody.querySelector('#kcfg-restore-status');
        try {
          await restoreStocks(stock.productId, stock.sizes || [], stock.warehouseMap || {},
            (m) => { st.textContent = m; });
          storeClear(STORE_KEY_STOCK);
          cardBody.innerHTML = '<div class="kcfg-info kcfg-info--success" style="font-size:12px;">Stany towaru #' +
            escapeHtml(String(stock.productId)) + ' przywrócone.</div>';
        } catch (e) {
          cardBody.innerHTML = '<div class="kcfg-info kcfg-info--error" style="font-size:12px;">Błąd przywracania: ' +
            escapeHtml(e.message) + '</div>';
        }
        addBtn('Zamknij', 'kcfg-btn-primary', () => card.remove());
      });
    }

    if (batch) {
      addBtn('Wznów', 'kcfg-btn-primary', () => {
        card.remove();
        resumePendingBatch(doc, batch);
      });
    }
  }
