...
(function () {
  ...
  function cleanBlogDetails(d) {
    ...
    Object.entries(d).forEach(([k, v]) => {
      ...
      // Wpis musi mieć id, które po stringifikacji odpowiada kluczowi
      // (chroni przed Set/obiektami zapisanymi jako "[object Set]")
      if (v.id != null && String(v.id) !== String(k)) return;
      // Musi mieć sensowną strukturę — name jako obiekt z polami lang
      // (albo pole error gdy scrape się nie udał)
      if (!v.error && (!v.name || typeof v.name !== 'object')) return;
      out[k] = v;
    });
    return out;
  }
