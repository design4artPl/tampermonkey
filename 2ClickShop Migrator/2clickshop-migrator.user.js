...
(function () {
  ...
  // ============================================================================
  // KLUCZOWE: 2ClickShop ładuje MooTools (engine.js) który nadpisuje Array.from
  // tak że Array.from(Set) zwraca [Set] zamiast wartości — Set nie jest dla
  // MooTools "enumerable". Spread [...set] używa natywnego @@iterator i działa.
  // safeArrayFrom obchodzi MooTools dla Set/Map/NodeList/HTMLCollection.
  // ============================================================================
  function safeArrayFrom(iterable) {
    ...
    if (typeof iterable.length === 'number') {
      const out = [];
      for (let i = 0; i < iterable.length; i++) out.push(iterable[i]);
      return out;
    }
    return [];
  }
  ...
  function cleanBlogDetails(d) {
    ...
    Object.entries(d).forEach(([k, v]) => {
      if (!/^\d+$/.test(String(k))) { removed++; return; }
      if (!v || typeof v !== 'object') { removed++; return; }
      if (v.id != null && String(v.id) !== String(k)) { removed++; return; }
      if (!v.error && (!v.name || typeof v.name !== 'object')) { removed++; return; }
      out[k] = v;
    });
    if (removed > 0) {
      console.log(`[Migrator] cleanBlogDetails: usunięto ${removed} śmieciowych wpisów z cache`);
      // Persist od razu, żeby nie odrabiać następnej sesji
      try { GM_setValue('tcm_blogDetails', out); } catch (e) {}
    }
    return out;
  }
  ...
  // BLOGS MODULE
  // =========================================================================
  // 2ClickShop wstawia w komórkach listy literalny tekst "[Brak nazwy]" / "[Brak]"
  // gdy wpis nie ma tłumaczenia w bieżącym języku. Traktujemy jak pustkę.
  function isBlankName(s) {
    if (!s) return true;
    const t = s.trim();
    if (!t) return true;
    return /^\[\s*brak(\s+nazwy)?\s*\]$/i.test(t);
  }
  function cleanListName(s) {
    return isBlankName(s) ? '' : (s || '').trim();
  }
  ...
  // Zwraca tylko valid wpisy z blogDetails — filtruje śmieci typu
  // "[object Set]" / brak struktury name. Używać wszędzie zamiast
  // Object.values(state.blogDetails).
  function validBlogDetailValues() {
    return Object.entries(state.blogDetails || {})
      .filter(([k, v]) => /^\d+$/.test(String(k)) && v && typeof v === 'object'
        && (v.error || (v.name && typeof v.name === 'object')))
      .map(([, v]) => v);
  }
  ...
  function renderBlogs() {
    ...
    const validDetails = validBlogDetailValues();
    BLOG_LANGS.forEach(code => {
      const detailsCountForLang = validDetails
    ...
    // Bieżący język — używany w sekcji eksportu i niżej w liście
    ...
    // Eksport tylko bieżącego języka — preferuj pełne detale, fallback do basic
    const langDetailsCount = validBlogDetailValues()
      .filter(d => d.name && d.name[lang] && !isBlankName(d.name[lang])).length;
    const langBasicCount = (state.blogData[lang] || []).filter(b => b.hasTranslation).length;
    const langCountText = langDetailsCount > 0
      ? `${langDetailsCount} pełnych detali`
      : (langBasicCount > 0 ? `${langBasicCount} basic (etap 2 nie pobrany)` : '0');
    wrap.appendChild(el('div', { style: { fontSize: '11px', color: '#475569', padding: '8px 4px 2px', fontWeight: '600' } },
      `Eksport ${lang.toUpperCase()} (${langCountText})`));
    const exportsLang = el('div', { class: 'tcm-actions' });
    exportsLang.appendChild(el('button', { class: 'tcm-btn tcm-btn-secondary', onClick: () => exportBlog('json', lang) }, 'JSON'));
    exportsLang.appendChild(el('button', { class: 'tcm-btn tcm-btn-secondary', onClick: () => exportBlog('csv', lang) }, 'CSV'));
    exportsLang.appendChild(el('button', { class: 'tcm-btn tcm-btn-secondary', onClick: () => exportBlog('xml', lang) }, 'XML'));
    wrap.appendChild(exportsLang);

    // Eksport wszystkich języków
    const allBasicCount = BLOG_LANGS.reduce((s, l) => s + (state.blogData[l] || []).length, 0);
    const allCountText = detailsCount > 0
      ? `${detailsCount} pełnych detali`
      : (allBasicCount > 0 ? `${allBasicCount} basic (etap 2 nie pobrany)` : '0');
    wrap.appendChild(el('div', { style: { fontSize: '11px', color: '#475569', padding: '8px 4px 2px', fontWeight: '600' } },
      `Eksport wszystkich języków (${allCountText})`));
    ...
    const detailsArr = validBlogDetailValues();
    let list;
    let listSource = '';
  ...
  function renderBlogStats(box) {
    ...
    BLOG_LANGS.forEach(lang => {
      ...
      // Preferuj wartości policzone z pełnej listy (jeśli pobrana),
      // inaczej użyj preview-stats (sample z pierwszej + ostatniej strony)
      const translated = listData.length > 0
        ? listData.filter(b => b.hasTranslation).length
        : (info.translatedCount || 0);
      const totalRows = listData.length || info.exactCount || 0;
      const detailsTranslated = validBlogDetailValues()
      ...
      const row = el('div', { class: 'tcm-stats-row' }, [
        ...
        el('span', { class: 'tcm-stats-count' },
          totalRows
            ? `${translated}/${totalRows} tłumaczeń`
            : ''),
  ...
  async function previewBlogStats() {
    ...
    const results = await Promise.all(BLOG_LANGS.map(async lang => {
      try {
        // Page 1: liczymy wiersze ORAZ ile z nich ma prawdziwą nazwę
        // (panel admina pokazuje wszystkie wpisy w każdym języku z "[Brak nazwy]"
        // jako placeholder — exactCount = wszystkie, translatedCount = z nazwą)
        const countTranslated = (d) => safeArrayFrom(d.querySelectorAll("tr[id^='news_']"))
          .filter(tr => cleanListName(tr.querySelectorAll('td')[1]?.textContent).length > 0)
          .length;
        ...
        const translatedOnFirst = countTranslated(doc);
        ...
        let translatedCount = translatedOnFirst;
        if (pages > 1) {
          ...
          const translatedOnLast = countTranslated(docL);
          exactCount = (pages - 1) * rowsOnFirst + rowsOnLast;
          // Założenie: średnio tyle samo przetłumaczonych na środkowych stronach
          translatedCount = (pages - 1) * translatedOnFirst + translatedOnLast;
        }
        return [lang, { pages, rowsOnFirst, exactCount, translatedCount }];
      } catch (e) {
  ...
  function buildBlogUrl(page, lang) {
    // UWAGA: Wymaga jawnego &lang=XX dla każdego języka (w tym pl). Bez tego
    // panel admina zwraca dane w języku ostatnio wybranym w sesji panelu —
    // identyczny bug jak był w buildReviewUrl (naprawiony w v1.24).
    return `/admin/index.php?k=news&w=blog&page=${page}&lang=${lang || 'pl'}`;
  }
    ...
    try {
      ...
      const parseRows = (doc) => {
        ...
        doc.querySelectorAll("tr[id^='news_']").forEach(tr => {
          ...
          seenIds.add(id);
          const name = cleanListName(tds[1]?.textContent).slice(0, 200);
          out.push({
            id,
            order: tds[0]?.textContent.trim().replace('.', '') || '',
            name,
            key: tds[2]?.textContent.trim() || '',
            category: cleanListName(tds[4]?.textContent),
  ...
  async function scrapeBlogPostDetail(id) {
    ...
    // Popup edycji — &lang=pl jawnie (część endpointów 2ClickShop bez lang zwraca pustkę)
    const r1 = await fetch(`/admin/popup.php?k=news&w=blog&id=${id}&lang=pl`, { credentials: 'include' });
    if (!r1.ok) throw new Error(`HTTP ${r1.status} popup id=${id}`);
    const html1 = await r1.text();
    // Heurystyka: jeśli odpowiedź to login page, etap 2 nie zadziała wcale
    if (html1.includes('ZALOGUJ SIĘ DO PANELU') || html1.length < 500) {
      throw new Error(`popup id=${id}: sesja wygasła lub pusta odpowiedź (${html1.length} B)`);
    }
  ...
  async function scrapeBlogAll(sourceLang) {
    ...
    // Clean start: usuń ze stanu dane języków które pobieramy — chroni przed
    // leakage śmieciowych wpisów ze starych buggy wersji (np. id=Set zapisane
    // przed v1.35). Po nadpisaniu state.blogData[lang] = perLangList stare
    // dane są zastąpione, ale jeśli scrape rzuci wcześniej, śmieci zostaną.
    langs.forEach(lang => { state.blogData[lang] = []; });
    ...
    box.appendChild(label);
    box.appendChild(bar);
    const errBox = el('div', { id: 'tcm-blog-err', style: { display: 'none', marginTop: '8px', padding: '6px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '4px', fontSize: '11px', color: '#991b1b', wordBreak: 'break-all' } });
    ...
    try {
      ...
      const ids = safeArrayFrom(idsSet).sort((a, b) => parseInt(a) - parseInt(b));

      // DIAGNOSTYKA: flat log (Playwright nie czyta JSHandle@object)
      window.__tcm = state;
      console.log('[diag] idsSet.size=' + idsSet.size);
      console.log('[diag] ids.length=' + ids.length);
      ids.slice(0, 5).forEach((x, i) => {
        console.log(`[diag] ids[${i}]: typeof=${typeof x}, ` +
          `isSet=${x instanceof Set}, isArray=${Array.isArray(x)}, ` +
          `str=${String(x).slice(0, 50)}`);
      });
      console.log('[diag] state.blogData.pl.length=' + (state.blogData.pl?.length ?? 'undef'));
      (state.blogData.pl || []).slice(0, 3).forEach((b, i) => {
        console.log(`[diag] blogData.pl[${i}]: id-typeof=${typeof b?.id}, ` +
          `id-str=${String(b?.id).slice(0, 30)}, name=${(b?.name || '').slice(0, 30)}`);
      });
      console.log('[diag] window.__tcm dostępne — sprawdź w konsoli');
      ...
      for (const id of ids) {
        ...
        // Ochrona: jeśli id nie jest stringiem/numerem, pomijamy z ostrzeżeniem
        // (chroni przed śmieciem typu Set który dotąd zatrzymywał scrape)
        if (typeof id !== 'string' && typeof id !== 'number') {
          console.warn('[Migrator] etap 2: pomijam non-string id:', id, typeof id);
          skipped++;
          continue;
        }
        const idStr = String(id);
        if (!/^\d+$/.test(idStr)) {
          console.warn('[Migrator] etap 2: pomijam niepoprawne id:', idStr);
          skipped++;
          continue;
        }
        label.textContent = `Pełne dane ${done}/${ids.length} (id=${idStr})${errors ? ` — błędów: ${errors}` : ''}`;
        try {
          ...
          if (!firstErrorMsg) firstErrorMsg = `${e.message || e}`.slice(0, 200);
          // Pokaż pierwszy błąd w UI od razu, żeby user wiedział co się dzieje
          if (errors === 1 || errors === 5 || errors === 20) {
            const errBox = document.getElementById('tcm-blog-err');
            if (errBox) {
              errBox.textContent = `⚠ Pierwszy błąd: ${firstErrorMsg}`;
      ...
      fill.style.width = '100%';
      const label2 = `${errors === done ? '✗' : '✓'} Pobrano ${done - errors}/${ids.length} wpisów`
      ...
      if (errors > 0) {
        const errBox = document.getElementById('tcm-blog-err');
        if (errBox) {
          errBox.textContent = `⚠ ${errors} z ${done} wpisów z błędem. Pierwszy: ${firstErrorMsg}`;
          errBox.style.display = 'block';
        }
      }
  ...
  // -------------------------------------------------------------------------
  // langFilter: null/undefined = wszystkie wpisy, string ('pl'/'en'/...) = tylko z tłumaczeniem w tym języku
  function exportBlog(format, langFilter) {
    let detailsArr = validBlogDetailValues().sort((a, b) => parseInt(a.id) - parseInt(b.id));
    // Filtrowanie per-lang: tylko wpisy z prawdziwą nazwą w tym języku
    if (langFilter) {
      detailsArr = detailsArr.filter(d => {
        const n = d.name && d.name[langFilter];
        return n && n.trim() && !isBlankName(n);
      });
    }
    ...
    if (detailsArr.length > 0) {
      if (format === 'json') {
        ...
        blob = new Blob(['﻿' + blogToCSV(detailsArr, langFilter)], { type: 'text/csv;charset=utf-8' });
        ext = 'csv';
      } else if (format === 'xml') {
        blob = new Blob([blogToXML(detailsArr, langFilter)], { type: 'application/xml' });
        ext = 'xml';
      }
    } else {
      // Fallback: eksport podstawowej listy z etapu 1 (jeśli etap 2 nie zakończony)
      let list = langFilter
        ? (state.blogData[langFilter] || [])
        : BLOG_LANGS.flatMap(l => (state.blogData[l] || []).map(b => ({...b, _lang: l})));
      if (langFilter) {
        list = list.filter(b => b.hasTranslation);
      }
      if (!list.length) {
        alert(langFilter
          ? `Brak wpisów z tłumaczeniem ${langFilter.toUpperCase()} oraz brak pobranych detali. Pobierz wpisy lub etap 2 (popup detali).`
    ...
    a.download = `blogs${langSuffix}_${detailsArr.length ? 'full' : 'basic'}_${ts}.${ext}`;
    a.click();
  ...
  function blogToCSV(arr, langFilter) {
    // Per-lang: kolumny tylko dla wybranego języka. Per-all: 7 języków × wszystkie pola.
    const langsToExport = langFilter ? [langFilter] : BLOG_LANGS;
    const cols = ['id', 'date', 'author', 'sub', 'photo_banner'];
    langsToExport.forEach(l => cols.push(`name_${l}`));
    langsToExport.forEach(l => cols.push(`link_${l}`));
    langsToExport.forEach(l => cols.push(`key_${l}`));
    langsToExport.forEach(l => cols.push(`visible_${l}`));
    langsToExport.forEach(l => cols.push(`category_${l}`));
    langsToExport.forEach(l => cols.push(`photo_icon_${l}`));
    langsToExport.forEach(l => cols.push(`desc_${l}`));
    langsToExport.forEach(l => cols.push(`seo_title_${l}`));
    langsToExport.forEach(l => cols.push(`seo_description_${l}`));
    langsToExport.forEach(l => cols.push(`seo_keywords_${l}`));
    langsToExport.forEach(l => cols.push(`seo_noindex_${l}`));
    langsToExport.forEach(l => cols.push(`attachments_name_${l}`));
    langsToExport.forEach(l => cols.push(`attachments_url_${l}`));
  ...
  function blogToXML(arr, langFilter) {
    const langsToExport = langFilter ? [langFilter] : BLOG_LANGS;
    let xml = '<?xml version="1.0" encoding="UTF-8"?>\n<blogs>\n';
    arr.forEach(d => {
      xml += `  <blog id="${d.id}">\n`;
  ...
  function clearBlogList() {
    if (!confirm('Wyczyścić wszystkie pobrane dane blogów (listy + detale + statystyki)?')) return;
    state.blogData = {};
    state.blogDetails = {};
    ...
    console.log('[Migrator] clearBlogList: stan blogu wyczyszczony');
    renderBlogs();
  }
    ...
    rows.forEach((tr, idx) => {
      ...
      if (!_productDiagDumped && idx === 0) {
        ...
        console.log('[Migrator][diag] td[] textContent:',
          safeArrayFrom(tds).map((td, i) => `td[${i}]="${td.textContent.trim().slice(0, 80)}"`));
  ...
  function renderReviews() {
    ...
    // Drugi rząd: Uzupełnij brakujące produkty (fetch /order/product-rate.html)
    if (slugCount > 0) {
      const fillBtns = el('div', { style: { display: 'flex', gap: '6px', padding: '4px' } });
      fillBtns.appendChild(el('button', {
        class: 'tcm-btn tcm-btn-sm tcm-btn-secondary',
        style: { flex: '1' },
        onClick: fillMissingFromOrderRate,
      }, 'Uzupełnij brakujące produkty (z /order/product-rate)'));
      cfgBox.appendChild(fillBtns);
    }
  ...
  // Drugi przebieg: dla każdej nierozwiązanej opinii fetchuje stronę
  // /order/product-rate.html (gdy URL na to wskazuje) i wyciąga slug produktu
  // z OG meta lub canonical link. Wynik wzbogaca review.productSlug i
  // ponownie zapisuje do state.reviewData.
  async function fillMissingFromOrderRate() {
    const langs = BLOG_LANGS;
    const todo = [];
    langs.forEach(lang => {
      (state.reviewData[lang] || []).forEach((r, idx) => {
        if (resolveProductId(r)) return; // już rozwiązana
        const isOrderRate = r.productUrl && /\/order\/product-rate\.html/i.test(r.productUrl);
        if (!isOrderRate) return; // tylko ten typ URL
        todo.push({ lang, idx, review: r });
      });
    });
    if (!todo.length) {
      alert('Brak nierozwiązanych opinii z URL typu /order/product-rate.html.');
      return;
    }
    ...
    progress.innerHTML = '';
    const box = el('div', { class: 'tcm-progress' });
    const label = el('div', {}, `Uzupełnianie 0/${todo.length}...`);
    const bar = el('div', { class: 'tcm-progress-bar' });
    const fill = el('div', { class: 'tcm-progress-fill', style: { width: '0%' } });
    bar.appendChild(fill);
    box.appendChild(label);
    ...
    for (const item of todo) {
      ...
      label.textContent = `Uzupełnianie ${done}/${todo.length}: review #${item.review.id}...`;
      try {
        const res = await fetch(item.review.productUrl, { credentials: 'include' });
        const html = await res.text();
        const doc = new DOMParser().parseFromString(html, 'text/html');
        // Próba 1: canonical link / og:url do strony produktu
        const candidates = [];
        const cano = doc.querySelector('link[rel="canonical"]');
        if (cano) candidates.push(cano.href);
        const ogUrl = doc.querySelector('meta[property="og:url"]');
        if (ogUrl) candidates.push(ogUrl.getAttribute('content'));
        // Próba 2: dowolny link do /produkt/X.html
        doc.querySelectorAll('a[href*="/produkt/"]').forEach(a => candidates.push(a.href));
        // Próba 3: og:title — nazwa produktu
        const ogTitle = doc.querySelector('meta[property="og:title"]');
        const titleText = ogTitle ? ogTitle.getAttribute('content') : '';
        // h1 nazwa produktu
        ...
        for (const c of candidates) {
          const s = extractProductSlug(c);
          if (s) { resolvedSlug = s; break; }
        }
        if (!resolvedName) resolvedName = titleText || h1Text || '';

        if (resolvedSlug || resolvedName) {
          // Zapisz do review (mutating state — następnie persist)
          if (resolvedSlug) item.review.productSlug = resolvedSlug;
          if (resolvedName && !item.review.productName) item.review.productName = resolvedName;
          filled++;
        } else {
          failed++;
        }
      } catch (e) {
        console.warn('[Migrator] fillMissing error', item, e);
        failed++;
      }
    ...
    GM_setValue('tcm_reviewData', state.reviewData);
    label.textContent = `✓ Uzupełniono ${filled}/${todo.length} (błędów: ${failed})`;
    fill.style.width = '100%';
    setTimeout(renderReviews, 800);
  }
  ...
  // Generuje warianty slugu do próby. Obsługuje przypadki gdy produkt
  // istnieje w katalogu pod podobnym slugiem ale z inną końcówką
  // (np. opinia ma "-5-sztuk", w katalogu jest "-1-szt" lub bez sufiksu).
  function slugVariants(slug) {
    if (!slug) return [];
    const variants = [slug];
    // Usuń końcówkę "-N-sztuk" / "-N-szt" / "-N-szt."
    const m1 = slug.match(/^(.+?)-\d+-(sztuk|szt\.?|sz)$/i);
    if (m1) variants.push(m1[1]);
    // Usuń końcówkę "-sztuk" / "-szt" bez liczby
    const m2 = slug.match(/^(.+?)-(sztuk|szt\.?)$/i);
    if (m2) variants.push(m2[1]);
    // Usuń końcówkę "-N" liczbowo (np. "-3g", "-100ml" sprawdzamy oddzielnie)
    const m3 = slug.match(/^(.+?)-\d+$/);
    if (m3) variants.push(m3[1]);
    return [...new Set(variants)];
  }

  function resolveProductId(review) {
    // 1. Slug — exact match (zapisane pole lub w locie z URL)
    ...
    // 2. Fuzzy slug — warianty bez końcówek liczbowych/sztuk
    if (slug) {
      for (const v of slugVariants(slug)) {
        if (v !== slug && state.productSlugMap[v]) return state.productSlugMap[v];
      }
    }
    // 3. Po nazwie (dla URL-i z order/product-rate bez slug w URL)
