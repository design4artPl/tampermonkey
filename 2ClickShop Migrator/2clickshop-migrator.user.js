...
(function () {
  ...
  function buildReviewUrl(page, lang) {
    let url = `/admin/index.php?k=comment&type=all&page=${page}`;
    if (lang && lang !== 'pl') url += `&lang=${lang}`;
    return url;
  }
  ...
  function parseProductRows(doc) {
    // Z listy produktów: tr#product_<ID>, td[2]=symbol, td[3]=nazwa, td[4]=slug
    const out = [];
    doc.querySelectorAll("tr[id^='product_']").forEach(tr => {
      const id = tr.id.replace('product_', '');
      const tds = tr.querySelectorAll('td');
      if (tds.length < 5) return;
      const name = tds[3]?.textContent.trim() || '';
      const slug = tds[4]?.textContent.trim() || '';
      // Slug w 2ClickShop to lowercase z myślnikami; sprawdzamy heurystycznie
      const isValidSlug = slug && /^[a-z0-9-]+$/i.test(slug) && !/^[a-f0-9]{32}$/i.test(slug);
      out.push({ id, name, slug: isValidSlug ? slug.toLowerCase() : '' });
    });
    return out;
  }
