...
(function () {
  ...
  function renderBlogs() {
    ...
    // Eksport tylko bieżącego języka — tylko wpisy z tłumaczeniem w tym lang
    ...
    wrap.appendChild(el('div', { style: { fontSize: '11px', color: '#475569', padding: '8px 4px 2px', fontWeight: '600' } },
      `Eksport ${lang.toUpperCase()} (${langDetailsCount} wpisów z tłumaczeniem)`));
    ...
    wrap.appendChild(el('div', { style: { fontSize: '11px', color: '#475569', padding: '8px 4px 2px', fontWeight: '600' } },
      `Eksport wszystkich języków (${detailsCount} wpisów)`));
  ...
  async function scrapeBlogAll(sourceLang) {
    ...
    try {
      ...
      // DIAGNOSTYKA: sprawdza gdzie się gubią ID. Po etap 1 powinno być ~372 ID
      // (sumarycznie z wszystkich pobieranych języków).
      console.log('[Migrator][diag] Etap 1 zakończony:', {
        idsSetSize: idsSet.size,
        idsLength: ids.length,
        firstIds: ids.slice(0, 5).map(x => ({ value: x, type: typeof x, str: String(x) })),
        blogDataKeys: Object.keys(state.blogData),
        blogDataPlLength: state.blogData.pl?.length,
        blogDataPlFirst: state.blogData.pl?.slice(0, 3)?.map(b => ({
          id: b.id,
          idType: typeof b.id,
          idStr: String(b.id),
