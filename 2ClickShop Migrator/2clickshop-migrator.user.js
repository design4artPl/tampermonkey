...
(function () {
  ...
  function buildProductUrl(page) {
    return `/admin/index.php?k=product&page=${page}`;
  }
  ...
  function renderReviews() {
    ...
    const mkInput = (label, key, width) => {
      const row = el('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 4px', fontSize: '12px' } }, [
        el('span', { style: { minWidth: '140px', color: '#475569' } }, label),
      ]);
      const inp = document.createElement('input');
      inp.type = 'text';
      inp.value = cfg[key] || '';
      inp.style.cssText = `flex:1;max-width:${width || '120px'};padding:4px 8px;border:1px solid #cbd5e1;border-radius:4px;font-size:12px;`;
    ...
    // shopId per język — grid 7 pól
    const shopIdHeader = el('div', { style: { padding: '8px 4px 4px', fontSize: '12px', fontWeight: '600', color: '#475569' } }, 'shopId per język opinii:');
    cfgBox.appendChild(shopIdHeader);
    const shopIdGrid = el('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '4px 12px', padding: '4px' } });
    BLOG_LANGS.forEach(lang => {
      const row = el('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px' } });
      row.appendChild(el('span', { style: { minWidth: '24px', color: '#475569', fontWeight: '600' } }, lang.toUpperCase()));
      const inp = document.createElement('input');
      inp.type = 'text';
      inp.value = (cfg.shopIdByLang && cfg.shopIdByLang[lang]) || '';
      inp.placeholder = lang === 'pl' ? '1' : '';
      inp.style.cssText = 'flex:1;padding:3px 6px;border:1px solid #cbd5e1;border-radius:4px;font-size:12px;';
      inp.addEventListener('change', () => {
      ...
      row.appendChild(inp);
      shopIdGrid.appendChild(row);
    });
    cfgBox.appendChild(shopIdGrid);

    cfgBox.appendChild(mkInput('language (gdy nieznany):', 'defaultLanguage', '80px'));

    // forceConfirmedByPurchase: select auto/true/false
    const cbpRow = el('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 4px', fontSize: '12px' } }, [
      el('span', { style: { minWidth: '140px', color: '#475569' } }, 'opinionConfirmedByPurchase:'),
    ]);
    const cbpSel = document.createElement('select');
    cbpSel.style.cssText = 'flex:1;max-width:200px;padding:4px 8px;border:1px solid #cbd5e1;border-radius:4px;font-size:12px;';
    [
    ...
    cbpRow.appendChild(cbpSel);
    cfgBox.appendChild(cbpRow);

    // Status mappingu produktów + przycisk pobierania
    const slugCount = Object.keys(state.productSlugMap).length;
    const nameCount = Object.keys(state.productNameMap).length;
    const mapRow = el('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 4px', fontSize: '12px' } }, [
      el('span', { style: { minWidth: '140px', color: '#475569' } }, 'Mapping products_id:'),
      el('span', { style: { flex: '1', color: slugCount ? '#16a34a' : '#dc2626' } },
        slugCount
          ? `✓ ${slugCount} po slug, ${nameCount} po nazwie`
          : '⚠ brak — products_id będzie pusty'),
    ]);
    mapRow.appendChild(el('button', {
      class: 'tcm-btn tcm-btn-sm tcm-btn-secondary',
    ...
    const exports = el('div', { class: 'tcm-actions' });
    exports.appendChild(el('button', { class: 'tcm-btn tcm-btn-secondary', onClick: () => exportReview('json') }, 'JSON'));
    exports.appendChild(el('button', { class: 'tcm-btn tcm-btn-secondary', onClick: () => exportReview('csv') }, 'CSV'));
    exports.appendChild(el('button', { class: 'tcm-btn tcm-btn-secondary', onClick: () => exportReview('xml') }, 'XML'));
    if (Object.values(state.reviewData).some(arr => (arr || []).length > 0)) {
      exports.appendChild(el('button', {
        class: 'tcm-btn tcm-btn-secondary',
    ...
    if (firstEntry && !state.reviewStats) {
      previewReviewStats();
    }
  }
  ...
  function normalizeProductName(name) {
    ...
    return name
      .toLowerCase()
      .replace(/^promocja!?\s*/i, '')         // usuń prefix "PROMOCJA!"
      .replace(/\s+/g, ' ')                    // wielokrotne spacje → 1
      .replace(/\s*[-–—]\s*/g, ' ')           // myślniki z otaczającymi spacjami → spacja
      .trim();
  }

  function resolveProductId(review) {
    // Najpierw po slug z URL
    if (review.productSlug && state.productSlugMap[review.productSlug]) {
      return state.productSlugMap[review.productSlug];
    }
  ...
  function diagnoseMapping() {
    ...
    allReviews.forEach(r => {
      let id = '';
      if (r.productSlug && state.productSlugMap[r.productSlug]) {
        id = state.productSlugMap[r.productSlug];
        resolvedBySlug++;
      ...
      if (!id) {
        ...
        if (unresolvedSamples.length < 10) {
          unresolvedSamples.push({
            reviewId: r.id,
            lang: r._lang,
            productSlug: r.productSlug || '(brak)',
            productName: r.productName || '(brak)',
    ...
    if (progress) {
      ...
      if (unresolvedSamples.length > 0) {
        box.appendChild(el('div', { style: { marginTop: '8px', fontSize: '11px', color: '#dc2626' } },
          `Przykład nierozwiązanej: "${unresolvedSamples[0].productName}" → slug: ${unresolvedSamples[0].productSlug}, normalized: "${unresolvedSamples[0].normalizedName}"`));
      }
      progress.appendChild(box);
    }
  }
  ...
  function exportReview(format) {
    ...
    if (!all.length) { alert('Brak danych do eksportu.'); return; }
