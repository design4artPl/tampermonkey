...
(function () {
  ...
  function parseReviewRows(doc) {
    ...
    doc.querySelectorAll("tr[id^='comment_']").forEach(tr => {
      ...
      out.push({
        ...
        productUrl: productLink ? productLink.href : '',
  ...
  function exportReview(format) {
    // Eksport: union ze wszystkich języków, opinie posortowane po id desc (najnowsze najpierw)
    // Każda opinia zawiera pole `lang` żeby było wiadomo z którego języka pochodzi
    const all = [];
    BLOG_LANGS.forEach(lang => {
      (state.reviewData[lang] || []).forEach(r => all.push({ ...r, lang }));
    });
    ...
    if (format === 'json') {
      ...
      const cols = ['id', 'lang', 'date', 'nick', 'email', 'rating', 'visible', 'ip', 'productName', 'productUrl', 'content'];
      const lines = [cols.join(';')];
      all.forEach(r => {
        lines.push(cols.map(c => csvCell(c === 'visible' ? (r[c] ? '1' : '0') : r[c])).join(';'));
      });
      ...
      let xml = '<?xml version="1.0" encoding="UTF-8"?>\n<reviews>\n';
      all.forEach(r => {
        xml += `  <review id="${r.id}" lang="${r.lang}">\n`;
        xml += `    <date>${xmlEscape(r.date)}</date>\n`;
        xml += `    <nick>${xmlEscape(r.nick)}</nick>\n`;
        xml += `    <email>${xmlEscape(r.email)}</email>\n`;
        xml += `    <rating>${xmlEscape(r.rating)}</rating>\n`;
        xml += `    <visible>${r.visible ? 'true' : 'false'}</visible>\n`;
        xml += `    <ip>${xmlEscape(r.ip)}</ip>\n`;
        xml += `    <product url="${xmlEscape(r.productUrl)}">${xmlEscape(r.productName)}</product>\n`;
