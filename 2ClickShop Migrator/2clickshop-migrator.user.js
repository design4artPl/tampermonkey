  ...
  function diagnoseMapping() {
      ...
      if (!id) {
        ...
        if (unresolvedSamples.length < 10) {
          unresolvedSamples.push({
            reviewId: r.id,
            lang: r._lang,
            productSlugStored: r.productSlug || '(brak)',
            productSlugFromUrl: extractProductSlug(r.productUrl) || '(brak)',
    ...
    if (progress) {
      ...
      if (unresolvedSamples.length > 0) {
        const s = unresolvedSamples[0];
        box.appendChild(el('div', { style: { marginTop: '8px', fontSize: '11px', color: '#dc2626', wordBreak: 'break-all' } },
          `Przykład nierozwiązanej: "${s.productName}"`));
        box.appendChild(el('div', { style: { fontSize: '11px', color: '#64748b', wordBreak: 'break-all' } },
          `  slug stored: ${s.productSlugStored}`));
        box.appendChild(el('div', { style: { fontSize: '11px', color: '#64748b', wordBreak: 'break-all' } },
          `  slug from URL: ${s.productSlugFromUrl}`));
        box.appendChild(el('div', { style: { fontSize: '11px', color: '#64748b', wordBreak: 'break-all' } },
          `  normalized name: "${s.normalizedName}"`));
        box.appendChild(el('div', { style: { fontSize: '11px', color: '#64748b', wordBreak: 'break-all' } },
          `  URL: ${s.productUrl || '(brak)'}`));

        // Znajdź 5 najbliższych kluczy w nameMap — pokazuje czy katalog ma w ogóle
        // ten produkt pod inną/podobną nazwą.
        const closest = findClosestKeys(s.normalizedName, nameKeys, 5);
        if (closest.length > 0) {
          box.appendChild(el('div', { style: { marginTop: '6px', fontSize: '11px', color: '#0369a1', fontWeight: '600' } },
            '  Najbliższe klucze w nameMap (substring/prefix):'));
          closest.forEach(k => {
            box.appendChild(el('div', { style: { fontSize: '11px', color: '#0369a1', wordBreak: 'break-all', paddingLeft: '12px' } },
              `  • "${k}" → id=${state.productNameMap[k]}`));
          });
        } else {
          box.appendChild(el('div', { style: { marginTop: '6px', fontSize: '11px', color: '#dc2626' } },
            '  Brak podobnych kluczy w nameMap — produkt prawdopodobnie nie istnieje w katalogu (lub nazwa jest zupełnie inna)'));
        }
      }
      progress.appendChild(box);
    }
