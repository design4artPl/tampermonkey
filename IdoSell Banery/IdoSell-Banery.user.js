  ...
  async function importFlow(file, dstShops) {
    ...
    for (const [srcShopId, dstId] of Object.entries(mapping)) {
      ...
      for (const srcRegion of REGIONS) {
        ...
        // Upload + attach the banner's images to an existing banner id (used by both
        // the "add" pass and the "update" pass). Pushes any failures to imageReminders.
        async function uploadImagesFor(bannerId, item) {
          const images = item._meta && item._meta.images;
          if (!bannerId || !images || Object.keys(images).length === 0) return;
          try {
            const uploaded = await uploadAndAttachImages(bannerId, dstId, images);
            const done = Object.keys(uploaded);
            const skip = Object.keys(images).filter(s => !done.includes(s));
        ...
        for (const item of items) {
          ...
          if (existingId && duplicateMode === 'update') {
            ...
            try {
              ...
              updated++;
              ok++;
              // Update path: re-upload images to the existing banner (the add-pass
              // image loop below only covers freshly created banners).
              await uploadImagesFor(existingId, item);
            } catch (e) {
              err++;
        ...
        for (let i = 0; i < addedItems.length; i++) {
          await uploadImagesFor(newIds[i], addedItems[i]);
        }
      }
