...
(function () {
    ...
    /** Zwraca ID zaznaczonych towarów ze WSZYSTKICH stron (z IAI.Table.checkedID), z fallbackiem na DOM */
    function getSelectedProductIds(doc, win) {
        // 1. Główne źródło: IAI.Table.checkedID.products — trzyma ID ze wszystkich stron
        try {
        ...
        // 2. Fallback: DOM checkboxes (tylko aktualna strona)
    ...
    function changeAvailability(win, productId, stockId, sizeId, status) {
        // IAI panel używa POST (mimo że parametry są w URL)
        return ajaxPost(win, `/panel/ajax/product-edit.php?function=ajaxChangeAvailability&productId=${productId}&stockId=${stockId}&sizeId=${sizeId}&status=${status}`, '');
    }
                ...
                if (manualAction === 'all_jest') {
                    ...
                    for (const pid of productIds) {
                        ...
                        try {
                            ...
                            const errMsg = err ? ` (ostatni błąd: ${lastError})` : '';
                            log(`Towar ${pid} → ${ok} OK / ${err} błędów (${selectedWh.length} mag × ${sizes.length} rozm)${errMsg}`, err ? 'err' : 'ok');
                        } catch (e) {
                            log(`Towar ${pid} → BŁĄD: ${e.message}`, 'err');
                        }
                        await sleep(150);
