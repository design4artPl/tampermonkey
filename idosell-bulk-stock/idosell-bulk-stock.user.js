...
(function () {
    ...
    /**
     * Zmienia status JEST/NIEMA dla danego stockId×sizeId.
     * UWAGA: status musi być LICZBĄ, nie literą:
     *   JEST  → -1
     *   NIEMA → 0
     * Wartości 'y'/'n' są cicho ignorowane przez endpoint.
     */
    ...
    /** Pobiera sizeIds z aceform-tables.php?name=quantity (tabela JEST/NIEMA) */
    async function fetchProductSizes(win, productId) {
        const result = { sizes: [], source: 'none' };

        // ŹRÓDŁO 1: aceform-tables.php?name=quantity — z onclick manualStock(stockId, "sizeId", val, pid)
        try {
            ...
            const html = await new Promise((res, rej) => {
                xhr.open('POST', url, true);
                xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? res(xhr.responseText) : rej(new Error('HTTP ' + xhr.status));
                xhr.onerror = () => rej(new Error('Błąd sieci'));
                xhr.send('id=stocksQuantity&url=' + encodeURIComponent(url));
            });

            // Pattern: manualStock(stockId, &quot;sizeId&quot;, val, pid)
            const matches = [...html.matchAll(/manualStock\(\s*(-?\d+)\s*,\s*&quot;([^&]+)&quot;/g)];
            const sizes = new Set();
            for (const m of matches) sizes.add(m[2]);

            if (sizes.size) {
                result.sizes = Array.from(sizes);
                result.source = 'quantity_table_manualStock';
                return result;
            }
        } catch (e) {}

        // ŹRÓDŁO 2: aceform-tables.php?name=sizes (codes_producer)
        try {
    ...
    async function fetchWarehouses(win) {
        const xhr = new win.XMLHttpRequest();
        const html = await new Promise((res, rej) => {
            xhr.open('GET', '/panel/stocks.php', true);
            xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? res(xhr.responseText) : rej(new Error('HTTP ' + xhr.status));
            xhr.onerror = () => rej(new Error('Błąd sieci'));
            xhr.send();
        });
    ...
    async function openModal(doc, win, productIds) {
        ...
        <div class="bs-modal">
            ...
            <div class="bs-bd">
                ...
                <div class="bs-section" id="bs-sec-manual">
                    ...
                    <div id="bs-manual-perwh" class="bs-hidden" style="margin-top:14px">
                        ...
                        <div class="bs-radio-group" id="bs-status">
                            <label class="bs-checked"><input type="radio" name="bs-status" value="-1" checked> JEST (dostępny)</label>
                            <label><input type="radio" name="bs-status" value="0"> NIEMA (niedostępny)</label>
                        </div>
                ...
                <label class="bs-debug-row">
                    <input type="checkbox" id="bs-debug"> Tryb diagnostyczny (loguje sizeIds, response, błędy dla pierwszego towaru)
                </label>
        ...
        ov.querySelector('#bs-apply').addEventListener('click', async () => {
            ...
            const statusNum = ov.querySelector('input[name="bs-status"]:checked')?.value || '-1';
            const statusLabel = statusNum === '-1' ? 'JEST' : 'NIEMA';
            ...
            if (mode !== 'auto' && manualAction !== 'none') {
                if (manualAction === 'all_jest') {
                    log(`══ Wszystkie magazyny → JEST (ajaxMassSetAvailability) ══`);
                    let i = 0;
                    for (const pid of productIds) {
                        i++;
                        fill.style.width = Math.round((i/total)*100) + '%';
                    ...
                    log(`══ Magazyny [${whCodes}] → ${statusLabel} (status=${statusNum}) ══`);
                    let i = 0;
                    for (const pid of productIds) {
                        i++;
                        fill.style.width = Math.round((i/total)*100) + '%';
                        ...
                        try {
                            ...
                            if (debug && i === 1) {
                                log(`  [debug] źródło sizeId: ${sd.source}`, 'dbg');
                            ...
                            let ok = 0, err = 0, lastError = '', noChange = 0;
                            for (const stockId of selectedWh) {
                                for (const sizeId of sd.sizes) {
                                    try {
                                        const r = await changeAvailability(win, pid, stockId, sizeId, statusNum);
                                        if (debug && i === 1 && (ok + err + noChange) === 0) {
                                            log(`  [debug] URL: ?function=ajaxChangeAvailability&productId=${pid}&stockId=${stockId}&sizeId=${sizeId}&status=${statusNum}`, 'dbg');
                                        ...
                                        if (r && r.errno !== undefined && r.errno !== 0) {
                                            ...
                                            // commits puste = stan był już taki, lub endpoint zignorował
                                            noChange++;
                                        }
                                    } catch (e) {
                                        err++;
                            ...
                            const cls = err ? 'err' : (ok ? 'ok' : 'warn');
                            log(`Towar ${pid} (${sd.sizes.length} rozm: ${sd.sizes.slice(0,3).join(',')}${sd.sizes.length>3?'...':''}) → ${ok} OK / ${err} bł.${ncMsg}${errMsg}`, cls);
                        } catch (e) {
                            log(`Towar ${pid} → BŁĄD: ${e.message}`, 'err');
                        }
                        await sleep(150);
