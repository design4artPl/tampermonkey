...
// @version      1.5.0
// @description  Masowe ustawianie trybu gospodarki, stanu JEST/NIEMA, ilości na magazynach. Z uploadem CSV/XML.
// @author       SyncOffer
...
(function () {
    ...
    /** Pobiera sizeIds z aceform-tables.php?name=quantity */
    ...
    /** Parsuje CSV z formatem id_towaru;id_magazynu;wartość (lub stan) */
    function parseCsvFile(text) {
        ...
        for (const line of lines) {
            ...
            const pid = f[0]; const wh = f[1]; const val = f[2];
            if (!/^\d+$/.test(pid)) continue; // nagłówek lub nieprawidłowy
            rows.push({ productId: pid, warehouse: wh, value: val });
        }
        return rows;
    }

    /** Parsuje XML <item id="..." warehouse="..." qty="..."/> lub available */
    ...
    async function openModal(doc, win, productIds) {
        ...
        function bindRadio(name, onChange) {
            $$(`label.ms-radio input[name="${name}"]`).forEach(r => {
                ...
                r.onchange = () => { setRadio(name, r.value); onChange(r.value); };
            });
        }
        ...
        function renderFileZone() {
            ...
            if (clr) clr.onclick = () => { state.file = null; renderFileZone(); refreshSections(); updateApplyBtn(); };
            if (dz && fi) {
        ...
        function renderFileSection() {
            ...
            if (state.mode === 'auto') {
                intro.textContent = 'Plik powinien zawierać identyfikator towaru, identyfikator magazynu oraz ilość — po jednym wpisie na linię.';
                spec.innerHTML = `CSV: <code>id_towaru;id_magazynu;ilość</code><br>XML: <code>&lt;item id="123" warehouse="0" qty="10"/&gt;</code>`;
            } else {
                intro.textContent = 'Plik powinien zawierać identyfikator towaru, identyfikator magazynu oraz stan (1=JEST, 0=NIEMA) — po jednym wpisie na linię.';
                spec.innerHTML = `CSV: <code>id_towaru;id_magazynu;stan</code> <span style="color:#b0b8c9;margin-left:6px">(stan: 1 = JEST, 0 = NIEMA)</span><br>XML: <code>&lt;item id="123" warehouse="0" available="1"/&gt;</code>`;
            }
        ...
        function renderFiniteContent() {
            ...
            if (state.finiteMode === 'value') {
                ...
                const qty = $('#ms-qty');
                qty.oninput = () => { state.quantity = qty.value; updateApplyBtn(); };
                $('#ms-qty-up').onclick = () => { qty.value = String(Math.max(0, Number(qty.value || 0) + 1)); state.quantity = qty.value; updateApplyBtn(); };
                $('#ms-qty-down').onclick = () => { qty.value = String(Math.max(0, Number(qty.value || 0) - 1)); state.quantity = qty.value; updateApplyBtn(); };
            } else {
        ...
        function renderFiniteFileZone() {
            ...
            if (clr) clr.onclick = () => { state.file = null; renderFiniteFileZone(); updateApplyBtn(); };
            if (dz && fi) {
                dz.onclick = (e) => { if (e.target.tagName !== 'INPUT') fi.click(); };
                fi.onchange = () => { if (fi.files.length) { state.file = fi.files[0]; renderFiniteFileZone(); updateApplyBtn(); } };
                dz.ondragover = (e) => { e.preventDefault(); dz.classList.add('ms-drag'); };
                dz.ondragleave = () => dz.classList.remove('ms-drag');
                dz.ondrop = (e) => {
                    e.preventDefault(); dz.classList.remove('ms-drag');
                    if (e.dataTransfer.files.length) { state.file = e.dataTransfer.files[0]; renderFiniteFileZone(); updateApplyBtn(); }
                };
            }
        }
    ...
    async function runFromFile(win, state, log, onProgress) {
        ...
        for (const row of rows) {
            ...
            try {
                ...
                if (state.mode === 'manual') {
                    const statusNum = (row.value === '1' || row.value === 'y' || row.value === 'true') ? '-1' : '0';
                    for (const sizeId of sd.sizes) {
                        try {
                            const r = await changeAvailability(win, pid, stockId, sizeId, statusNum);
                            if (r && r.errno !== undefined && r.errno !== 0) err++;
                    ...
                    // auto + qty — placeholder (needs product-edit-quantity API)
                    log(`Towar ${pid} mag=${stockId} qty=${row.value}: ustawianie ilości w przygotowaniu`, 'warn');
                }
            } catch (e) {
                err++;
                log(`Towar ${pid}: BŁĄD ${e.message}`, 'error');
