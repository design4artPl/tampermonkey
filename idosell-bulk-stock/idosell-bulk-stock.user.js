...
// @version      1.2.1
// @description  Masowe ustawianie trybu gospodarki, stanu JEST/NIEMA i ilości na konkretnych magazynach dla zaznaczonych towarów (używa natywnego IAI.ajax.post)
// @author       SyncOffer
...
(function () {
    ...
    /** Używa natywnego IAI.ajax.post (YAHOO.util.Connect) — identyczny request jak panel */
    function changeAvailability(win, productId, stockId, sizeId, status) {
        return new Promise((resolve, reject) => {
            const url = `/panel/ajax/product-edit.php?function=ajaxChangeAvailability&productId=${productId}&stockId=${stockId}&sizeId=${encodeURIComponent(sizeId)}&status=${status}`;
            try {
                if (win.IAI && win.IAI.ajax && win.IAI.ajax.post) {
                    win.IAI.ajax.post(url, {
                        success: (o) => {
                            try {
                                const data = win.JSON.parse(o.responseText);
                                resolve(data);
                            } catch (e) { resolve({ raw: o.responseText }); }
                        },
                        failure: (o) => reject(new Error('HTTP ' + (o.status || '?') + ': ' + (o.statusText || ''))),
                    });
                } else {
                    // Fallback: zwykły XHR
                    const xhr = new win.XMLHttpRequest();
    ...
    /** Pobiera sizeIds z wielu źródeł — z fallbackiem */
    async function fetchProductSizes(win, productId) {
        const result = { sizes: [], source: 'none', raw: null };

        // ŹRÓDŁO 1: HTML pełnej strony product-edit.php — szukamy quantities[stockId][sizeId] lub stocks_sizes
        try {
            const html = await rawGet(win, `/panel/product-edit.php?idt=${productId}`);

            // Parse stocks_sizes_X JSON
            const ssm = html.match(/name="stocks_sizes_\d+"[^>]*value="([^"]+)"/);
            if (ssm) {
                const decoded = ssm[1].replace(/&quot;/g, '"');
                try {
                    const data = JSON.parse(decoded);
                    if (data.sizes && Array.isArray(data.sizes) && data.sizes.length) {
                        // Filtr: ignoruj kanały (retail/auction/pos) — to nie są rozmiary
                        const sizes = data.sizes.filter(s => !['retail','auction','pos','stock_quantity'].includes(s));
                        if (sizes.length) {
                            result.sizes = sizes;
                            result.source = 'stocks_sizes_json';
                            result.raw = data;
                            return result;
                        }
                    }
            ...
            // Pattern: quantities[stockId][sizeId]
            const qty = [...html.matchAll(/name="quantities\[(-?\d+)\]\[([^\]]+)\]"/g)];
            if (qty.length) {
                const sizes = [...new Set(qty.map(m => m[2]))];
                result.sizes = sizes;
                result.source = 'quantities_input';
                return result;
            }
        } catch (e) {}

        // ŹRÓDŁO 2: aceform-tables.php?name=sizes
        try {
            ...
            const codes = [...new Set([...html.matchAll(/name="codes_(?:producer|extern)\[([^\]]+)\]"/g)].map(m => m[1]))];
            if (codes.length) {
                result.sizes = codes;
                result.source = 'aceform_sizes_table';
                return result;
            }
        } catch (e) {}

        // ŹRÓDŁO 3: ajaxSizesDisplayByProductId — divy id="X_Y_retail" (X tu może być też sizeId w niektórych wersjach)
        try {
            const html = await rawGet(win, `/panel/ajax/product-edit.php?function=ajaxSizesDisplayByProductId&menu=n&productId=${productId}`);
            const ids = [...new Set([...html.matchAll(/id="([^"]+)_-?\d+_(?:retail|auction|pos)"/g)].map(m => m[1]))];
            if (ids.length) {
                result.sizes = ids;
                result.source = 'sizes_display_div';
                return result;
            }
        } catch (e) {}
    ...
    const MODAL_CSS = `
        ...
        .bs-debug-row { display:flex;align-items:center;gap:8px;font-size:12px;color:#666;margin-top:8px;font-weight:normal; }
        .bs-debug-row input { width:auto;margin:0; }
        ...
        .bs-log { margin-top:10px;max-height:240px;overflow-y:auto;font-size:12px;font-family:ui-monospace,monospace;background:#1e1e1e;color:#ddd;border-radius:6px;padding:10px;line-height:1.5; }
        .bs-log .ok { color:#4caf50; }
        .bs-log .err { color:#f44336; }
        .bs-log .warn { color:#ff9800; }
        .bs-log .dbg { color:#82aaff;font-size:11px; }
    `;
    ...
    async function openModal(doc, win, productIds) {
        ...
        <div class="bs-modal">
            ...
            <div class="bs-bd">
                <div class="bs-summary">Zaznaczonych towarów: <b>${productIds.length}</b> (ze wszystkich stron)</div>
                ...
                <label class="bs-debug-row">
                    <input type="checkbox" id="bs-debug"> Tryb diagnostyczny (loguje surowe odpowiedzi serwera dla pierwszego towaru)
                </label>
        ...
        ov.querySelector('#bs-apply').addEventListener('click', async () => {
            ...
            const debug = ov.querySelector('#bs-debug').checked;
                ...
                if (manualAction === 'all_jest') {
                    ...
                    for (const pid of productIds) {
                        ...
                        try {
                            ...
                            if (debug && i === 1) {
                                log(`  [debug] źródło: ${sd.source}`, 'dbg');
                                log(`  [debug] sizeIds: ${JSON.stringify(sd.sizes)}`, 'dbg');
                            }
                            if (!sd.sizes.length) {
                                log(`Towar ${pid} → brak rozmiarów (sprawdzono 3 źródła)`, 'warn');
                                continue;
                            }
                            let ok = 0, err = 0, lastError = '', lastResponse = '';
                            for (const stockId of selectedWh) {
                                for (const sizeId of sd.sizes) {
                                    try {
                                        const r = await changeAvailability(win, pid, stockId, sizeId, status);
                                        if (debug && i === 1 && (ok + err) === 0) {
                                            log(`  [debug] URL: /panel/ajax/product-edit.php?function=ajaxChangeAvailability&productId=${pid}&stockId=${stockId}&sizeId=${sizeId}&status=${status}`, 'dbg');
                                            log(`  [debug] response: ${JSON.stringify(r).substring(0, 300)}`, 'dbg');
                                        }
                                        if (r && r.errno !== undefined && r.errno !== 0) {
                                            err++;
                                            lastError = r.error || ('errno=' + r.errno);
                                        } else {
                                            ok++;
                                        }
                                        lastResponse = JSON.stringify(r).substring(0, 100);
                                    } catch (e) {
                                        err++;
                                        lastError = e.message;
                                    }
                            ...
                            const errMsg = err ? ` [${lastError}]` : '';
                            log(`Towar ${pid} (${sd.source}, ${sd.sizes.length} rozm: ${sd.sizes.slice(0,3).join(',')}${sd.sizes.length>3?'...':''}) → ${ok} OK / ${err} bł.${errMsg}`, err ? 'err' : 'ok');
                        } catch (e) {
                            log(`Towar ${pid} → BŁĄD: ${e.message}`, 'err');
                        }
                        await sleep(150);
