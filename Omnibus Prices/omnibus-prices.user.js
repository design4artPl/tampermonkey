...
(function () {
  ...
  function extractGrossPrices(prodNode) {
    ...
    let retail = null, wholesale = null;

    // Common v7 locations — try several.
    const candidates = [
      ['productRetailPrice', 'gross'],
    ...
    const wholesaleCandidates = [
      ['productWholesalePrice', 'gross'],
      ['productWholesalePrice', 'price'],
      ['priceWholesale', 'gross'],
    ...
    for (const path of candidates) {
      const v = resolve(prodNode, path);
      if (v != null && Number.isFinite(Number(v))) { retail = Number(v); break; }
    }
    for (const path of wholesaleCandidates) {
      const v = resolve(prodNode, path);
      if (v != null && Number.isFinite(Number(v))) { wholesale = Number(v); break; }
    }

    // Fallback: scan for "gross" keys adjacent to retail/wholesale naming.
      ...
      (function walk(node) {
        ...
        for (const [k, v] of Object.entries(node)) {
          ...
          if (v && typeof v === 'object') {
            if (retail == null && keyLc.includes('retail') && keyLc.includes('price')) {
              const g = v.gross ?? v.grossPrice ?? v.price;
              if (g != null && Number.isFinite(Number(g))) retail = Number(g);
            }
            if (wholesale == null && keyLc.includes('wholesale') && keyLc.includes('price')) {
              const g = v.gross ?? v.grossPrice ?? v.price;
              if (g != null && Number.isFinite(Number(g))) wholesale = Number(g);
            }
            walk(v);
          }
        }
  ...
  async function fetchAllProducts(cfg) {
    ...
    while (page < pages) {
      ...
      if (cfg.debug && page === 0) {
        write(`  SAMPLE RESPONSE (strona 0, pierwsze 3000 znaków):`, 'muted');
        const s = typeof res.data === 'string' ? res.data : JSON.stringify(res.data, null, 2);
        write(s.slice(0, 3000), 'muted');
      }
      ...
      for (const item of arr) {
        ...
        all.push({ id: String(id), retail, wholesale });
      }
  ...
  // Product payload with explicit mode override (manual or automatic) for the sync flow.
  function buildProductForceMode(id, struct, fallbackShopId, mode, retail, wholesale, includePricesInShops, includeSizes) {
    const omnibus = { omnibusPriceManagement: mode };
    if (retail != null) omnibus.omnibusPriceRetail = retail;
    if (wholesale != null) omnibus.omnibusPriceWholesale = wholesale;

    const shopOmnibus = includePricesInShops ? { ...omnibus } : { omnibusPriceManagement: mode };
    const shops = (struct && struct.shops && struct.shops.length)
      ? struct.shops.map(s => ({ shopId: Number(s.shopId), omnibusPrices: { ...shopOmnibus } }))
      : [ { shopId: Number(fallbackShopId), omnibusPrices: { ...shopOmnibus } } ];
  ...
  async function runSync() {
    ...
    try {
      ...
      if (cfg.dryRun) {
        write(`\n=== DRY RUN — przykładowe payloady manual + auto dla pierwszego produktu ===`, 'muted');
        const p = needsUpdate[0];
        const s = structures.get(p.id);
        const manualP = buildProductForceMode(p.id, s, cfg.shopId, 'manual', p.retail, p.wholesale, true, true);
        const autoP = buildProductForceMode(p.id, s, cfg.shopId, 'automatic', null, null, false, false);
        write('PUT #1 (manual + ceny): ' + JSON.stringify({ params: { products: [manualP] } }, null, 2), 'muted');
        write('PUT #2 (automatic): ' + JSON.stringify({ params: { products: [autoP] } }, null, 2), 'muted');
        return;
      }

      // 4) PUT #1: mode=manual + new prices (product + shops + sizes).
      const manualProducts = needsUpdate.map(p => buildProductForceMode(
        p.id, structures.get(p.id), cfg.shopId, 'manual', p.retail, p.wholesale, true, true
      ));
      const r1 = await doPut(cfg, manualProducts, 'PUT #1 (manual + ceny brutto)');
      write(`\nPUT #1 zakończony. OK: ${r1.ok}, błędnych: ${r1.fail}.`, r1.fail ? 'warn' : 'ok');

      // 5) PUT #2: mode=automatic (no prices, shop-only level suffices).
      const autoProducts = needsUpdate.map(p => buildProductForceMode(
        p.id, structures.get(p.id), cfg.shopId, 'automatic', null, null, false, false
      ));
      const r2 = await doPut(cfg, autoProducts, 'PUT #2 (powrót na automatic)');
      write(`\nPUT #2 zakończony. OK: ${r2.ok}, błędnych: ${r2.fail}.`, r2.fail ? 'warn' : 'ok');

      // 6) Verify that omnibus now matches gross prices.
      write(`\n=== Weryfikacja: omnibus vs brutto ===`, 'muted');
      const expectations = new Map(needsUpdate.map(p => [p.id, { retail: p.retail, wholesale: p.wholesale, mode: 'no_change' }]));
