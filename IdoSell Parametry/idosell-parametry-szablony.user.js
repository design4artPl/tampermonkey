...
`;
  ...
  function showTranslationsPage(doc) {
    ...
    // 2-FAZOWE ŁADOWANIE — total znany PRZED startem fazy 1, dzięki czemu
    // licznik X/Y i pasek postępu rosną MONOTONICZNIE (bez skoków w dół).
    // Faza 0 (~400 ms, concurrency 20): loadChildValues dla każdego parametru
    //   → zbieramy IDs wszystkich wartości; znamy total = params + values.
    // Faza 1: fetchLangData płasko dla wszystkich (params + values), concurrency 20.
    function loadInTwoPhases() {
      if (ptextEl) ptextEl.textContent = 'Wykrywanie zakresu danych…';
      var allValues = []; // {id, parentId}
      return chunkAll(paramList, 20, function (p) {
        ...
        // Total znany. Switch to determinate counter (monotonicznie).
        totalToLoad = paramList.length + allValues.length;
        doneCount = 0;
        updateProgress();
        // Lista wszystkich tasków (param + value)
        var tasks = [];
        for (var i = 0; i < paramList.length; i++) tasks.push({ kind: 'param', id: paramList[i].id });
        for (var k = 0; k < allValues.length; k++) tasks.push({ kind: 'value', id: allValues[k].id, parentId: allValues[k].parentId });
        return chunkAll(tasks, 20, function (t) {
          return fetchLangData(t.id).then(function (names) {
            loaded.push({ id: t.id, type: t.kind, parentId: t.parentId || null, names: names });
    ...
    updateProgress();
    loadInTwoPhases().then(function () { renderTable(); }, function (e) { renderTable(); if (_panel) _panel.showStatus('Błąd ładowania: ' + (e && e.message || e), true); });
