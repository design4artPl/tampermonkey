  ...
  function showTranslationsModal(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        '.tp-tr-modal { background:#fff; border-radius:12px; width:100%; max-width:calc(100vw - 40px); max-height:calc(100vh - 40px); display:flex; flex-direction:column; box-shadow:0 20px 50px rgba(0,0,0,.3); overflow:hidden; font-family:"DM Sans",system-ui,-apple-system,"Segoe UI",Roboto,sans-serif; }',
        ...
        // Modal body = scrollowalny kontener z kartą .panel-pro w środku.
        '.tp-tr-body { flex:1; overflow:auto; padding:18px 24px; background:#f8fafc; }',
        '.tp-tr-card { margin:0 !important; box-shadow:0 1px 2px rgba(0,0,0,0.04); }',
        '.tp-tr-card-body { background:#fff; }',
        ...
        '.tp-tr-grid { background:#fff; overflow-x:auto; }',
        // Override: .panel-pro ma overflow-x:hidden globalnie — pozwalamy na scroll w karcie Tłumaczeń
        '.tp-tr-card.panel-pro { overflow-x:visible; }',
    ...
    // tm-settings-v2 — aby reguły CSS .tm-input-wrap/.tm-field-select/.tm-node-text-input
    // (scope'owane do .tm-settings-v2 w SHARED_MODAL_CSS) zadziałały na inputy i filtr.
    var modal = d.createElement('div'); modal.className = 'tp-tr-modal tm-settings-v2';
    modal.innerHTML =
    ...
    function renderTable() {
      ...
      // Komórki: .tm-input-wrap + .tm-node-text-input + ikona prefix .tm-input-icon (jak modal edycji).
      function cellHtml(id, lang, v) {
        var ev = esc(v || '');
        return '<div class="tp-tr-cell"><div class="tm-input-wrap"><span class="material-symbols-outlined tm-input-icon">title</span><input class="tm-node-text-input tp-tr-input" data-id="' + esc(id) + '" data-lang="' + esc(lang) + '" value="' + ev + '" data-orig="' + ev + '"></div></div>';
      }
      ...
      // Grid template: pierwsza kolumna min 280px, pozostałe min 320px (input z ikoną + długie nazwy bez ucięcia).
      // Jak nie mieści się w widoku — pojawia się scroll poziomy w karcie (panel-pro overflow-x).
      var gridCols = 'minmax(280px,1.4fr) ' + allLangs.map(function () { return 'minmax(320px,1fr)'; }).join(' ');
      ...
      allLangs.forEach(function (lg) { html += '<div class="panel-pro__col">' + esc(getLangName(lg)) + '</div>'; });
      html += '</div></div>';
      ...
      var summaryTxt = paramItems.length + ' parametrów · ' + valueItems.length + ' wartości · ' + allLangs.length + ' języków';
