...
(function (root) {
  ...
  var CSS = [
    ...
    '.tp-fv-btn-rm, .tp-fv-btn-add { width:24px; height:24px; border:1px solid #cbd5e1; border-radius:50%; background:#fff; cursor:pointer; font-weight:700; line-height:1; flex-shrink:0; font-size:14px; padding:0; }',
...
.tp-col-drag {
  ...
  justify-content: center !important;
  height: 100%;
  /* v4.6.103: kolumna zwężona do 0 (checkbox przy lewej krawędzi).
     v4.6.114: drag-handle pozycjonowany absolute względem wiersza,
     pojawia się tylko na hover — overflow musi być visible. */
...
.tp-drag-handle {
  /* v4.6.114→117: hover-only — uchwyt pojawia się po najechaniu na wiersz
     wartości, pozycjonowany absolute w kolumnie check (lewa krawędź wiersza).
     Checkbox wartości chowany na hover (poniżej w CSS), żeby się nie nakładały. */
...
li.panel-pro__row:hover > .tp-col-drag .tp-drag-handle {
  opacity: 1;
  pointer-events: auto;
}
/* v4.6.118: tylko BEZPOŚREDNI drag-handle (wartość); parametry zawierają wartości
   przez block_group ul więc bez `>` selektor trafiał też w parametry */
li.panel-pro__row:hover:has(> .tp-col-drag > .tp-drag-handle) > .tp-col-check {
  opacity: 0;
  transition: opacity 0.12s;
}
...
`;
  ...
  async function saveValueFilters(nodeId, shop, lang, activeIds, pricestep, pricestepnet) {
    var r = await fetchAjax('action=getNode&node_id=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(lang) + '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0');
    var d = (r && r.data) || {};
  ...
  async function renderValueFiltersEditor(host, doc, nodeId, shop, lang) {
    ...
    function buildHtml() {
      ...
      html += '<div class="tp-fv-mode">';
      html += '<label><input type="radio" name="tpfv_mode_' + nodeId + '" value="default"' + (isDefault ? ' checked' : '') + '> Zestaw domyślny</label>';
      html += '<label><input type="radio" name="tpfv_mode_' + nodeId + '" value="custom"' + (isDefault ? '' : ' checked') + '> Niestandardowy</label>';
      html += '</div>';
    ...
    function renderLists() {
      ...
      if (stateActive.length === 0) {
        ...
        html = '';
        for (i = 0; i < stateActive.length; i++) {
          f = stateActive[i];
          html += '<div class="tp-fv-row" data-idx="' + i + '"><span class="tp-fv-name">' + escapeHtml(f.name) + '</span><button type="button" class="tp-fv-btn-rm" title="Usuń">×</button></div>';
        }
      }
      elA.innerHTML = html;
      if (stateSrc.length === 0) {
        ...
        html = '';
        for (i = 0; i < stateSrc.length; i++) {
          f = stateSrc[i];
          html += '<div class="tp-fv-row" data-idx="' + i + '"><span class="tp-fv-name">' + escapeHtml(f.name) + '</span><button type="button" class="tp-fv-btn-add" title="Dodaj">+</button></div>';
        }
      }
      elS.innerHTML = html;
      ...
      // wire buttons
      var rmBtns = elA.querySelectorAll('.tp-fv-btn-rm');
      for (i = 0; i < rmBtns.length; i++) {
        (function (b) {
          b.addEventListener('click', function () {
            var row = b.parentNode;
            var idx = parseInt(row.getAttribute('data-idx'), 10);
            var moved = stateActive.splice(idx, 1)[0];
            if (moved) stateSrc.push(moved);
            renderLists();
          });
        })(rmBtns[i]);
      }
      var addBtns = elS.querySelectorAll('.tp-fv-btn-add');
      for (i = 0; i < addBtns.length; i++) {
        (function (b) {
          b.addEventListener('click', function () {
            var row = b.parentNode;
            var idx = parseInt(row.getAttribute('data-idx'), 10);
            var moved = stateSrc.splice(idx, 1)[0];
            if (moved) stateActive.push(moved);
            renderLists();
          });
        })(addBtns[i]);
      }
    ...
    // przelacznik trybu
    var radios = host.querySelectorAll('input[name="tpfv_mode_' + nodeId + '"]');
    for (var ri = 0; ri < radios.length; ri++) {
      radios[ri].addEventListener('change', function () {
        var checked = host.querySelector('input[name="tpfv_mode_' + nodeId + '"]:checked');
        editor.style.display = (checked && checked.value === 'custom') ? '' : 'none';
      });
    }
    ...
    return {
      getMode: function () {
        var c = host.querySelector('input[name="tpfv_mode_' + nodeId + '"]:checked');
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function buildBody(dn) {
        ...
        var filterCard = card('filter_alt', 'Filtrowanie',
          '<div class="tm-hint-bar tm-hint-bar--info"><span class="tm-hint-icon"></span><span>Wybierz <b>Zestaw domyślny</b> (panel sklepu) lub <b>Niestandardowy</b> — wtedy zarządzaj listą aktywnych filtrów i krokiem przedziału cen.</span></div>' +
      ...
      function doSave() {
        ...
        fetchAjax(nameBody).then(function (r1) { if (r1 && r1.errno && r1.errno !== 0) settingsErr = (r1.message || ('errno ' + r1.errno)); }, function (e) { settingsErr = (e && e.message) || String(e); }).then(function () {
          ...
          return saveValueFilters(nodeId, curShop, curLang, ids, ps, ps);
        }).then(function () {
  ...
  function mountPanelPro(doc, attempt) {
    ...
    var panel = PanelPro.mount(doc, {
      ...
      columns: [
        // v4.6.83: drag i check tej samej szeroko\u015bci (32px) \u2014 drag-handle warto\u015bci
        // i checkbox parametru w tej samej pozycji X (\u015brodek slotu = 16px od jego
        // kraw\u0119dzi). Wcze\u015bniej drag=24px, check=40px powodowa\u0142y rozjazd.
        { id: 'drag',     label: '',         width: '0px' },
        { id: 'check',    label: '',         width: '32px' },
        { id: 'expand',   label: '',         width: '32px' },
        { id: 'name',     label: 'Nazwa',    hint: '(Dwuklik na nazw\u0119 = edycja)', width: '1fr' },
