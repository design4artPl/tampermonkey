...
`;
  ...
  async function fetchFullTree(doc) {
    var root = doc.querySelector('#block_group0');
    if (!root) return [];
    ...
    for (var i = 0; i < roots.length; i += BATCH) {
      ...
      batchData.forEach(function (x) { if (x) result.push(x); });
    }
    return result;
  }
  ...
  async function exportTreeFull(doc, format) {
    if (_panel && _panel.showStatus) _panel.showStatus('Pobieranie warto\u015bci wszystkich parametr\u00f3w...');
    var tree;
    try {
      tree = await fetchFullTree(doc);
    } catch (e) {
