...
`;
  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      // ====== v4.6.31: Nawigacja w sklepie (tylko WARTOŚCI) ======
      // Zweryfikowane na demo37: /panel/ajax/parameters.php tree=parameters parent=0
      //   getNode / getNodeDesc / setNodeDesc / editNodeShopData (round-trip OK).
      // Faza 1a: edytowalny „Opis na liście towarów" (getNodeDesc/setNodeDesc) +
      //   odczyt nazwy nagłówka/meta/robots/link. Reszta edytowalna po
      //   przechwyceniu pełnego natywnego payloadu editNodeShopData.
      if (isVal) {
        var cardN = d.createElement('div'); cardN.className = 'tp-se-card';
        cardN.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">travel_explore</span>Nawigacja w sklepie</div>';
        ...
        nHint.innerHTML = '<span>ℹ</span><span>Ustawienia podstrony tej wartości w nawigacji sklepu (osobno per sklep i język). Dostępne tylko dla sklepów, w których ta wartość ma kupowalne towary. „Opis na liście towarów" to inny opis niż w karcie „Treść".</span>';
        ...
        function navPost(body2) {
          return fetchAjaxRaw('/panel/ajax/parameters.php', body2);
        }
        ...
        // Kandydaci sklepów: unia kluczy per-sklep ze wszystkich langData; fallback 1..7
        var navShopSet = {};
        langs.forEach(function (lg) {
          var L = dt.langData[lg] || {};
        ...
        var navCandidates = Object.keys(navShopSet).sort(function (a, b) { return (+a) - (+b); });
        if (!navCandidates.length) navCandidates = ['1', '2', '3', '4', '5', '6', '7'];
        var navEligible = [];      // [shop]
        var navData = {};          // shop -> getNode data (dla curLang)
        var navReady = false;
        var navEd = null;          // edytor opisu listy (createEditorWidget)
        var navCurShop = null;

        function navEsc(s) { return (typeof escapeHtml === 'function') ? escapeHtml(s == null ? '' : String(s)) : String(s == null ? '' : s); }

        function navRadioRow(label, name, opts, cur) {
          var row = d.createElement('div'); row.className = 'tp-se-row';
          var l = d.createElement('span'); l.className = 'tp-se-lbl'; l.textContent = label;
          ...
          opts.forEach(function (o) {
            ...
            var rb = d.createElement('input'); rb.type = 'radio'; rb.name = name + '_' + Math.random().toString(36).slice(2, 7); rb.value = o.v;
            if (o.v === cur) rb.checked = true;
            inputs[o.v] = rb;
            lab.appendChild(rb); lab.appendChild(d.createTextNode(o.t));
            c.appendChild(lab);
          });
          row.appendChild(l); row.appendChild(c);
          row.__get = function () { for (var k in inputs) { if (inputs[k].checked) return k; } return cur; };
          row.__inputs = inputs;
          return row;
        }
        function navTextRow(label, val, ph) {
          var row = d.createElement('div'); row.className = 'tp-se-row';
          var l = d.createElement('span'); l.className = 'tp-se-lbl'; l.textContent = label;
          var c = d.createElement('div'); c.className = 'tp-se-ctl';
          var inp = d.createElement('input'); inp.type = 'text'; inp.value = val || ''; if (ph) inp.placeholder = ph;
          c.appendChild(inp); row.appendChild(l); row.appendChild(c);
          row.__input = inp;
          return row;
        }
        function renderNavBody(shop) {
          ...
          navEd = null;
          navCurShop = shop;
          var dnode = navData[shop];
          if (!dnode) {
            navHost.innerHTML = '<div style="font-size:12.5px;color:#98a2b3;padding:6px 0;">Brak danych nawigacji dla tego sklepu.</div>';
            return;
          }
          // zachowywane (nieedytowalne tu) — by nie nadpisać natywnych ustawień
          var keepFilterDefault = dnode.filter_default ? 'y' : 'n';
          var keepView = (dnode.display_limit || dnode.sort ||
            (dnode.display_mode && dnode.display_mode_default && dnode.display_mode !== dnode.display_mode_default)) ? 'own' : 'default';

          // Link do strony (read-only)
          var seo = (dnode.seolink && dnode.seolink[0]) || dnode.link || '';
          var rowLink = d.createElement('div'); rowLink.className = 'tp-se-row';
          rowLink.innerHTML = '<span class="tp-se-lbl">Link do strony</span><div class="tp-se-ctl">' +
            (seo ? '<a href="' + navEsc(seo) + '" target="_blank" rel="noopener" style="color:#4f8cff;word-break:break-all;">' + navEsc(seo) + '</a>' : '<span style="color:#98a2b3;">brak</span>') + '</div>';
          navHost.appendChild(rowLink);

          // Nazwa w nagłówku (auto / własna) — EDYTOWALNE
          var hlAuto = !dnode.headline_name;
          var rowHlMode = navRadioRow('Nazwa w nagłówku', 'hlmode', [
            { v: 'd', t: 'wygeneruj automatycznie' + (dnode.title_name || dnode.name ? ' („' + (dnode.title_name || dnode.name) + '")' : '') },
            { v: 'o', t: 'własna nazwa' }
          ], hlAuto ? 'd' : 'o');
          navHost.appendChild(rowHlMode);
          var rowHlText = navTextRow('', dnode.headline_name || '', 'własna nazwa w nagłówku strony');
          rowHlText.style.display = hlAuto ? 'none' : '';
          navHost.appendChild(rowHlText);
          ['d', 'o'].forEach(function (k) {
            rowHlMode.__inputs[k].addEventListener('change', function () {
              rowHlText.style.display = (rowHlMode.__get() === 'o') ? '' : 'none';
            });
          });

          // Meta (auto / podane) — EDYTOWALNE
          var rowMetaMode = navRadioRow('Ustawienia meta', 'metamode', [
            { v: 'y', t: 'wygeneruj automatycznie' }, { v: 'n', t: 'użyj podanych poniżej' }
          ], (dnode.meta_title || dnode.meta_description || dnode.meta_keywords) ? 'n' : 'y');
          navHost.appendChild(rowMetaMode);
          var metaBox = d.createElement('div');
          var rowMT = navTextRow('Meta title', dnode.meta_title || '');
          var rowMD = navTextRow('Meta description', dnode.meta_description || '');
          var rowMK = navTextRow('Meta keywords', dnode.meta_keywords || '');
          metaBox.appendChild(rowMT); metaBox.appendChild(rowMD); metaBox.appendChild(rowMK);
          metaBox.style.display = (rowMetaMode.__get() === 'n') ? '' : 'none';
          navHost.appendChild(metaBox);
          ['y', 'n'].forEach(function (k) {
            rowMetaMode.__inputs[k].addEventListener('change', function () {
              metaBox.style.display = (rowMetaMode.__get() === 'n') ? '' : 'none';
            });
          });
          var rowRi = navRadioRow('Robots index', 'ri', [
            { v: 'default', t: 'automatycznie' }, { v: 'index', t: 'index' }, { v: 'noindex', t: 'noindex' }
          ], dnode.meta_robots_index || 'default');
          navHost.appendChild(rowRi);
          var rowRf = navRadioRow('Robots follow', 'rf', [
            { v: 'default', t: 'automatycznie' }, { v: 'follow', t: 'follow' }, { v: 'nofollow', t: 'nofollow' }
          ], dnode.meta_robots_follow || 'default');
          navHost.appendChild(rowRf);

          // Ustawienia wyświetlania / filtrów — stan zachowywany (info; edytor w kolejnej fazie)
          var rowKeep = d.createElement('div'); rowKeep.className = 'tp-se-row';
          rowKeep.innerHTML = '<span class="tp-se-lbl">Wyświetlanie / filtry</span><div class="tp-se-ctl" style="font-size:12px;color:#98a2b3;">' +
            'widok: ' + navEsc(keepView === 'own' ? 'własne' : 'domyślne') + ', filtry: ' + navEsc(keepFilterDefault === 'n' ? 'własne' : 'domyślne') +
            ' <span style="color:#cbd2dd;">(zachowywane bez zmian — edytor w kolejnej wersji)</span></div>';
          navHost.appendChild(rowKeep);

          // Opis na liście towarów (EDYTOWALNY — zweryfikowane setNodeDesc)
          ...
          var navBtnRow = d.createElement('div'); navBtnRow.style.cssText = 'display:flex;justify-content:flex-end;gap:8px;margin-top:10px;';
          ...
          navSave.style.cssText = 'font-size:12px;padding:6px 14px;'; navSave.textContent = 'Zapisz nawigację (sklep ' + shop + ')';
          navBtnRow.appendChild(navState); navBtnRow.appendChild(navSave);
          navHost.appendChild(navBtnRow);

          navState.textContent = 'Wczytywanie opisu…';
          navPost('action=getNodeDesc&node=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(curLang) +
            '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0').then(function (gd) {
            var desc = (gd && gd.desc) || '';
            try { navEd.setValue(desc); } catch (e) {}
            navState.textContent = '';
          }).catch(function () { navState.textContent = 'błąd wczytywania opisu'; });

          navSave.addEventListener('click', function () {
            var hlMode = rowHlMode.__get();
            var hl = (hlMode === 'o') ? (rowHlText.__input.value || '') : '';
            var md = rowMetaMode.__get();
            var mt = rowMT.__input.value || '', mdsc = rowMD.__input.value || '', mk = rowMK.__input.value || '';
            var mi = rowRi.__get(), mf = rowRf.__get();
            var descHtml = ''; try { descHtml = navEd.getValue(); } catch (e) {}
            navSave.disabled = true; navState.textContent = 'Zapisywanie…';
            // 1) editNodeShopData — DOKŁADNY natywny payload (zachowuje view/filter_default)
            var b = 'action=editNodeShopData&node_type=false&filter_default=' + keepFilterDefault +
              ...
              '&meta_index=' + mi + '&meta_follow=' + mf + '&headline_name=' + encodeURIComponent(hl);
            navPost(b).then(function (r1) {
              ...
              // odśwież cache węzła
              return navGetNode(shop, curLang).then(function (g) { if (g && g.data) navData[shop] = g.data; }, function () {});
            }).then(function () {
              navSave.disabled = false; navState.textContent = 'Zapisano ✓';
              if (_panel) _panel.showStatus('Zapisano nawigację (sklep ' + shop + ', ' + getLangName(curLang) + ')');
            }).catch(function (e) {
              navSave.disabled = false; navState.textContent = 'błąd zapisu';
              alert('Błąd zapisu nawigacji: ' + (e.message || e));
            });
          });
        }

        function renderNavShopSelect() {
          if (!navReady) { navShopSel.innerHTML = '<option>Wczytywanie…</option>'; navShopSel.disabled = true; return; }
          if (!navEligible.length) {
            navShopSel.innerHTML = '<option>Brak — ta wartość nie ma podstron nawigacji</option>';
            navShopSel.disabled = true;
            navHost.innerHTML = '<div style="font-size:12.5px;color:#98a2b3;padding:6px 0;">Ta wartość nie jest w nawigacji żadnego sklepu (brak kupowalnych towarów z tą wartością).</div>';
            return;
          }
          navShopSel.disabled = false;
          navShopSel.innerHTML = navEligible.map(function (s) {
            var dm = navData[s] || {};
            return '<option value="' + s + '">Sklep ' + s + (dm.title_name ? '' : '') + '</option>';
          }).join('');
          navShopSel.value = navEligible[0];
          renderNavBody(navEligible[0]);
        }
        navShopSel.addEventListener('change', function () { renderNavBody(navShopSel.value); });

        function loadNavEligibility() {
          navReady = false; navEligible = []; navData = {};
          renderNavShopSelect();
          var pending = navCandidates.length;
          if (!pending) { navReady = true; renderNavShopSelect(); return; }
          navCandidates.forEach(function (s) {
            navGetNode(s, curLang).then(function (r) {
              var dd = r && r.data;
              var ok = dd && ((dd.seolink && dd.seolink.length && dd.seolink[0]) || (dd.link && /traits=/.test(dd.link)));
              if (ok) { navEligible.push(s); navData[s] = dd; }
            }).catch(function () {}).then(function () {
              pending--;
              if (pending <= 0) {
                navReady = true;
                navEligible.sort(function (a, b) { return (+a) - (+b); });
                renderNavShopSelect();
              }
            });
          });
        }
        // przeładuj przy zmianie języka (nawigacja jest per shop+lang)
        var _navPrevLoadLang = null;
        function navOnLangMaybeChanged() {
          if (_navPrevLoadLang === curLang) return;
          _navPrevLoadLang = curLang;
          loadNavEligibility();
        }
        navOnLangMaybeChanged();
