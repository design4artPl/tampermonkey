...
`;
  ...
  function showExportModal(targetDoc, defaultScope, kind) {
    ...
    var stateGeneral = isSec ? [
      ...
      { key: 'display_settings', label: 'Wyświetlanie (tryb, sortowanie, limit)', on: false },
      { key: 'filters', label: 'Filtry (aktywne, pricestep, per-filter)', on: false },
      { key: 'icon_type', label: 'Typ grafiki (img / img_rwd)', on: false }
    ];
    var stateSeo = isSec ? [] : [
    ...
    var stateExtras = isSec ? [
      ...
      { key: 'icon_files', label: 'Pliki grafik (URL ikon search/projector)', desc: 'URLe ikon per shop+lang; tryb base64 w kolejnej iteracji', on: false }
    ];
    var savedApiKey = '';
    ...
    shadow.querySelector('[data-role="ok"]').addEventListener('click', function () {
      ...
      // Backward compat: shopSettings on, jezeli ktorekolwiek z per-shop polach
      extras.shopSettings = !!(extras.headline_name || extras.node_desc || extras.display_settings || extras.filters || extras.icon_type || extras.meta_title || extras.meta_description || extras.meta_keywords || extras.meta_robots_index || extras.meta_robots_follow);
