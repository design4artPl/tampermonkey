  ...
  function showTranslationsPage(doc) {
    ...
    function renderTable() {
      ...
      function cellHtml(id, lang, v) {
        var ev = esc(v || '');
        return '<div class="tp-tr-cell" data-id="' + esc(id) + '" data-lang="' + esc(lang) + '">' +
          '<span class="tp-tr-cell-check" title="Zaznacz komórkę do skopiowania"></span>' +
          '<div class="tm-input-wrap"><span class="material-symbols-outlined tm-input-icon">title</span><input class="tm-node-text-input tp-tr-input" data-id="' + esc(id) + '" data-lang="' + esc(lang) + '" value="' + ev + '" data-orig="' + ev + '"></div>' +
        '</div>';
      }
    ...
    function applyCopyTarget() {
      ...
      Object.keys(selectedCells).forEach(function (key) {
        ...
        var srcInput = sel.cellEl && sel.cellEl.querySelector('input.tp-tr-input');
        if (!srcInput) return;
        var value = srcInput.value;
        checkedLangs.forEach(function (tgt) {
          var tgtInput = bodyEl.querySelector('input.tp-tr-input[data-id="' + sel.id + '"][data-lang="' + tgt + '"]');
          if (!tgtInput) return;
          if (tgtInput.value !== value) {
            tgtInput.value = value;
            // Dispatchujemy input event żeby dirty state się zaktualizował
