  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      ok.addEventListener('click', function () {
        stash();
        // DIAGNOSTYKA v4.6.30: co realnie przechwyci\u0142 edytor (do raportu u\u017cytkownika)
        var _dCur = (st[curLang] && st[curLang].description) || '';
        var _dSample = _dCur.replace(/\s+/g, ' ').slice(0, 60);
        try {
          console.log('[parametry][opis] v4.6.30 nodeId=' + nodeId + ' curLang=' + curLang +
        ...
        var _diag = { ss: '?', sd: {}, sdErr: {} };
        try { console.log('[parametry][opis] v4.6.30 nameBody=', nameBody.slice(0, 300)); } catch (e) {}
        fetchAjax(nameBody).then(function (r1) {
          try { console.log('[parametry][opis] v4.6.30 setSettings resp=', r1); } catch (e) {}
          _diag.ss = (r1 && typeof r1.errno !== 'undefined') ? ('errno ' + r1.errno) : 'OK';
          if (r1 && r1.errno && r1.errno !== 0) settingsErr = (r1.message || ('errno ' + r1.errno));
        }, function (e) { settingsErr = (e && e.message) || String(e); _diag.ss = 'REJECT ' + settingsErr; try { console.warn('[parametry][opis] v4.6.30 setSettings REJECT', e); } catch (_) {} }).then(function () {
          // v4.6.28: RĘCZNA pętla indeksowa (BEZ Array.prototype.reduce/slice/forEach)
          // — panel ładuje Prototype.js, który psuł iterację (descLangs.reduce
          // nie wykonywał ciała → setDescription „NIE WYSŁANO").
          function sendDesc(i) {
            ...
            try { console.log('[parametry][opis] v4.6.30 ->setDescription', lg, 'len=' + (((st[lg] && st[lg].description) || '').length), 'body=' + dBody.slice(0, 200)); } catch (e) {}
            return fetchAjax(dBody).then(function (rd) {
              try { console.log('[parametry][opis] v4.6.30 <-setDescription', lg, 'resp=', rd); } catch (e) {}
              _diag.sd[lg] = rd && (typeof rd.errno !== 'undefined' ? ('errno ' + rd.errno) : 'OK');
              if (rd && rd.errno && rd.errno !== 0) { _diag.sdErr[lg] = (rd.error || rd.message || ('errno ' + rd.errno)); }
            }, function (e) {
              _diag.sd[lg] = 'REJECT'; _diag.sdErr[lg] = String(e && e.message || e);
              try { console.warn('[parametry][opis] v4.6.30 setDescription REJECT', lg, e); } catch (_) {}
            }).then(function () { return sendDesc(i + 1); });
          }
          return sendDesc(0);
        }).then(function () {
          // v4.6.28: NIEZALEŻNA weryfikacja — ponownie odczytaj z serwera
          return fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (chk) {
            var srv = '';
            try { srv = (chk && chk.data && chk.data.langData && chk.data.langData[curLang] && chk.data.langData[curLang].description) || ''; } catch (e) {}
            var sent = _dCur || '';
            var msg = 'OPIS DIAG v4.6.30\n' +
              'nodeId=' + nodeId + '  język=' + curLang + '\n' +
              'wysłano: ' + sent.length + ' zn  „' + sent.replace(/\s+/g, ' ').slice(0, 50) + '”\n' +
              'setSettings: ' + _diag.ss + '\n' +
              'setDescription[' + curLang + ']: ' + (_diag.sd[curLang] || 'NIE WYSŁANO') + (_diag.sdErr[curLang] ? (' — ' + _diag.sdErr[curLang]) : '') + '\n' +
              'SERWER po ponownym odczycie: ' + (srv || '').length + ' zn  „' + (srv || '').replace(/\s+/g, ' ').slice(0, 50) + '”\n' +
              'ZGODNE: ' + ((srv || '') === sent ? 'TAK ✓' : 'NIE ✗');
            try { console.log('[parametry][opis] v4.6.30 WERYFIKACJA', { sent: sent, server: srv, diag: _diag }); } catch (e) {}
          ...
          var _opisInfo = ' [v4.6.30 opis ' + curLang + ': ' + _dCur.length + ' zn.]';
