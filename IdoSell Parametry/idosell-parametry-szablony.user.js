...
`;
  ...
  // v4.6.134: dialog "Powiel nazwę/opis na języki" — wybor jezyka zrodlowego,
  // jezykow docelowych (checkboxy) i czego (name/description). Zwraca przez callback.
  function openCopyTextsDialog(doc, langs, curLang, st, getLangName, onApply) {
    var d = doc || document;
    var overlay = d.createElement('div');
    overlay.className = 'tm-settings-v2';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(15,20,35,.45);backdrop-filter:blur(4px);z-index:9999999;display:flex;align-items:center;justify-content:center;font-family:\'Segoe UI\',Roboto,Arial,sans-serif;padding:16px';
    var modal = d.createElement('div');
    modal.style.cssText = 'background:#fff;border:1px solid #e4e7ec;border-radius:14px;width:100%;max-width:560px;box-shadow:0 20px 60px rgba(0,0,0,0.25);overflow:hidden;max-height:90vh;display:flex;flex-direction:column';

    // Header
    function esc(s) { return (s == null ? '' : String(s)).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;'); }
    var header = d.createElement('div');
    header.style.cssText = 'display:flex;align-items:center;gap:12px;padding:16px 20px;border-bottom:1px solid #e4e7ec';
    header.innerHTML =
      '<div style="width:36px;height:36px;background:#eff4ff;border:1px solid #bfcffd;border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#2563eb"><span class="material-symbols-outlined">content_copy</span></div>' +
      '<div style="flex:1"><h2 style="font-size:15px;font-weight:600;color:#1a202c;margin:0">Powiel teksty między językami</h2><p style="font-size:12px;color:#9aa3b0;margin:2px 0 0">Wybierz źródło, cel i co skopiować</p></div>' +
      '<button type="button" class="tpcd-close" style="width:30px;height:30px;background:none;border:1px solid transparent;border-radius:7px;cursor:pointer;color:#9aa3b0;display:flex;align-items:center;justify-content:center"><span class="material-symbols-outlined">close</span></button>';
    modal.appendChild(header);

    // Body
    var body = d.createElement('div');
    body.style.cssText = 'flex:1;min-height:0;overflow-y:auto;padding:16px 20px;display:flex;flex-direction:column;gap:16px';

    // 1. Co kopiować
    var whatBlock = d.createElement('div');
    whatBlock.innerHTML =
      '<div style="font-size:10.5px;font-weight:600;text-transform:uppercase;letter-spacing:.7px;color:#9aa3b0;margin-bottom:8px">Co skopiować</div>' +
      '<div style="display:flex;gap:16px">' +
        '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px"><input type="checkbox" class="tpcd-what-name" checked> Nazwa</label>' +
        '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px"><input type="checkbox" class="tpcd-what-desc" checked> Opis</label>' +
      '</div>';
    body.appendChild(whatBlock);

    // 2. Język źródłowy
    var srcBlock = d.createElement('div');
    var srcOpts = langs.map(function (lg) {
      return '<option value="' + esc(lg) + '"' + (lg === curLang ? ' selected' : '') + '>' + esc(getLangName(lg)) + ' (' + esc(lg) + ')</option>';
    }).join('');
    srcBlock.innerHTML =
      '<div style="font-size:10.5px;font-weight:600;text-transform:uppercase;letter-spacing:.7px;color:#9aa3b0;margin-bottom:8px">Język źródłowy</div>' +
      '<select class="tpcd-src" style="width:100%;padding:8px 12px;border:1.5px solid #e4e7ec;border-radius:8px;font-size:13px;background:#fff;outline:none;box-sizing:border-box">' + srcOpts + '</select>';
    body.appendChild(srcBlock);

    // 3. Języki docelowe
    var tgtBlock = d.createElement('div');
    var tgtRows = langs.map(function (lg) {
      var isCur = lg === curLang;
      return '<label class="tpcd-tgt-row" style="display:flex;align-items:center;gap:8px;padding:6px 0;cursor:pointer;font-size:13px;' + (isCur ? 'opacity:.45' : '') + '">' +
        '<input type="checkbox" class="tpcd-tgt" value="' + esc(lg) + '"' + (isCur ? ' disabled' : ' checked') + '>' +
        '<span>' + esc(getLangName(lg)) + ' <span style="color:#9aa3b0">(' + esc(lg) + ')</span></span>' +
        (isCur ? '<span style="font-size:11px;color:#9aa3b0;margin-left:auto;font-style:italic">źródłowy</span>' : '') +
      '</label>';
    }).join('');
    tgtBlock.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">' +
        '<div style="font-size:10.5px;font-weight:600;text-transform:uppercase;letter-spacing:.7px;color:#9aa3b0">Języki docelowe</div>' +
        '<div style="display:flex;gap:8px">' +
          '<button type="button" class="tpcd-all" style="font-size:11px;padding:4px 10px;border:1px solid #e4e7ec;border-radius:6px;background:#fafbfc;color:#4a5568;cursor:pointer">Wszystkie</button>' +
          '<button type="button" class="tpcd-none" style="font-size:11px;padding:4px 10px;border:1px solid #e4e7ec;border-radius:6px;background:#fafbfc;color:#4a5568;cursor:pointer">Żadne</button>' +
        '</div>' +
      '</div>' +
      '<div class="tpcd-tgts" style="border:1px solid #e4e7ec;border-radius:8px;padding:6px 12px;max-height:220px;overflow-y:auto">' + tgtRows + '</div>';
    body.appendChild(tgtBlock);

    modal.appendChild(body);

    // Footer
    var footer = d.createElement('div');
    footer.style.cssText = 'display:flex;gap:8px;justify-content:flex-end;padding:14px 20px;border-top:1px solid #e4e7ec;background:#fafbfc';
    footer.innerHTML =
      '<button type="button" class="tpcd-cancel" style="font-size:13px;padding:8px 20px;border:1.5px solid #e4e7ec;border-radius:8px;background:transparent;color:#4a5568;cursor:pointer">Anuluj</button>' +
      '<button type="button" class="tpcd-apply" style="font-size:13px;padding:8px 20px;border:none;border-radius:8px;background:#2563eb;color:#fff;cursor:pointer;font-weight:500">Powiel</button>';
    modal.appendChild(footer);
    ...
    // Bulk select buttons
    function tgtCbs() { return modal.querySelectorAll('.tpcd-tgt:not(:disabled)'); }
    modal.querySelector('.tpcd-all').addEventListener('click', function () { tgtCbs().forEach(function (cb) { cb.checked = true; }); });
    modal.querySelector('.tpcd-none').addEventListener('click', function () { tgtCbs().forEach(function (cb) { cb.checked = false; }); });

    // Source change → disable current source row + re-enable previous
    var srcSel = modal.querySelector('.tpcd-src');
    function updateDisabledSource() {
      var src = srcSel.value;
      modal.querySelectorAll('.tpcd-tgt-row').forEach(function (row) {
        var cb = row.querySelector('.tpcd-tgt');
        var lg = cb.value;
        var isSrc = lg === src;
        cb.disabled = isSrc;
        if (isSrc) cb.checked = false;
        row.style.opacity = isSrc ? '.45' : '';
        var srcLabel = row.querySelector('span[style*="margin-left:auto"]');
        if (isSrc && !srcLabel) {
          var sp = d.createElement('span');
          sp.style.cssText = 'font-size:11px;color:#9aa3b0;margin-left:auto;font-style:italic';
          sp.textContent = 'źródłowy';
          row.appendChild(sp);
        } else if (!isSrc && srcLabel) {
          srcLabel.remove();
        }
      });
    }
    srcSel.addEventListener('change', updateDisabledSource);

    // Apply
    modal.querySelector('.tpcd-apply').addEventListener('click', function () {
      var src = srcSel.value;
      var tgts = Array.prototype.slice.call(modal.querySelectorAll('.tpcd-tgt:checked')).map(function (cb) { return cb.value; });
      var copyName = modal.querySelector('.tpcd-what-name').checked;
      var copyDesc = modal.querySelector('.tpcd-what-desc').checked;
      if (!copyName && !copyDesc) { alert('Zaznacz co skopiować (nazwa lub opis).'); return; }
      if (tgts.length === 0) { alert('Wybierz przynajmniej jeden język docelowy.'); return; }
      onApply(src, tgts, copyName, copyDesc);
      close();
    });
  }
  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      footer.querySelector('.tp-copylang').addEventListener('click', function () {
        stashName();
        openCopyTextsDialog(d, langs, curLang, st, getLangName, function (src, tgts, copyName, copyDesc) {
          for (var ti = 0; ti < tgts.length; ti++) {
            var lg = tgts[ti];
            if (copyName) st[lg].name = st[src].name;
            if (copyDesc) st[lg].description = st[src].description;
          }
          if (_panel) {
            var parts = [];
            if (copyName) parts.push('nazwę');
            if (copyDesc) parts.push('opis');
            _panel.showStatus('Powielono ' + parts.join(' i ') + ' z „' + getLangName(src) + '" na ' + tgts.length + ' jęz. (zapisz, aby zatwierdzić)');
          }
          var ni = bodyEl.querySelector('[name=f_name]'); if (ni) ni.value = st[curLang].name;
          // jesli skopiowano opis do biezacego jezyka — odswiez edytor TinyMCE
          if (copyDesc && tgts.indexOf(curLang) >= 0) {
            try {
