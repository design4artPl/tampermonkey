...
`;
  ...
  function openCopyTextsDialog(doc, langs, curLang, st, getLangName, onApply) {
    ...
    function render() {
      var canSubmit = (copyName || copyDesc) && targetLangs.length > 0;
        ...
        '<div style="padding:0 22px 20px">' +
          ...
          '<div style="padding:14px 0;border-top:1px solid #f0f2f5">' +
            ...
            '<div style="display:flex;gap:20px">' +
              '<label class="tpcd-what" data-field="name" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:500;color:#1a1a2e">' +
                checkboxBtn(copyName, false) + 'Nazwa' +
              '</label>' +
              '<label class="tpcd-what" data-field="desc" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:500;color:#1a1a2e">' +
                checkboxBtn(copyDesc, false) + 'Opis' +
              '</label>' +
            '</div>' +
          '</div>' +
          // Język źródłowy
    ...
    function bindEvents() {
      ...
      modal.querySelector('.tpcd-apply').addEventListener('click', function () {
        var canSubmit = (copyName || copyDesc) && targetLangs.length > 0;
        if (!canSubmit) return;
        onApply(sourceLang, targetLangs.slice(), copyName, copyDesc);
        close();
      });
    }
  ...
  // v4.6.140: dialog "Powiel ustawienia między sklepami" — analogicznie do openCopyTextsDialog
  // (DM Sans, custom checkbox, gradient submit). Kopiuje: wyswietlanie (display_mode),
  // filtry (filter_*), typ grafiki (icon_*_type).
  function openCopyShopsDialog(doc, shops, curShop, shopLabel, onApply) {
    ...
    var copyDisplay = true, copyFilters = true, copyGfxType = true;
    ...
    function checkboxBtn(checked, disabled) {
      ...
      return '<button type="button" class="tpcs-cb"' + (disabled ? ' disabled' : '') + ' style="width:20px;height:20px;border-radius:5px;flex-shrink:0;border:' + bd + ';background:' + bg + ';cursor:' + cur + ';display:flex;align-items:center;justify-content:center;transition:all .15s;opacity:' + op + ';padding:0">' + inner + '</button>';
    }

    function render() {
      var canSubmit = (copyDisplay || copyFilters || copyGfxType) && targetShops.length > 0;
      ...
      modal.innerHTML =
        ...
        '<div style="padding:20px 22px 16px;display:flex;align-items:flex-start;justify-content:space-between">' +
          '<div style="display:flex;align-items:center;gap:12px">' +
            ...
            '<div>' +
              ...
              '<div style="font-size:12.5px;color:#98a2b3;margin-top:2px">Wyświetlanie, filtry i typ grafik</div>' +
            '</div>' +
          '</div>' +
        ...
        '<div style="padding:0 22px 20px">' +
          ...
          '<div style="padding:14px 0;border-top:1px solid #f0f2f5">' +
            ...
            '<div style="display:flex;flex-direction:column;gap:8px">' +
              '<label class="tpcs-what" data-field="display" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:500;color:#1a1a2e">' +
                checkboxBtn(copyDisplay, false) + 'Wyświetlanie (tryb listy, sortowanie, limit)' +
              '</label>' +
              '<label class="tpcs-what" data-field="filters" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:500;color:#1a1a2e">' +
                checkboxBtn(copyFilters, false) + 'Filtry (aktywne + pricestep + per-filter)' +
              '</label>' +
              '<label class="tpcs-what" data-field="gfxtype" style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:500;color:#1a1a2e">' +
                checkboxBtn(copyGfxType, false) + 'Typ grafiki (img / img_rwd)' +
              '</label>' +
            '</div>' +
          '</div>' +
          // Sklep zrodlowy
        ...
        '<div style="padding:14px 22px;border-top:1px solid #eef0f4;display:flex;align-items:center;justify-content:flex-end;gap:8px;background:#fafbfc">' +
          ...
          '<button type="button" class="tpcs-apply"' + (canSubmit ? '' : ' disabled') + ' style="padding:9px 22px;border-radius:8px;border:none;background:' + (canSubmit ? 'linear-gradient(135deg,#4f8cff,#3b6de0)' : '#e0e3ea') + ';color:' + (canSubmit ? '#fff' : '#98a2b3') + ';font-size:13px;font-weight:600;cursor:' + (canSubmit ? 'pointer' : 'not-allowed') + ';box-shadow:' + (canSubmit ? '0 2px 8px rgba(79,140,255,.35)' : 'none') + ';font-family:' + FONT + ';transition:all .2s">Powiel</button>' +
        '</div>';
    ...
    function bindEvents() {
      ...
      modal.querySelectorAll('.tpcs-what').forEach(function (lab) {
        lab.addEventListener('click', function () {
          var f = lab.getAttribute('data-field');
          if (f === 'display') copyDisplay = !copyDisplay;
          else if (f === 'filters') copyFilters = !copyFilters;
          else if (f === 'gfxtype') copyGfxType = !copyGfxType;
          render();
        });
      });
      ...
      modal.querySelector('.tpcs-apply').addEventListener('click', function () {
        var canSubmit = (copyDisplay || copyFilters || copyGfxType) && targetShops.length > 0;
        if (!canSubmit) return;
        onApply(sourceShop, targetShops.slice(), { display: copyDisplay, filters: copyFilters, gfxtype: copyGfxType });
        close();
      });
    }
  ...
  // v4.6.140: faktyczne kopiowanie ustawien per shop (display_mode/filtry/typ grafik).
  // API calls: getNode dla src → saveDisplayMode / setNodeFilters / setSettings dla tgts.
  async function copyShopSettings(nodeId, srcShop, tgtShops, lang, what) {
    ...
    // 2) Dla kazdego sklepu docelowego — wykonaj kopiowanie wybranych atrybutow
    for (var i = 0; i < tgtShops.length; i++) {
      var tgt = tgtShops[i];

      // 2a) display_mode (wyswietlanie)
      if (what.display) {
        try {
          var dispMode = srcNode.display_mode || '';
          var sort = srcNode.sort || '';
          var displayLimit = srcNode.display_limit || '';
        ...
        } catch (e) { errors.push('Sklep ' + tgt + ' display: ' + (e.message || e)); }
      }

      // 2b) Filtry (filter_default + filter_active + pricestep + per-filter opts)
      if (what.filters) {
        try {
          var isDef = !!srcNode.filter_default;
          if (isDef) {
            await saveValueFiltersDefault(nodeId, tgt, lang);
          } else {
            var activeIds = (srcNode.filter && Array.isArray(srcNode.filter.active))
              ? srcNode.filter.active.map(function (f) { return f.id; }) : [];
            var ps = (srcNode.filter_pricestep != null) ? String(srcNode.filter_pricestep) : '10';
            var psn = (srcNode.filter_pricestepnet != null) ? String(srcNode.filter_pricestepnet) : '10';
            // Per-filter opts (custom_name, display, default_enabled, pricerange, pricerangenet)
            var perOpts = {};
            var loadObj = function (key, prop) {
              if (srcNode[key] && typeof srcNode[key] === 'object') {
                for (var k in srcNode[key]) if (srcNode[key][k] != null && srcNode[key][k] !== '') {
                  if (!perOpts[k]) perOpts[k] = {};
                  perOpts[k][prop] = srcNode[key][k];
                }
              }
            };
            loadObj('filter_name', 'custom_name');
            loadObj('filter_display', 'display');
            loadObj('filter_default_enabled', 'default_enabled');
            if (srcNode.filter_pricerange && typeof srcNode.filter_pricerange === 'object') {
              if (!perOpts['pricerange_0']) perOpts['pricerange_0'] = {};
              perOpts['pricerange_0'].pricerange = srcNode.filter_pricerange;
            }
            if (srcNode.filter_pricerangenet && typeof srcNode.filter_pricerangenet === 'object') {
              if (!perOpts['pricerangenet_0']) perOpts['pricerangenet_0'] = {};
              perOpts['pricerangenet_0'].pricerangenet = srcNode.filter_pricerangenet;
            }
      ...
      // 2c) Typ grafiki (icon_search_type / icon_projector_type per lang per shop)
      // Z getParameterLangData: dt.langData[lang].icon_search_type[shop] = 'img' | 'img_rwd'
      if (what.gfxtype) {
        try {
          ...
          var iconSearchType = ld.icon_search_type && ld.icon_search_type[srcShop];
          var iconProjectorType = ld.icon_projector_type && ld.icon_projector_type[srcShop];
          var parts = [];
          if (iconSearchType) parts.push('icon_search_type[' + encodeURIComponent(lang) + '][' + encodeURIComponent(tgt) + ']=' + encodeURIComponent(iconSearchType));
          if (iconProjectorType) parts.push('icon_projector_type[' + encodeURIComponent(lang) + '][' + encodeURIComponent(tgt) + ']=' + encodeURIComponent(iconProjectorType));
          if (parts.length) {
