    ...
    if (extras.redirects) {
      ...
      for (var rsi = 0; rsi < redirShops.length; rsi++) {
        ...
        try {
          ...
          // v4.6.157: fetch przez iframe XHR — outer fetch zwraca login page (brak sesji)
          var html = await fetchTextFromIframe(gate);
          // v4.6.158: wykryj login page (konfiguracja systemu ma osobna sesje)
          if (html.indexOf('panel_login') >= 0 && html.indexOf('panel_password') >= 0) {
            redirAuthFailed = true;
      ...
      if (redirAuthFailed) {
        var goLogin = confirm('Eksport przekierowań wymaga zalogowania do "Konfiguracja systemu → Przekierowania" (osobna sesja od reszty panelu).\n\nKliknij OK aby otworzyć tę stronę w nowej karcie. Po zalogowaniu wróć tu i ponów eksport.');
        if (goLogin) {
          try { window.open('/panel/config-system-services.php?shop=' + encodeURIComponent(redirShops[0]) + '&action=redirects', '_blank'); } catch (e) {}
        }
        if (_panel) _panel.showStatus('Eksport: pominięto przekierowania (wymagane logowanie do konfig. systemu)');
      }
