...
`;
  ...
  function showAddValueModal(doc, paramId, paramName) {
    ...
    // State
    var bulkRecords = []; // lista { name, redirectFrom?, paramName? } z pliku \u2014 JUZ przefiltrowanych po paramName
    ...
    function render() {
      ...
      modal.innerHTML =
        ...
        '<div style="padding:14px 22px;border-top:1px solid #eef0f4;background:#fafbfc;display:flex;justify-content:flex-end;gap:8px;flex-shrink:0">' +
          ...
          '<button type="button" class="tpav-ok" style="padding:9px 22px;border-radius:8px;border:none;background:linear-gradient(135deg,#4f8cff,#3b6de0);color:#fff;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 2px 8px rgba(79,140,255,.35)">' + (hasBulk ? ('Utw\u00f3rz ' + bulkRecords.length + ' warto\u015bci') : 'Utw\u00f3rz') + '</button>' +
        '</div>';
    ...
    function handleFile(file) {
      ...
      reader.onload = function () {
        try {
          ...
          // v4.6.170: filtruj rekordy z paramName po znormalizowanej nazwie aktualnego parametru
          // (rekordy BEZ paramName akceptowane \u2014 backward compat z 1-2 kolumnowymi plikami)
          ...
          for (var i = 0; i < parsed.length; i++) {
            ...
            if (r.paramName) {
              ...
              // Brak paramName w pliku \u2014 akceptuj (1-2 kolumnowy plik)
              accepted.push(r);
            }
          }
          if (!accepted.length) {
    ...
    async function doCreateBulk(records) {
      ...
      // Wybierz current shop (1. z IAI.shops_list) dla redirectu
      ...
      var done = 0, errors = [], redirsAdded = 0;
      for (var i = 0; i < records.length; i++) {
        var rec = records[i];
        var n = rec.name;
        var label = 'Tworzenie ' + (i + 1) + '/' + records.length + '\u2026';
        ...
        try {
          ...
          // Wyci\u0105gnij nowe ID warto\u015bci (data.children[last].id)
          var newId = null;
          if (resp && resp.data && Array.isArray(resp.data.children) && resp.data.children.length) {
            newId = resp.data.children[resp.data.children.length - 1].id;
          }
          done++;
          // 2. Redirect (je\u015bli redirectFrom)
          if (rec.redirectFrom && newId) {
            okBtn.textContent = 'Redirect ' + (i + 1) + '/' + records.length + '\u2026';
            try {
              // getNode \u2192 seolink[0] dla nowo utworzonej warto\u015bci
              var rN = await fetchAjax('action=getNode&node_id=' + encodeURIComponent(newId) + '&lang=' + encodeURIComponent(LANG) + '&shop=' + encodeURIComponent(currentShop) + '&tree=parameters&parent=0');
              var seo = (rN && rN.data && rN.data.seolink && rN.data.seolink[0]) || '';
              if (seo) {
                // _addRedirect oczekuje path (uri) \u2014 wyci\u0105gamy path z absolute URL
                var newPath = seo;
                ...
                errors.push({ name: n, msg: 'Redirect: brak seolink dla nowej warto\u015bci (' + newId + ')' });
              }
            } catch (eR) {
      ...
      var summary = 'Utworzono ' + done + '/' + records.length + ' warto\u015bci' + (redirsAdded ? ', dodano ' + redirsAdded + ' przekierowa\u0144' : '');
      if (errors.length) {
        alert(summary + '.\n\nB\u0142\u0119dy:\n' + errors.map(function (e) { return '\u2022 ' + e.name + ' \u2014 ' + e.msg; }).join('\n'));
      } else if (_panel) {
        _panel.showStatus(summary + ' do \u201e' + paramName + '"');
      }
