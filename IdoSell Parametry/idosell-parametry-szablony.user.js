  ...
  function showTranslationsModal(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        '.tp-tr-modal { background:#fff; border-radius:12px; width:100%; max-width:1720px; max-height:calc(100vh - 40px); display:flex; flex-direction:column; box-shadow:0 20px 50px rgba(0,0,0,.3); overflow:hidden; font-family:"DM Sans",system-ui,-apple-system,"Segoe UI",Roboto,sans-serif; }',
    ...
    function renderTable() {
      ...
      // Grid template: pierwsza kolumna szersza (min 260px), pozostałe min 220px (żeby inputy nie były cięte)
      var gridCols = 'minmax(260px,1.6fr) ' + allLangs.map(function () { return 'minmax(220px,1fr)'; }).join(' ');
