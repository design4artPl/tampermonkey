  ...
  async function getAllSections(doc) {
    try {
      ...
      var sections = resp.data.map(function (s) {
        return { id: String(s.id), name: String(s.name || '').trim(), paramCount: 0 };
      });
      var BATCH = 5;
      for (var i = 0; i < sections.length; i += BATCH) {
        ...
        await Promise.all(batch.map(async function (s) {
          try {
            var r = await fetchAjax('action=getList&type=parameter&parent=' + encodeURIComponent(s.id) + '&lang=' + LANG);
            s.paramCount = (r && r.data) ? r.data.length : 0;
          } catch (e) {}
        }));
      }
