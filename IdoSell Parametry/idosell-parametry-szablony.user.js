...
`;
  ...
  // v4.6.169: parser plikow z lista wartosci do dodania
  function _parseValuesFile(content, filename) {
    ...
    try {
      if (ext === 'json') {
        ...
        names = arr.map(function (it) {
          if (typeof it === 'string') return it;
          if (it && typeof it === 'object') return it.name || it.value || it.label || '';
          return '';
        });
      } else if (ext === 'xml') {
        var parser = new DOMParser();
        var xmlDoc = parser.parseFromString(content, 'text/xml');
        // Szukaj <value>X</value>, <name>X</name>, <item>X</item>
        var nodes = xmlDoc.querySelectorAll('value, name, item, entry');
        for (var i = 0; i < nodes.length; i++) {
          var t = (nodes[i].getAttribute('name') || nodes[i].textContent || '').trim();
          if (t) names.push(t);
        }
      } else {
        // CSV / TXT \u2014 kazda linia = wartosc; lub kolumna 0 z przecinkow
        var lines = content.split(/\r?\n/);
        for (var li = 0; li < lines.length; li++) {
          var line = lines[li].trim();
          if (!line) continue;
          // Pomin naglowek "name" / "wartosc"
          if (li === 0 && /^(name|nazwa|wartosc|warto\u015b\u0107|value)$/i.test(line)) continue;
          // CSV: bierz pierwsza kolumne
          var cols = line.split(/[,;\t]/);
          var v = (cols[0] || '').trim().replace(/^["']|["']$/g, '');
          if (v) names.push(v);
        }
      }
    } catch (e) {
  ...
  function showAddValueModal(doc, paramId, paramName) {
    ...
    // State
    var bulkNames = []; // lista nazw z pliku

    function render() {
      var hasBulk = bulkNames.length > 0;
      var bulkListHtml = hasBulk ? bulkNames.map(function (n, idx) {
        return '<div class="tpav-item" data-idx="' + idx + '" style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-bottom:1px solid #f0f2f5;font-size:13px;color:#344054">' +
      ...
      modal.innerHTML =
          ...
          (hasBulk ?
            ...
            '<div style="margin-bottom:10px">' +
              '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">' +
                '<div style="font-size:11.5px;font-weight:600;color:#667085;text-transform:uppercase;letter-spacing:.5px">Za\u0142adowano ' + bulkNames.length + ' warto\u015bci</div>' +
        ...
        '<div style="padding:14px 22px;border-top:1px solid #eef0f4;background:#fafbfc;display:flex;justify-content:flex-end;gap:8px;flex-shrink:0">' +
          ...
          '<button type="button" class="tpav-ok" style="padding:9px 22px;border-radius:8px;border:none;background:linear-gradient(135deg,#4f8cff,#3b6de0);color:#fff;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 2px 8px rgba(79,140,255,.35)">' + (hasBulk ? ('Utw\u00f3rz ' + bulkNames.length + ' warto\u015bci') : 'Utw\u00f3rz') + '</button>' +
        '</div>';
    ...
    function bindEvents() {
      ...
      if (clearBtn) clearBtn.addEventListener('click', function () { bulkNames = []; render(); });

      modal.querySelectorAll('.tpav-rm').forEach(function (b) {
        b.addEventListener('click', function () {
          var idx = parseInt(b.getAttribute('data-idx'), 10);
          bulkNames.splice(idx, 1);
          render();
        });
      });
    ...
    async function doCreateBulk(names) {
      ...
      for (var i = 0; i < names.length; i++) {
        var n = names[i];
        okBtn.textContent = 'Tworzenie ' + (i + 1) + '/' + names.length + '\u2026';
        try {
      ...
      if (errors.length) {
        alert('Utworzono ' + done + '/' + names.length + ' warto\u015bci.\n\nB\u0142\u0119dy:\n' + errors.map(function (e) { return '\u2022 ' + e.name + ' \u2014 ' + e.msg; }).join('\n'));
      } else if (_panel) {
        _panel.showStatus('Utworzono ' + done + ' warto\u015bci do \u201e' + paramName + '"');
      }
      setTimeout(function () { (d.defaultView || window).location.reload(); }, 700);
    }
