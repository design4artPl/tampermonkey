...
// @version      4.4.0
// @description  Toolbar do grupowej edycji parametrow: panel-pro v1.2.3 inline, checkboxy, zaznaczanie, rozwijanie/zwijanie, grupowe usuwanie/edycja, import CSV
// @author       SyncOffer
...
(function () {
  ...
  /* === Embedded panel-pro widget v1.2.3 (inline) === */
/* ============================================================================
 * panel-pro v1.2.1
...
(function (root) {
  'use strict';
  if (root.PanelPro && root.PanelPro.VERSION === '1.2.3') return;

  var VERSION = '1.2.3';
  var STYLE_ID = 'panel-pro-styles-v1_2_3';
  ...
  var CSS = [
    ...
    '.panel-pro { background: #fff; border: 1px solid #dadce0; border-radius: 8px; display: flex; flex-direction: column; overflow: visible; width: 100%; min-width: 0; box-sizing: border-box; font-family: "Google Sans", Roboto, "Segoe UI", Arial, sans-serif; font-size: 14px; color: #202124; margin: 0 0 24px 0; position: relative; }',
...
`;
  ...
  function updateCounter(doc) {
    ...
    if (_panel && _panel.setCounter) {
      var _t = doc.querySelectorAll('li[id^="m_"]').length;
    ...
    _counterTimer = requestAnimationFrame(function() {
      ...
      if (!counterEl) return;
      var total = doc.querySelectorAll('li[id^="m_"]').length;
  ...
  function collapseAll(doc) {
    // Click all "hide" buttons — they are CSS-hidden but .click() still fires handlers
    const hideButtons = doc.querySelectorAll('.hideChildren');
    for (const btn of hideButtons) {
      btn.click();
    }
    // Update expand icons to collapsed state
  ...
  function waitForIframe(cb, attempts) {
    ...
    if (doc && doc.querySelector('h1')) {
      ...
      setTimeout(() => waitForIframe(cb, attempts + 1), 500);
    }
  }
