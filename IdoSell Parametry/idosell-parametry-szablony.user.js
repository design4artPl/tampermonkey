...
`;
  ...
  function mountPanelPro(doc, attempt) {
    ...
    var panel = PanelPro.mount(doc, {
      ...
      toolbar: {
        search: {
          placeholder: 'Szukaj parametrów i wartości — wiele oddziel | lub ,',
  ...
  // v4.6.90: dok\u0142adna UNIKALNA liczba towar\u00f3w parametru przez view-manager.php.
  // numberOfOccurrence jest zawodne (0 dla parametr\u00f3w kontekstowych, zani\u017ca dla
  // zwyk\u0142ych \u2014 20617: occ=2, faktycznie 3), a suma natywnych "towary:N" zawy\u017ca
  // (towar przypisany do kilku warto\u015bci liczony wielokrotnie \u2014 20617: suma 39, real 3).
  // POST view-manager.php?type=products&view=ajax_content z trait=<paramId> renderuje
  // przefiltrowan\u0105 list\u0119 ze <span class="total-result">N</span> = dok\u0142adna liczba
  // zdeduplikowanych towar\u00f3w. Dzia\u0142a te\u017c dla parametr\u00f3w kontekstowych.
  // Zweryfikowane demo37 2026-05-22. Cache 'pcount' (liczba nie zale\u017cy od j\u0119zyka).
  function fetchParamProductCount(nodeId) {
    var cached = tpCacheGet('pcount', nodeId, TP_TTL_PRODUCTS);
    if (cached !== null) return Promise.resolve(cached);
    var win = getIframeWin();
    return new Promise(function (resolve) {
      var xhr = new win.XMLHttpRequest();
      xhr.open('POST', '/panel/ajax/view-manager.php?type=products&view=ajax_content');
      xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      xhr.onload = function () {
        var m = /class="total-result">(\d+)</.exec(xhr.responseText || '');
      ...
      xhr.send(
        '__iai_shop_panel%5B__encoding%5D=utf-8'
        + '&trait=' + encodeURIComponent(nodeId)
        + '&trait_id%5B0%5D=' + encodeURIComponent(nodeId)
        + '&current_page=1&sort%5Bcolumn%5D=id&sort%5Btype%5D=d'
  ...
  async function loadParameterProductCountsInBackground(doc, rowsList) {
    ...
    async function processOne(t) {
      try {
        // v4.6.90: dok\u0142adna unikalna liczba towar\u00f3w (patrz fetchParamProductCount)
        var total = await fetchParamProductCount(t.nid);
        t.cell.textContent = '';
        if (total === null) {
