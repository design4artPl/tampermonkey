...
`;
  ...
  function enhanceRow(li, doc) {
    ...
    if (isVal) {
      ...
      seoAction.className = 'material-symbols-outlined tp-action-btn';
  ...
  function createEditorWidget(doc, initialHtml, rows) {
    ...
    wysBtn.type = 'button'; wysBtn.className = 'tp-ed-btn tp-ed-on'; wysBtn.textContent = 'Edytor wizualny';
    var srcBtn = doc.createElement('button');
    srcBtn.type = 'button'; srcBtn.className = 'tp-ed-btn'; srcBtn.textContent = 'Źródło HTML';
    bar.appendChild(wysBtn); bar.appendChild(srcBtn); wrap.appendChild(bar);
    ...
    wysEl.className = 'tp-ed-wys'; wysEl.contentEditable = 'true';
    wysEl.style.minHeight = ((rows || 7) * 22) + 'px';
    wysEl.innerHTML = initialHtml || '';

    // Wlasny WYSIWYG: contentEditable + pasek formatowania (execCommand).
    // Panelowy TinyMCE to v6 zablokowany dla zewn. wywolan (zweryfikowane na
    // demo37: tinymce.init nie tworzy edytora w zadnej konfiguracji) - dlatego
    // wlasny, niezawodny edytor w tym samym dokumencie.
    function sync() { src.value = wysEl.innerHTML; }
    var fmt = doc.createElement('div');
    fmt.className = 'tp-ed-fmt';
    fmt.style.cssText = 'display:flex;flex-wrap:wrap;gap:4px;padding:6px;background:#f8f9fb;border:1px solid #e0e3ea;border-bottom:none;border-radius:10px 10px 0 0;';
    function cmd(c, v) { try { doc.execCommand(c, false, v); } catch (e) {} }
    function mkF(html, title, fn) {
      ...
      b.addEventListener('click', function (e) { e.preventDefault(); wysEl.focus(); try { fn(); } catch (_) {} sync(); });
      return b;
    }
    fmt.appendChild(mkF('<b>B</b>', 'Pogrubienie', function () { cmd('bold'); }));
    fmt.appendChild(mkF('<i>I</i>', 'Kursywa', function () { cmd('italic'); }));
    fmt.appendChild(mkF('<u>U</u>', 'Podkreślenie', function () { cmd('underline'); }));
    fmt.appendChild(mkF('&bull; lista', 'Lista punktowana', function () { cmd('insertUnorderedList'); }));
    fmt.appendChild(mkF('1. lista', 'Lista numerowana', function () { cmd('insertOrderedList'); }));
    fmt.appendChild(mkF('Akapit', 'Akapit', function () { cmd('formatBlock', 'p'); }));
    fmt.appendChild(mkF('Link', 'Wstaw link', function () {
      var u = (typeof prompt === 'function') ? prompt('Adres URL linku:', 'https://') : null;
      if (u) cmd('createLink', u);
    }));
    fmt.appendChild(mkF('Usuń link', 'Usuń link', function () { cmd('unlink'); }));
    fmt.appendChild(mkF('✕ format', 'Wyczyść formatowanie', function () { cmd('removeFormat'); cmd('unlink'); }));
    wrap.appendChild(fmt);
    wrap.appendChild(wysEl);

    wysEl.addEventListener('input', sync);
    wysEl.addEventListener('blur', sync);

    function showSrc() {
      src.value = wysEl.innerHTML;
      wysEl.style.display = 'none'; fmt.style.display = 'none';
      src.style.display = '';
    ...
    function showWys() {
      ...
      src.style.display = 'none';
      fmt.style.display = ''; wysEl.style.display = '';
      wysBtn.classList.add('tp-ed-on'); srcBtn.classList.remove('tp-ed-on');
    }
    srcBtn.addEventListener('click', showSrc);
    wysBtn.addEventListener('click', showWys);
    // domyslnie: edytor wizualny (jak prosil uzytkownik)
    src.style.display = 'none'; fmt.style.display = ''; wysEl.style.display = '';
  ...
  // v4.6.33: osobny modal SEO dla WARTOŚCI (Meta tagi / Robots / Adres URL / Przekierowania)
  // API navigation.php (tree=parameters, parent=0) zweryfikowane round-trip na demo37.
  // Wartości NIE mają własnego URL/redirects/canonical (getSEO/editNodeSEOData = no-op) —
  // Adres URL = read-only auto (seolink), Przekierowania = szkielet (port do strony
  // config-system-services redirects omówimy później). Meta/Robots zapis przez
  // editNodeShopData DOKŁADNYM natywnym payloadem (zachowuje headline_name/view/filter_default).
  function showValueSeoModal(doc, nodeId) {
    var d = doc || document;
    if (_panel && _panel.showStatus) _panel.showStatus('Wczytywanie SEO…');
    function navPost(body) { return fetchAjaxRaw('/panel/ajax/parameters.php', body); }
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      var langs = Object.keys(dt.langData).sort(function (a, b) {
        var ia = order.indexOf(a); var ib = order.indexOf(b);
        if (ia < 0) ia = 999; if (ib < 0) ib = 999; return (ia - ib) || a.localeCompare(b);
      });
      ...
      // kandydaci sklepów: unia kluczy per-sklep w langData; fallback 1..7
      var ss = {};
      langs.forEach(function (lg) {
        var L = dt.langData[lg] || {};
        ['icon_search_type', 'icon_projector_type', 'display_mode', 'icon_search', 'icon_projector'].forEach(function (k) {
          if (L[k] && typeof L[k] === 'object') Object.keys(L[k]).forEach(function (s) { if (/^\d+$/.test(s)) ss[s] = true; });
        });
      });
      ...
      var overlay = d.createElement('div'); overlay.className = 'tp-overlay';
      var modal = d.createElement('div'); modal.className = 'tp-modal';
      modal.style.width = '760px'; modal.style.maxWidth = 'calc(100vw - 32px)';
      var header = d.createElement('div');
      header.className = 'tp-modal-header';
      header.style.cssText = 'background:#fff;color:#202124;border-bottom:1px solid #eef0f4;';
      header.innerHTML =
        '<span style="width:40px;height:40px;border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#e8eeff;color:#1a73e8;"><span class="material-symbols-outlined" style="font-size:22px">travel_explore</span></span>' +
        '<span style="display:flex;flex-direction:column;gap:2px;min-width:0;">' +
          '<span style="font-size:16px;font-weight:600;color:#1a1a2e;">SEO — ' + esc(elName) + '</span>' +
          '<span style="font-size:12.5px;font-weight:400;color:#98a2b3;">ID: ' + esc(nodeId) + ' · wartość parametru</span>' +
        '</span>' +
        '<button type="button" class="tp-modal-header-close" aria-label="Zamknij" style="color:#b0b8c9;">✕</button>';

      var body = d.createElement('div'); body.className = 'tp-modal-body tp-se';

      // Karta: Sklep + Język
      var cardCtx = d.createElement('div'); cardCtx.className = 'tp-se-card';
      cardCtx.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">storefront</span>Sklep i język</div>';
      var bodyCtx = d.createElement('div'); bodyCtx.className = 'tp-se-body'; cardCtx.appendChild(bodyCtx);
      var rowShop = d.createElement('div'); rowShop.className = 'tp-se-row';
      rowShop.innerHTML = '<span class="tp-se-lbl">Sklep</span>';
      var ctlShop = d.createElement('div'); ctlShop.className = 'tp-se-ctl';
      var shopSel = d.createElement('select');
      shopSel.innerHTML = shops.map(function (s) { return '<option value="' + s + '">Sklep ' + s + '</option>'; }).join('');
      ctlShop.appendChild(shopSel); rowShop.appendChild(ctlShop); bodyCtx.appendChild(rowShop);
      var rowLng = d.createElement('div'); rowLng.className = 'tp-se-row';
      rowLng.innerHTML = '<span class="tp-se-lbl">Język</span>';
      var ctlLng = d.createElement('div'); ctlLng.className = 'tp-se-ctl';
      var lngSel = d.createElement('select');
      lngSel.innerHTML = langs.map(function (lg) { return '<option value="' + lg + '"' + (lg === curLang ? ' selected' : '') + '>' + esc(getLangName(lg)) + '</option>'; }).join('');
      ctlLng.appendChild(lngSel); rowLng.appendChild(ctlLng); bodyCtx.appendChild(rowLng);
      body.appendChild(cardCtx);

      // Karta: Meta tagi
      var cardMeta = d.createElement('div'); cardMeta.className = 'tp-se-card';
      cardMeta.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">code</span>Meta tagi</div>';
      var bodyMeta = d.createElement('div'); bodyMeta.className = 'tp-se-body'; cardMeta.appendChild(bodyMeta);
      function radioRow(parent, label, name, opts, getCur) {
        var row = d.createElement('div'); row.className = 'tp-se-row';
        row.innerHTML = '<span class="tp-se-lbl">' + esc(label) + '</span>';
        var c = d.createElement('div'); c.className = 'tp-se-ctl'; c.style.cssText = 'display:flex;flex-wrap:wrap;gap:12px;';
        var inputs = {};
        opts.forEach(function (o) {
          var lab = d.createElement('label'); lab.style.cssText = 'display:flex;align-items:center;gap:5px;font-size:12.5px;color:#344054;cursor:pointer;';
          var rb = d.createElement('input'); rb.type = 'radio'; rb.name = name; rb.value = o.v;
          inputs[o.v] = rb;
          lab.appendChild(rb); lab.appendChild(d.createTextNode(o.t)); c.appendChild(lab);
        });
        row.appendChild(c); parent.appendChild(row);
        row.__set = function (v) { if (inputs[v]) inputs[v].checked = true; };
        row.__get = function () { for (var k in inputs) if (inputs[k].checked) return k; return ''; };
        row.__inputs = inputs;
        return row;
      }
      var rMeta = radioRow(bodyMeta, 'Ustawienia meta', 'tp_seo_metadef', [
        { v: 'y', t: 'Wygeneruj automatycznie' }, { v: 'n', t: 'Użyj podanych poniżej' }
      ]);
      var metaBox = d.createElement('div');
      function txtRow(parent, label, tag) {
        var row = d.createElement('div'); row.className = 'tp-se-row tp-se-row--col';
        row.innerHTML = '<span class="tp-se-lbl" style="margin-bottom:4px;">' + esc(label) + '</span>';
        var inp = tag === 'area' ? d.createElement('textarea') : d.createElement('input');
        if (tag !== 'area') inp.type = 'text'; else inp.rows = 3;
        inp.style.cssText = 'width:100%;box-sizing:border-box;';
        row.appendChild(inp); parent.appendChild(row);
        row.__input = inp; return row;
      }
      var rMT = txtRow(metaBox, 'Meta title', 'text');
      var rMD = txtRow(metaBox, 'Meta description', 'area');
      var rMK = txtRow(metaBox, 'Meta keywords', 'text');
      bodyMeta.appendChild(metaBox);
      ['y', 'n'].forEach(function (k) { rMeta.__inputs[k].addEventListener('change', function () { metaBox.style.display = rMeta.__get() === 'n' ? '' : 'none'; }); });
      body.appendChild(cardMeta);

      // Karta: Robots
      var cardRob = d.createElement('div'); cardRob.className = 'tp-se-card';
      cardRob.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">smart_toy</span>Robots</div>';
      var bodyRob = d.createElement('div'); bodyRob.className = 'tp-se-body'; cardRob.appendChild(bodyRob);
      var rRi = radioRow(bodyRob, 'Meta robots index', 'tp_seo_ri', [
        { v: 'default', t: 'Automatycznie' }, { v: 'index', t: 'index' }, { v: 'noindex', t: 'noindex' }
      ]);
      var rRf = radioRow(bodyRob, 'Meta robots follow', 'tp_seo_rf', [
        { v: 'default', t: 'Automatycznie' }, { v: 'follow', t: 'follow' }, { v: 'nofollow', t: 'nofollow' }
      ]);
      body.appendChild(cardRob);

      // Karta: Adres URL (read-only — wartości nie mają własnego URL)
      var cardUrl = d.createElement('div'); cardUrl.className = 'tp-se-card';
      cardUrl.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">link</span>Adres URL</div>';
      var bodyUrl = d.createElement('div'); bodyUrl.className = 'tp-se-body'; cardUrl.appendChild(bodyUrl);
      var rowUrlMode = d.createElement('div'); rowUrlMode.className = 'tp-se-row';
      rowUrlMode.innerHTML = '<span class="tp-se-lbl">Tryb adresu URL</span><div class="tp-se-ctl"><span style="display:inline-flex;align-items:center;gap:6px;color:#475467;font-size:13px;"><span class="material-symbols-outlined" style="font-size:16px;color:#98a2b3;">tune</span>Automatyczny</span></div>';
      bodyUrl.appendChild(rowUrlMode);
      var rowUrlVal = d.createElement('div'); rowUrlVal.className = 'tp-se-row';
      var urlValSpan = d.createElement('span'); urlValSpan.style.cssText = 'color:#667085;font-size:12.5px;word-break:break-all;';
      var rowUrlValCtl = d.createElement('div'); rowUrlValCtl.className = 'tp-se-ctl'; rowUrlValCtl.appendChild(urlValSpan);
      rowUrlVal.innerHTML = '<span class="tp-se-lbl">Adres URL</span>'; rowUrlVal.appendChild(rowUrlValCtl);
      bodyUrl.appendChild(rowUrlVal);
      var urlOrig = d.createElement('div');
      urlOrig.style.cssText = 'margin-top:8px;font-size:12.5px;color:#475467;background:#f8f9fb;border:1px solid #e8ebf0;border-radius:8px;padding:9px 12px;';
      bodyUrl.appendChild(urlOrig);
      body.appendChild(cardUrl);

      // Karta: Przekierowania (szkielet — logika później)
      var cardRdr = d.createElement('div'); cardRdr.className = 'tp-se-card';
      cardRdr.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">redo</span>Przekierowania</div>';
      var bodyRdr = d.createElement('div'); bodyRdr.className = 'tp-se-body'; cardRdr.appendChild(bodyRdr);
      var rdrAdd = d.createElement('button');
      rdrAdd.type = 'button'; rdrAdd.className = 'tp-btn-modal-secondary';
      rdrAdd.style.cssText = 'font-size:12px;padding:7px 14px;border-style:dashed;';
      rdrAdd.innerHTML = '<span class="material-symbols-outlined" style="font-size:16px;vertical-align:-3px;">add</span> Dodaj przekierowanie';
      rdrAdd.addEventListener('click', function () {
        if (_panel) _panel.showStatus('Przekierowania per sklep — wkrótce (port do listy przekierowań)');
        else alert('Zarządzanie przekierowaniami zostanie podpięte do listy przekierowań per sklep (wkrótce).');
      });
      var rdrEmpty = d.createElement('div');
      rdrEmpty.style.cssText = 'text-align:center;color:#98a2b3;font-size:12.5px;padding:14px 0 4px;';
      rdrEmpty.textContent = 'Brak przekierowań';
      bodyRdr.appendChild(rdrAdd); bodyRdr.appendChild(rdrEmpty);
      body.appendChild(cardRdr);

      var footer = d.createElement('div'); footer.className = 'tp-modal-footer';
      var cancel = d.createElement('button'); cancel.className = 'tp-btn-modal-secondary'; cancel.type = 'button'; cancel.textContent = 'Anuluj';
      var ok = d.createElement('button'); ok.className = 'tp-btn-modal-primary'; ok.type = 'button'; ok.textContent = 'Zapisz';
      ...
      var loaded = null; // ostatnio wczytany getNode.data (do zachowania pól)
      function applyData(dd) {
        loaded = dd || {};
        var hasMeta = (dd.meta_title || dd.meta_description || dd.meta_keywords);
        rMeta.__set(hasMeta ? 'n' : 'y');
        rMT.__input.value = dd.meta_title || '';
        rMD.__input.value = dd.meta_description || '';
        rMK.__input.value = dd.meta_keywords || '';
        metaBox.style.display = hasMeta ? '' : 'none';
        rRi.__set(dd.meta_robots_index || 'default');
        rRf.__set(dd.meta_robots_follow || 'default');
        var seo = (dd.seolink && dd.seolink[0]) || '';
        var path = seo;
        try { path = new URL(seo).pathname; } catch (e) {}
        urlValSpan.textContent = path || '(automatyczny)';
        urlOrig.innerHTML = '<strong>Oryginalny adres:</strong> ' + (seo
          ? '<a href="' + esc(seo) + '" target="_blank" rel="noopener" style="color:#4f8cff;word-break:break-all;">' + esc(seo) + '</a>'
          : '<span style="color:#98a2b3;">(brak)</span>');
      }
      function load() {
        ok.disabled = true; ok.textContent = 'Wczytywanie…';
        navPost('action=getNode&tes=t&node_id=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(curLang) +
          '&shop=' + encodeURIComponent(curShop) + '&tree=parameters&parent=0').then(function (r) {
          var dd = r && r.data;
          if (!dd) { alert('Brak danych SEO dla tego sklepu/języka.'); ok.disabled = false; ok.textContent = 'Zapisz'; return; }
          applyData(dd);
          ok.disabled = false; ok.textContent = 'Zapisz';
        }).catch(function (e) { ok.disabled = false; ok.textContent = 'Zapisz'; alert('Błąd wczytywania SEO: ' + (e.message || e)); });
      }
      shopSel.addEventListener('change', function () { curShop = shopSel.value; load(); });
      lngSel.addEventListener('change', function () { curLang = lngSel.value; load(); });
      load();

      ok.addEventListener('click', function () {
        var md = rMeta.__get();
        var mt = rMT.__input.value || '', mdsc = rMD.__input.value || '', mk = rMK.__input.value || '';
        var mi = rRi.__get() || 'default', mf = rRf.__get() || 'default';
        ok.disabled = true; ok.textContent = 'Zapisywanie…';
        // świeży getNode → zachowaj headline_name/filter_default/view (nie nadpisuj)
        navPost('action=getNode&tes=t&node_id=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(curLang) +
          '&shop=' + encodeURIComponent(curShop) + '&tree=parameters&parent=0').then(function (r) {
          var dd = (r && r.data) || loaded || {};
          var keepFD = dd.filter_default ? 'y' : 'n';
          var keepView = (dd.display_limit || dd.sort || (dd.display_mode && dd.display_mode_default && dd.display_mode !== dd.display_mode_default)) ? 'own' : 'default';
          var keepHl = dd.headline_name || '';
          ...
          ok.disabled = false; ok.textContent = 'Zapisz';
          if (_panel) _panel.showStatus('Zapisano SEO (sklep ' + curShop + ', ' + getLangName(curLang) + ')');
          close();
        }).catch(function (e) {
          ok.disabled = false; ok.textContent = 'Zapisz';
          alert('Błąd zapisu SEO: ' + (e.message || e));
        });
      });
  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      // ====== v4.6.33: Ustawienia w sklepie (tylko WARTOŚCI) — nie-SEO ======
      // SEO (Meta/Robots/Adres URL/Przekierowania) jest w osobnym modalu (ikona SEO).
      // Tu zostają: Nazwa w nagłówku + Opis na liście towarów (per sklep+język).
      // Zapis editNodeShopData DOKŁADNYM payloadem — meta/robots/view/filter_default
      // ZACHOWYWANE z getNode (zero nadpisania). Pre-konfiguracja „na zaś" dozwolona.
      if (isVal) {
        var cardN = d.createElement('div'); cardN.className = 'tp-se-card';
        cardN.innerHTML = '<div class="tp-se-head"><span class="material-symbols-outlined">tune</span>Ustawienia w sklepie</div>';
        var bodyN = d.createElement('div'); bodyN.className = 'tp-se-body';
        cardN.appendChild(bodyN);
        var nHint = d.createElement('div'); nHint.className = 'tp-se-hint';
        nHint.innerHTML = '<span>ℹ</span><span>Nazwa w nagłówku strony i opis na liście towarów dla tej wartości — osobno per sklep i język. Meta/Robots/Adres URL są w osobnym oknie (ikona SEO). „Opis na liście towarów" to inny opis niż w karcie „Treść".</span>';
        bodyN.appendChild(nHint);

        var rowNShop = d.createElement('div'); rowNShop.className = 'tp-se-row';
        var lblNShop = d.createElement('span'); lblNShop.className = 'tp-se-lbl'; lblNShop.textContent = 'Sklep';
        var ctlNShop = d.createElement('div'); ctlNShop.className = 'tp-se-ctl';
        var navShopSel = d.createElement('select');
        navShopSel.innerHTML = '<option>Wczytywanie…</option>'; navShopSel.disabled = true;
        ctlNShop.appendChild(navShopSel);
        rowNShop.appendChild(lblNShop); rowNShop.appendChild(ctlNShop);
        bodyN.appendChild(rowNShop);

        var navHost = d.createElement('div'); navHost.style.cssText = 'margin-top:10px;';
        bodyN.appendChild(navHost);
        body.appendChild(cardN);

        function navPost(body2) { return fetchAjaxRaw('/panel/ajax/parameters.php', body2); }
        function navGetNode(shop, lg) {
          return navPost('action=getNode&tes=t&node_id=' + encodeURIComponent(nodeId) +
            '&lang=' + encodeURIComponent(lg) + '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0');
        }
        function navEsc(s) { return (typeof escapeHtml === 'function') ? escapeHtml(s == null ? '' : String(s)) : String(s == null ? '' : s); }

        var navShopSet = {};
        langs.forEach(function (lg) {
          var L = dt.langData[lg] || {};
          ['icon_search_type', 'icon_projector_type', 'display_mode', 'icon_search', 'icon_projector'].forEach(function (k) {
            if (L[k] && typeof L[k] === 'object') Object.keys(L[k]).forEach(function (s) { if (/^\d+$/.test(s)) navShopSet[s] = true; });
          });
        });
        var navShops = Object.keys(navShopSet).sort(function (a, b) { return (+a) - (+b); });
        if (!navShops.length) navShops = ['1', '2', '3', '4', '5', '6', '7'];
        var navEd = null;

        function renderNavBody(shop) {
          navHost.innerHTML = '';
          try { if (navEd && navEd.destroy) navEd.destroy(); } catch (e) {}
          navEd = null;

          // Nazwa w nagłówku (auto / własna)
          var rowHlMode = d.createElement('div'); rowHlMode.className = 'tp-se-row';
          rowHlMode.innerHTML = '<span class="tp-se-lbl">Nazwa w nagłówku</span>';
          var hlCtl = d.createElement('div'); hlCtl.className = 'tp-se-ctl'; hlCtl.style.cssText = 'display:flex;flex-wrap:wrap;gap:12px;';
          var rbAuto = d.createElement('input'); rbAuto.type = 'radio'; rbAuto.name = 'tp_nav_hl';
          var rbOwn = d.createElement('input'); rbOwn.type = 'radio'; rbOwn.name = 'tp_nav_hl';
          var la = d.createElement('label'); la.style.cssText = 'display:flex;align-items:center;gap:5px;font-size:12.5px;color:#344054;cursor:pointer;'; la.appendChild(rbAuto); la.appendChild(d.createTextNode('wygeneruj automatycznie'));
          var lo = d.createElement('label'); lo.style.cssText = la.style.cssText; lo.appendChild(rbOwn); lo.appendChild(d.createTextNode('własna nazwa'));
          hlCtl.appendChild(la); hlCtl.appendChild(lo);
          rowHlMode.appendChild(hlCtl); navHost.appendChild(rowHlMode);
          var rowHlText = d.createElement('div'); rowHlText.className = 'tp-se-row';
          rowHlText.innerHTML = '<span class="tp-se-lbl"></span>';
          var hlTxtCtl = d.createElement('div'); hlTxtCtl.className = 'tp-se-ctl';
          var hlInput = d.createElement('input'); hlInput.type = 'text'; hlInput.placeholder = 'własna nazwa w nagłówku strony'; hlInput.style.cssText = 'width:100%;box-sizing:border-box;';
          hlTxtCtl.appendChild(hlInput); rowHlText.appendChild(hlTxtCtl); navHost.appendChild(rowHlText);
          function hlSync() { rowHlText.style.display = rbOwn.checked ? '' : 'none'; }
          rbAuto.addEventListener('change', hlSync); rbOwn.addEventListener('change', hlSync);

          // Opis na liście towarów
          var rowD2 = d.createElement('div'); rowD2.className = 'tp-se-row tp-se-row--col';
          navEd = createEditorWidget(d, '', 6);
          var edHead2 = d.createElement('div'); edHead2.className = 'tp-ed-v2head';
          var edLbl2 = d.createElement('span'); edLbl2.className = 'tp-ed-v2lbl'; edLbl2.textContent = 'Opis wyświetlany na liście towarów (sklep ' + shop + ', ' + getLangName(curLang) + ')';
          edHead2.appendChild(edLbl2);
          var edBar2 = navEd.container.querySelector('.tp-ed-bar');
          if (edBar2) edHead2.appendChild(edBar2);
          navEd.container.insertBefore(edHead2, navEd.container.firstChild);
          rowD2.appendChild(navEd.container);
          navHost.appendChild(rowD2);

          var btnRow = d.createElement('div'); btnRow.style.cssText = 'display:flex;justify-content:flex-end;gap:8px;margin-top:10px;';
          var navState = d.createElement('span'); navState.style.cssText = 'font-size:12px;color:#98a2b3;align-self:center;';
          var navSave = d.createElement('button'); navSave.type = 'button'; navSave.className = 'tp-btn-modal-secondary';
          navSave.style.cssText = 'font-size:12px;padding:6px 14px;'; navSave.textContent = 'Zapisz ustawienia (sklep ' + shop + ')';
          btnRow.appendChild(navState); btnRow.appendChild(navSave);
          navHost.appendChild(btnRow);

          navState.textContent = 'Wczytywanie…';
          navGetNode(shop, curLang).then(function (r) {
            var dd = (r && r.data) || {};
            var hasHl = !!dd.headline_name;
            rbAuto.checked = !hasHl; rbOwn.checked = hasHl;
            hlInput.value = dd.headline_name || '';
            hlSync();
            return navPost('action=getNodeDesc&node=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(curLang) +
              '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0').then(function (gd) {
              try { navEd.setValue((gd && gd.desc) || ''); } catch (e) {}
              navState.textContent = '';
            });
          }).catch(function () { navState.textContent = 'błąd wczytywania'; });

          navSave.addEventListener('click', function () {
            var hl = rbOwn.checked ? (hlInput.value || '') : '';
            var descHtml = ''; try { descHtml = navEd.getValue(); } catch (e) {}
            navSave.disabled = true; navState.textContent = 'Zapisywanie…';
            // świeży getNode → zachowaj meta/robots/view/filter_default (zero nadpisania)
            navGetNode(shop, curLang).then(function (r) {
              var dd = (r && r.data) || {};
              var keepFD = dd.filter_default ? 'y' : 'n';
              var keepView = (dd.display_limit || dd.sort || (dd.display_mode && dd.display_mode_default && dd.display_mode !== dd.display_mode_default)) ? 'own' : 'default';
              var keepMD = (dd.meta_title || dd.meta_description || dd.meta_keywords) ? 'n' : 'y';
              var b = 'action=editNodeShopData&node_type=false&filter_default=' + keepFD +
                '&target=false&lang=' + encodeURIComponent(curLang) + '&shop=' + encodeURIComponent(shop) +
                '&tree=parameters&node=' + encodeURIComponent(nodeId) + '&view=' + keepView + '&parent=0' +
                '&click_action=false&element_hidden=false&displayShowAll=false&showAllIs=false&expand=false' +
                '&node_gfx=false&gfx_active_type=false&gfx_inactive_type=false&gfx_omo_type=false&gfx_type=false' +
                '&meta_default=' + keepMD + '&meta_keywords=' + encodeURIComponent(dd.meta_keywords || '') +
                '&meta_title=' + encodeURIComponent(dd.meta_title || '') + '&meta_desc=' + encodeURIComponent(dd.meta_description || '') +
                '&meta_index=' + (dd.meta_robots_index || 'default') + '&meta_follow=' + (dd.meta_robots_follow || 'default') +
                '&headline_name=' + encodeURIComponent(hl);
              return navPost(b);
            }).then(function (r1) {
              if (!r1 || r1.status !== 'ok') throw new Error((r1 && (r1.error || r1.message)) || 'editNodeShopData');
              return navPost('action=setNodeDesc&desc=' + encodeURIComponent(descHtml) + '&node=' + encodeURIComponent(nodeId) +
                '&lang=' + encodeURIComponent(curLang) + '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0').then(function (r2) {
                if (!r2 || r2.status !== 'ok') throw new Error((r2 && (r2.error || r2.message)) || 'setNodeDesc');
              });
            }).then(function () {
              navSave.disabled = false; navState.textContent = 'Zapisano ✓';
              if (_panel) _panel.showStatus('Zapisano ustawienia w sklepie ' + shop + ' (' + getLangName(curLang) + ')');
            }).catch(function (e) {
              navSave.disabled = false; navState.textContent = 'błąd zapisu';
              alert('Błąd zapisu: ' + (e.message || e));
            });
          });
        }

        navShopSel.disabled = false;
        navShopSel.innerHTML = navShops.map(function (s) { return '<option value="' + s + '">Sklep ' + s + '</option>'; }).join('');
        navShopSel.addEventListener('change', function () { renderNavBody(navShopSel.value); });

        var _navPrevLang = null;
        function navMaybeReload() {
          if (_navPrevLang === curLang) return;
          _navPrevLang = curLang;
          renderNavBody(navShopSel.value || navShops[0]);
        }
        navMaybeReload();
        var _navLangPoll = setInterval(function () {
          if (!d.body.contains(overlay)) { clearInterval(_navLangPoll); return; }
          navMaybeReload();
        }, 600);
      }
      ...
      ok.addEventListener('click', function () {
        ...
        fetchAjax(nameBody).then(function (r1) {
        ...
        }, function (e) { settingsErr = (e && e.message) || String(e); }).then(function () {
          // v4.6.29: RĘCZNA pętla indeksowa (BEZ Array.prototype.reduce/slice/forEach) —
          // panel ładuje Prototype.js, który psuł iterację reduce/slice → setDescription
          // nie był wysyłany. Opis zapisujemy NIEZALEŻNIE od wyniku setSettings.
          function sendDesc(i) {
            if (i >= langs.length) return Promise.resolve();
            var lg = langs[i];
            var dBody = 'action=setDescription&id=' + encodeURIComponent(nodeId) + '&lang=' + lg + '&description=' + encodeURIComponent((st[lg] && st[lg].description) || '');
            return fetchAjax(dBody).then(function () {}, function () {}).then(function () { return sendDesc(i + 1); });
          }
          return sendDesc(0);
        }).then(function () {
          ...
          if (_panel) _panel.showStatus(settingsErr
            ...
            : ('Zapisano zmiany elementu „' + newName + '”'), !!settingsErr);
          close();
        }).catch(function (e) {
          ok.disabled = false; ok.textContent = 'Zapisz';
