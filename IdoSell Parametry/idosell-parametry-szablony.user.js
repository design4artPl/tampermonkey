...
/* v4.6.84: dla PARAMETRU rearrange — folder pod checkbox wartości.
   Natural DOM order: drag(slot 1), check(slot 2), expand(slot 3).
   Dla param: check(slot 1), expand(slot 2), drag(slot 3, empty/hidden).
   Wynik wizualny: param ma [checkbox][folder][empty][name],
                   wartość ma [drag][checkbox][arrow][name].
   Folder param i checkbox wartości oba w slot 2 → ta sama kolumna X. */
li.tp-row-param > .tp-col-drag   { grid-column: 3 !important; visibility: hidden !important; }
li.tp-row-param > .tp-col-check  { grid-column: 1 !important; }
li.tp-row-param > .tp-col-expand { grid-column: 2 !important; }
.tp-drag-handle {
...
`;
  ...
  function enhanceRow(li, doc) {
    ...
    // v4.6.84: typ wiersza jako klasa — używane do CSS rearranging (check/expand/drag
    // slot order per-type) aby folder parametru był nad checkboxem wartości.
    if (isPar) li.classList.add('tp-row-param');
    else if (isVal) li.classList.add('tp-row-value');
