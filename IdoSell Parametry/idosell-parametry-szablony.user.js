...
`;
  ...
  // v4.6.53: STEAL-NATIVE TinyMCE v6. parameters.php to quirks-mode → własny
  // tinymce.init rzuca "not in standards mode". Workaround: otworzyć natywne
  // editEl_<id>→changeDesc_<lang> offscreen, poczekać aż panel zainicjalizuje
  // TinyMCE, wyciągnąć .tox-tinymce + textarea i przenieść do naszego hosta.
  // Edytor żyje (getContent/setContent ✓). Zweryfikowane demo37 2026-05-20.
  function stealNativeEditor(nodeId, useLang) {
    var GIDOC = (typeof getIframeDoc === 'function') ? getIframeDoc() : document;
    var GW = (typeof getIframeWin === 'function') ? getIframeWin() : window;
    if (!GW.tinymce) return Promise.reject(new Error('tinymce nie załadowane'));
    return new Promise(function (resolve, reject) {
      // Ukryj natywne UI synchronicznie ZANIM jakikolwiek dialog się pojawi
      var styId = 'tp-tm-hide';
      var sty = GIDOC.getElementById(styId);
      if (!sty) {
        sty = GIDOC.createElement('style'); sty.id = styId;
        var HIDE = 'position:fixed !important;left:-99999px !important;top:-99999px !important;width:1px !important;height:1px !important;max-height:1px !important;opacity:0 !important;visibility:hidden !important;pointer-events:none !important;z-index:-2147483647 !important;';
        var SEL = ['[id^="form_editEl_"]','[id^="editElWindow"]','[id^="longdesc_edit_window"]','#windowTopLayer_c','#wait_c','#wait_mask','#ajax_c','.yui-panel-container.shadow','.yui3-widget-mask','.ui-widget-overlay'];
        sty.textContent = SEL.map(function (s) { return 'html.tp-tm-loading ' + s; }).join(',') + '{' + HIDE + '}';
        GIDOC.head.appendChild(sty);
      }
      try { GIDOC.documentElement.classList.add('tp-tm-loading'); } catch (e) {}
      var done = false;
      function fin() { try { GIDOC.documentElement.classList.remove('tp-tm-loading'); } catch (e) {} }
      function rej(e) { if (done) return; done = true; fin(); reject(e); }
      function res(v) { if (done) return; done = true; fin(); resolve(v); }
      var beforeIds = ((GW.tinymce.get() || []).map(function (e) { return e.id; }));

      var p = 0;
      var ivOpen = setInterval(function () {
        p++;
        var sm = GIDOC.getElementById('showMenuSub_' + nodeId);
        if (sm) { try { sm.click(); } catch (e) {} }
        var edl = GIDOC.getElementById('editEl_' + nodeId);
        if (edl) { clearInterval(ivOpen); try { edl.click(); } catch (e) {} waitChangeDesc(); }
        else if (p > 40) { clearInterval(ivOpen); rej(new Error('editEl_' + nodeId + ' nie pojawił się')); }
      }, 120);

      function waitChangeDesc() {
        var t = 0;
        var iv = setInterval(function () {
          t++;
          var changeBtn = GIDOC.getElementById('changeDesc_' + useLang);
          if (changeBtn) { clearInterval(iv); try { changeBtn.click(); } catch (e) {} waitEditor(); return; }
          if (t > 50) { clearInterval(iv); rej(new Error('changeDesc_' + useLang + ' nie znalezione')); }
        }, 150);
      }
      ...
      function waitEditor() {
        ...
        var iv = setInterval(function () {
          t++;
          var eds = (GW.tinymce && GW.tinymce.get()) || [];
          for (var i = 0; i < eds.length; i++) {
            var ed = eds[i];
            if (ed && ed.id && beforeIds.indexOf(ed.id) === -1 && ed.id.indexOf('fg_node_desc_') === 0) {
              // Editor zainicjalizowany — wyciągamy
              clearInterval(iv);
              var container = ed.editorContainer;
              var ta = ed.targetElm;
              if (!container || !ta) { rej(new Error('Edytor bez container/targetElm')); return; }
              // Tymczasowy offscreen host (żeby natywny cleanup nie zabrał containera)
              var tmpHost = GIDOC.createElement('div');
              tmpHost.style.cssText = 'position:fixed;left:-99999px;top:-99999px;width:1px;height:1px;overflow:hidden;';
              GIDOC.body.appendChild(tmpHost);
              try { tmpHost.appendChild(ta); tmpHost.appendChild(container); } catch (e) {}
              // Sprzątanie natywnych dialogów
              try {
                var ld = GIDOC.getElementById('longdesc_edit_window_c'); if (ld) ld.remove();
                var fe = GIDOC.querySelector('form[id^="form_editEl_"]');
                if (fe) { var cc = fe; while (cc && cc.parentElement && cc.parentElement.tagName !== 'BODY') cc = cc.parentElement; if (cc) cc.remove(); }
                GIDOC.querySelectorAll('.yui3-widget-mask, .ui-widget-overlay').forEach(function (n) { n.remove(); });
              } catch (er) {}
              res({ editor: ed, container: container, textarea: ta, tmpHost: tmpHost });
              return;
            }
          }
          if (t > 100) { clearInterval(iv); rej(new Error('TinyMCE nie zainicjalizowany w czasie')); }
        }, 150);
      }
    });
  }
  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      var saveBtn = footer.querySelector('.tm-btn-save');
      function close() {
        // v4.6.53: zniszcz steal-native TinyMCE i wyczyść tymczasowe hosty
        try { destroyEd(edDesc, edDescIsTm); } catch (e) {}
        try { destroyEd(edList, edListIsTm); } catch (e) {}
        overlay.classList.add('tm-closing'); setTimeout(function () { if (overlay.parentNode) overlay.remove(); }, 200);
      }
      header.querySelector('.tm-settings-close').addEventListener('click', close);
      ...
      // edytory opisu (per lang) i opisu listy (per shop+lang) — TinyMCE steal-native + fallback contentEditable
      var edDesc = null, edList = null, navData = null;
      var edDescIsTm = false, edListIsTm = false; // czy stolen TinyMCE czy fallback contentEditable
      var listDescState = {}; // key 'shop@lang' -> HTML
      function getEdContent(ed, isTm) { try { return isTm ? ed.editor.getContent() : ed.getValue(); } catch (e) { return ''; } }
      function setEdContent(ed, isTm, html) { try { if (isTm) ed.editor.setContent(html || ''); else ed.setValue(html || ''); } catch (e) {} }
      function destroyEd(ed, isTm) { try { if (isTm && ed && ed.editor) { ed.editor.remove(); if (ed.tmpHost && ed.tmpHost.parentNode) ed.tmpHost.parentNode.removeChild(ed.tmpHost); } } catch (e) {} }
      ...
      function buildBody(dn) {
        ...
        var contentCard = card('edit_note', 'Treść',
          ...
          (isVal ? (rowF('Nazwa w nagłówku', mkSel('f_hlmode', [['d', 'wygeneruj automatycznie'], ['o', 'własna nazwa']], hlAuto ? 'd' : 'o', 'tune')) +
            '<div class="tm-field-row tm-field-row--stacked" id="row_hltext"' + (hlAuto ? ' style="display:none"' : '') + '><span class="tm-field-label">Nazwa w nagłówku strony</span>' + mkInp('f_headline', dn.headline_name || '', 'badge') + '</div>') : '') +
        ...
        // Header z labelem nad steal-native TinyMCE (bez relokacji paska — TinyMCE ma własny toolbar)
        function tmHeader(labelText) {
          var head = d.createElement('div'); head.className = 'tp-ed-v2head';
          var lab = d.createElement('span'); lab.className = 'tp-ed-v2lbl'; lab.textContent = labelText;
          head.appendChild(lab);
          return head;
        }
        function makeFallbackContentEd(host, labelText, html) {
          var w = createEditorWidget(d, html || '', 7);
          host.appendChild(w.container);
          relocateEdBar(w, labelText);
          return w;
        }
        var hd = bodyEl.querySelector('#host_desc');
        var hl = bodyEl.querySelector('#host_list');

        // Próbujemy steal-native dla edDesc; w razie błędu — fallback do contentEditable
        stealNativeEditor(nodeId, curLang).then(function (stolen) {
          var hdr = tmHeader('Opis (HTML)'); hd.appendChild(hdr); hd.appendChild(stolen.container);
          edDesc = stolen; edDescIsTm = true;
          try { stolen.editor.setContent(st[curLang].description || ''); } catch (e) {}
        }, function (err) {
          edDesc = makeFallbackContentEd(hd, 'Opis (HTML)', st[curLang].description); edDescIsTm = false;
          if (_panel) _panel.showStatus('Edytor opisu: użyto fallback (powód: ' + (err && err.message || err) + ')', true);
        }).then(function () {
          if (!hl) { edList = null; return; }
          // Drugi steal — dla opisu listy (osobna instancja, ten sam wzorzec)
          return stealNativeEditor(nodeId, curLang).then(function (stolen) {
            var hdr2 = tmHeader('Opis na liście towarów (HTML)'); hl.appendChild(hdr2); hl.appendChild(stolen.container);
            edList = stolen; edListIsTm = true;
            // Załaduj treść getNodeDesc dla bieżącego (shop+lang)
            return P('/panel/ajax/parameters.php', 'action=getNodeDesc&node=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(curLang) + '&shop=' + encodeURIComponent(curShop) + '&tree=parameters&parent=0').then(function (g) {
              var desc = (g && g.data && g.data.desc) || (g && g.desc) || '';
              listDescState[curShop + '@' + curLang] = desc;
              try { stolen.editor.setContent(desc); } catch (e) {}
            }, function () {});
          }, function (err) {
            edList = makeFallbackContentEd(hl, 'Opis na liście towarów (HTML)', ''); edListIsTm = false;
            if (_panel) _panel.showStatus('Edytor opisu listy: użyto fallback', true);
            return P('/panel/ajax/parameters.php', 'action=getNodeDesc&node=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(curLang) + '&shop=' + encodeURIComponent(curShop) + '&tree=parameters&parent=0')
              .then(function (g) { var desc = (g && g.data && g.data.desc) || (g && g.desc) || ''; listDescState[curShop + '@' + curLang] = desc; try { edList.setValue(desc); } catch (e) {} }, function () {});
          });
        }).then(function () { wire(); renderGfx(); }, function () { wire(); renderGfx(); });
      }
      ...
      function stashName() {
        var ni = bodyEl.querySelector('[name=f_name]'); if (ni) st[curLang].name = ni.value;
        if (edDesc) { st[curLang].description = getEdContent(edDesc, edDescIsTm); }
        if (edList) { listDescState[curShop + '@' + curLang] = getEdContent(edList, edListIsTm); }
      }
      ...
      function reload() {
        ...
        bodyEl.classList.add('tp-seo-busy');
        // v4.6.53: zniszcz steal-native edytory przed re-renderem (buildBody robi innerHTML reset)
        try { destroyEd(edDesc, edDescIsTm); } catch (e) {} edDesc = null; edDescIsTm = false;
        try { destroyEd(edList, edListIsTm); } catch (e) {} edList = null; edListIsTm = false;
      ...
      function doSave() {
        ...
        var listHtml = ''; if (edList) listHtml = getEdContent(edList, edListIsTm);
