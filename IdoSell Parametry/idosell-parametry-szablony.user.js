  ...
  function showTranslationsModal(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        // Scroll poziomy WEWNĄTRZ karty (sticky toolbar i thead zostają — scrolluje tylko zawartość gridu)
        '.tp-tr-card-body { background:#fff; overflow-x:auto; }',
    ...
    function renderTable() {
      ...
      // tak żeby input z ikoną i normalna długość nazwy mieściła się bez ucięcia.
      // Gdy łączny min > viewport → scroll poziomy WEWNĄTRZ .tp-tr-card-body.
      var gridCols = 'minmax(260px,1fr) ' + allLangs.map(function () { return 'minmax(280px,1fr)'; }).join(' ');
