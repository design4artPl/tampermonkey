  ...
  function showTranslationsModal(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        // Search box i filtr używają ISTNIEJĄCYCH klas .tp-search-box/.tp-search-input
        // (taki sam jak na głównej stronie) oraz .tm-input-wrap/.tm-field-select
        // (te same co modal ustawień). Tu tylko korekta szerokości w naszym kontekście.
        '.tp-tr-toolbar .tp-search-box { flex:1; min-width:240px; max-width:520px; }',
        '.tp-tr-toolbar .tp-search-input { width:auto !important; flex:1; }',
        '.tp-tr-toolbar .tm-input-wrap { flex:0 0 220px; }',
        ...
        '.tp-tr-cell .tm-input-wrap { width:100%; }',
    ...
    modal.innerHTML =
      ...
      '<div class="tp-tr-toolbar">' +
        // Search: te same klasy co toolbar drzewa (.tp-search-box / .tp-search-input)
        '<div class="tp-search-box"><span class="material-symbols-outlined">search</span><input type="search" class="tp-search-input tp-tr-search" placeholder="Szukaj po nazwie lub ID…"></div>' +
        '<span class="tp-tr-search-hint">Wiele: oddziel <code>|</code> lub <code>,</code></span>' +
        // Filtr: .tm-input-wrap + .tm-field-select (klasy modalu ustawień)
        '<div class="tm-input-wrap"><span class="material-symbols-outlined tm-input-icon">filter_alt</span>' +
    ...
    // STREAMING WORKER POOL. Zamiast dwufazowo (params → values) — jedna pula
    // workerów (concurrency=WORKERS). Queue startuje z paramIds; gdy param task
    // skończy się, jego valueIds są wpychane natychmiast do queue. Wartości
    // zaczynają się ładować NATYCHMIAST, nie po wszystkich parametrach.
    // Per param: Promise.all([fetchLangData, loadChildValues]) — dwa requesty
    // równolegle dla każdego parametru, oszczędzamy połowę czasu fazy 1+2.
    function loadStreaming() {
      var WORKERS = 20;
      // Queue jako tablica obiektów {kind:'param'|'value', id, parentId?}
      var queue = [];
      for (var pi = 0; pi < paramList.length; pi++) queue.push({ kind: 'param', id: paramList[pi].id });
      var inFlight = 0;
      return new Promise(function (resolveAll) {
        ...
        function pump() {
          while (inFlight < WORKERS && queue.length > 0) {
        ...
        function handle(task) {
          if (task.kind === 'param') {
            // Równolegle: fetchLangData + loadChildValues
            Promise.all([
              fetchLangData(task.id).then(function (n) { return n; }, function () { return {}; }),
              loadChildValues(task.id).then(function (k) { return k || []; }, function () { return []; })
            ]).then(function (res) {
              var names = res[0], kids = res[1];
              loaded.push({ id: task.id, type: 'param', parentId: null, names: names });
              doneCount++;
              if (kids && kids.length) {
                totalToLoad += kids.length;
                for (var j = 0; j < kids.length; j++) queue.push({ kind: 'value', id: kids[j].id, parentId: task.id });
              }
              updateProgress();
              inFlight--; pump(); tryFinish();
            });
          } else {
            fetchLangData(task.id).then(function (names) {
              loaded.push({ id: task.id, type: 'value', parentId: task.parentId, names: names });
              doneCount++; updateProgress();
              inFlight--; pump(); tryFinish();
            }, function () { doneCount++; updateProgress(); inFlight--; pump(); tryFinish(); });
          }
        }
        pump();
      });
    ...
    updateProgress();
    loadStreaming().then(function () { renderTable(); }, function (e) { renderTable(); if (_panel) _panel.showStatus('Błąd ładowania: ' + (e && e.message || e), true); });
    ...
    function renderTable() {
      ...
      // Komórki: .tm-input-wrap + .tm-node-text-input (te same klasy co modal ustawień).
      // Klasy stanu dirty/saving/saved/error nakładamy na wrap, nie na input.
      function cellHtml(id, lang, v) {
        var ev = esc(v || '');
        return '<td class="tp-tr-cell"><div class="tm-input-wrap"><input class="tm-node-text-input tp-tr-input" data-id="' + esc(id) + '" data-lang="' + esc(lang) + '" value="' + ev + '" data-orig="' + ev + '"></div></td>';
      }
      ...
      paramItems.forEach(function (p) {
        ...
        allLangs.forEach(function (lg) { html += cellHtml(p.id, lg, p.names[lg]); });
        html += '</tr>';
        (valuesByParent[p.id] || []).forEach(function (v) {
          ...
          allLangs.forEach(function (lg) { html += cellHtml(v.id, lg, v.names[lg]); });
          html += '</tr>';
        });
      });
      ...
      // Listener edycji — klasy stanu (dirty/saved/error) nakładamy na .tm-input-wrap
      bodyEl.addEventListener('input', function (ev) {
        var t = ev.target;
        if (!t.classList || !t.classList.contains('tp-tr-input')) return;
        var wrap = t.closest('.tm-input-wrap') || t.parentNode;
    ...
    saveBtn.addEventListener('click', function () {
      ...
      function saveGroup(i) {
        ...
        arr.forEach(function (it) {
          ...
          var w = it.el.closest('.tm-input-wrap') || it.el.parentNode;
        ...
        fetchAjax(body).then(function (r) {
          ...
          arr.forEach(function (it) { var w = it.el.closest('.tm-input-wrap') || it.el.parentNode; if (w) { w.classList.remove('tp-tr-saving'); w.classList.add('tp-tr-error'); } });
