  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      ok.addEventListener('click', function () {
        stash();
        // DIAGNOSTYKA v4.6.29: co realnie przechwyci\u0142 edytor (do raportu u\u017cytkownika)
        var _dCur = (st[curLang] && st[curLang].description) || '';
        var _dSample = _dCur.replace(/\s+/g, ' ').slice(0, 60);
        try {
          console.log('[parametry][opis] v4.6.29 nodeId=' + nodeId + ' curLang=' + curLang +
        ...
        try { console.log('[parametry][opis] v4.6.29 nameBody=', nameBody.slice(0, 300)); } catch (e) {}
        fetchAjax(nameBody).then(function (r1) {
          try { console.log('[parametry][opis] v4.6.29 setSettings resp=', r1); } catch (e) {}
          _diag.ss = (r1 && typeof r1.errno !== 'undefined') ? ('errno ' + r1.errno) : 'OK';
          if (r1 && r1.errno && r1.errno !== 0) settingsErr = (r1.message || ('errno ' + r1.errno));
        }, function (e) { settingsErr = (e && e.message) || String(e); _diag.ss = 'REJECT ' + settingsErr; try { console.warn('[parametry][opis] v4.6.29 setSettings REJECT', e); } catch (_) {} }).then(function () {
          ...
          function sendDesc(i) {
            ...
            try { console.log('[parametry][opis] v4.6.29 ->setDescription', lg, 'len=' + (((st[lg] && st[lg].description) || '').length), 'body=' + dBody.slice(0, 200)); } catch (e) {}
            return fetchAjax(dBody).then(function (rd) {
              try { console.log('[parametry][opis] v4.6.29 <-setDescription', lg, 'resp=', rd); } catch (e) {}
              ...
              try { console.warn('[parametry][opis] v4.6.29 setDescription REJECT', lg, e); } catch (_) {}
          ...
          return fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (chk) {
            ...
            try { console.log('[parametry][opis] v4.6.29 WERYFIKACJA', { sent: sent, server: srv, diag: _diag }); } catch (e) {}
          ...
          var _opisInfo = ' [v4.6.29 opis ' + curLang + ': ' + _dCur.length + ' zn.]';
