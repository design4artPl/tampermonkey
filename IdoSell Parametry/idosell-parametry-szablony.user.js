...
`;
      ...
      toolbar: {
        ...
        sections: [
          ...
          { label: 'Transfer', buttons: [
            { icon: 'download',    label: 'Eksport JSON', tooltip: 'Eksport sekcji (zaznaczone lub wszystkie) do JSON', variant: 'text', onClick: function () { exportSections(doc, 'json'); } },
            { icon: 'table_chart', label: 'Eksport CSV',  tooltip: 'Eksport sekcji (zaznaczone lub wszystkie) do CSV',  variant: 'text', onClick: function () { exportSections(doc, 'csv'); } },
            { icon: 'upload',      label: 'Import',       tooltip: 'Import sekcji z pliku JSON lub CSV (tworzy brakuj\u0105ce)', variant: 'text', onClick: function () { openSectionsImportPicker(doc); } }
          ] }
        ]
      },
      columns: [
  ...
  function collectSectionsForExport(doc) {
    ...
    var pool = selected.length > 0 ? selected : items.filter(function (li) { return li.style.display !== 'none'; });
  ...
  async function loadParameterProductCountsInBackground(doc) {
    ...
    for (var i = 0; i < targets.length; i += BATCH) {
      ...
      await Promise.all(batch.map(async function (t) {
        try {
          var r = await fetchAjax('action=numberOfOccurrence&id=' + encodeURIComponent(t.nid));
          ...
          if (count > 0) {
            ...
            link.addEventListener('click', function (e) {
              e.stopPropagation();
              try { window.top.location.assign('products-list.php?trait=' + encodeURIComponent(t.nid)); }
              catch (err) { window.location.assign('products-list.php?trait=' + encodeURIComponent(t.nid)); }
            });
