...
(function (root) {
  ...
  var CSS = [
    ...
    '.panel-pro__toolbar__right { display: flex; align-items: center; flex-wrap: wrap; gap: 4px; min-width: 0; flex: 0 1 auto; }',
    ...
    '.panel-pro__search .material-symbols-outlined { font-size: 22px; color: #5f6368; flex-shrink: 0; }',
    ...
    'li.panel-pro__row > ul, li.panel-pro__row > [id^="block_group"] { grid-column: 1 / -1; padding: 0 !important; margin: 0 !important; }',
  ...
  function buildPageHeader(doc) {
    ...
    const langLinksHtml = otherLangs.map(function(l) {
      return '<a class="tp-page-header__lang-link" href="javascript:void(0)" title="' + escapeHtml(getLangName(l.code)) + '">' + getLangFlag(l.code) + '</a>';
    }).join('');
    ...
    header.innerHTML =
      ...
      '<div class="tp-page-header__meta">' +
        ...
        '<span class="tp-page-header__lang">' + getLangFlag(currentLang) + ' <strong>' + getLangName(currentLang) + '</strong></span>' +
        (otherLangs.length > 0 ? '' + langLinksHtml : '') +
      '</div>';
...
`;
  ...
  // v4.5.6: Inline rename (replace label with input, save via native modal in background)
  function _saveRenameViaNativeModal(doc, nodeId, newName) {
    return new Promise(function (resolve, reject) {
      var nameEl = doc.getElementById('showMenuSub_' + nodeId);
      if (!nameEl) return reject(new Error('no nameEl'));
      nameEl.click();
      setTimeout(function () {
        var ed = doc.getElementById('editEl_' + nodeId);
        if (!ed) return reject(new Error('no editEl'));
        // Hide ajax modal container while we operate so user does not see the flash
        var ajaxEl = doc.getElementById('ajax_c');
        var prevVis = ajaxEl ? ajaxEl.style.visibility : null;
        if (ajaxEl) ajaxEl.style.visibility = 'hidden';
        ed.click();
        var attempts = 0;
        var interval = setInterval(function () {
          attempts++;
          var nameInput = doc.getElementById('fg_name');
          if (nameInput) {
            clearInterval(interval);
            nameInput.value = newName;
            var form = nameInput.form;
            var modal = form ? form.closest('.yui-panel-container') : null;
            var saveBtn = modal ? Array.prototype.slice.call(modal.querySelectorAll('input[value="Zapisz"]')).find(function (b) { return b.offsetParent !== null; }) : null;
            if (!saveBtn) {
              if (ajaxEl) ajaxEl.style.visibility = prevVis || '';
              return reject(new Error('no save button'));
            }
            saveBtn.click();
            setTimeout(function () {
              if (ajaxEl) ajaxEl.style.visibility = prevVis || '';
  ...
  function startInlineRename(doc, li, nameEl, label, nodeId) {
    ...
    input.style.cssText = 'width:100%; min-width:120px; padding:3px 6px; font-size:14px; border:1px solid #2563eb; border-radius:4px; outline:none; box-shadow:0 0 0 2px rgba(37,99,235,0.15); background:#fff; font-family:inherit; color:#202124; box-sizing:border-box;';
    ...
    function save() {
      ...
      _saveRenameViaNativeModal(doc, nodeId, newName).then(function () {
  ...
  function sortAlphabetically(doc) {
    ...
    showForceDeleteStatus(doc, 'Posortowano alfabetycznie (' + (sortAscending ? 'Z\u2192A' : 'A\u2192Z') + ')');
  }
  ...
  // v4.1.0: View templates (snapshot of selected node IDs) — saved in GM/localStorage
  var VIEW_TEMPLATES_KEY = 'tp.viewTemplates.v1';
  function loadViewTemplates() {
    try { return JSON.parse(localStorage.getItem(VIEW_TEMPLATES_KEY) || '[]') || []; } catch (e) { return []; }
  }
  function saveViewTemplates(arr) {
    try { localStorage.setItem(VIEW_TEMPLATES_KEY, JSON.stringify(arr)); } catch (e) {}
  }
  function showSaveViewTemplateModal(doc) {
    if (selectedNodes.size === 0) { alert('Najpierw zaznacz parametry/wartosci ktore maja byc w szablonie'); return; }
    var name = prompt('Nazwa szablonu widoku (' + selectedNodes.size + ' zaznaczonych elementow):');
    if (!name) return;
    var tpls = loadViewTemplates();
    tpls.push({ name: name, ids: Array.from(selectedNodes), created: Date.now() });
    saveViewTemplates(tpls);
    if (_panel) _panel.showStatus('Zapisano szablon "' + name + '"');
  }
  function showManageViewTemplatesModal(doc) {
    var tpls = loadViewTemplates();
    if (tpls.length === 0) { alert('Brak zapisanych szablonow widokow'); return; }
    var msg = 'Zapisane szablony widokow:\n\n';
    tpls.forEach(function (t, i) {
      msg += (i + 1) + '. ' + t.name + ' (' + t.ids.length + ' elementow)\n';
    });
    msg += '\nWpisz numer aby zaladowac, lub "d N" aby usunac:';
    var input = prompt(msg);
    if (!input) return;
    var del = input.match(/^\s*d\s+(\d+)\s*$/i);
    if (del) {
      var di = parseInt(del[1], 10) - 1;
      if (tpls[di]) {
        if (confirm('Usunac szablon "' + tpls[di].name + '"?')) {
          tpls.splice(di, 1);
          saveViewTemplates(tpls);
          if (_panel) _panel.showStatus('Usunieto szablon');
        }
      }
      return;
    }
    var i = parseInt(input, 10) - 1;
    if (tpls[i]) {
      // Apply: select these IDs
      deselectAll(doc);
      tpls[i].ids.forEach(function (id) {
        selectedNodes.add(id);
        var li = doc.getElementById('m_' + id);
        if (li) {
          li.classList.add('tp-row--selected');
          li.classList.add('panel-pro--selected');
          var cb = li.querySelector('.tp-checkbox');
          if (cb) cb.checked = true;
        }
      });
      updateCounter(doc);
      if (_panel) {
        _panel.setSelection(selectedNodes.size);
        _panel.showStatus('Zaladowano szablon "' + tpls[i].name + '" (' + tpls[i].ids.length + ' elementow)');
      }
    }
  }

  function applyViewMode(doc) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (!nid) continue;
      var isPar = isParameter(doc, nid);
      var isVal = isValue(doc, nid);
      var show = true;
      if (_viewMode === 'params' && !isPar) show = false;
      if (_viewMode === 'values' && !isVal) show = false;
      li.classList.toggle('tp-row--view-hidden', !show);
    }
  }
      ...
      toolbar: {
        ...
        sections: [
          ...
          { label: 'Transfer', buttons: [
            { icon: 'download', label: 'Eksport', tooltip: 'Eksport drzewa do JSON', variant: 'text', onClick: function () { exportTreeAsJson(doc); } },
            { icon: 'upload',   label: 'Import',  tooltip: 'Import drzewa z JSON',   variant: 'text', onClick: function () { alert('Import JSON \u2014 funkcja w przygotowaniu'); } }
          ] },
          { label: 'Widok', dropdown: {
            icon: 'visibility',
            options: [
              { value: 'all',    label: 'Wszystko',         active: true },
              { value: 'params', label: 'Tylko parametry' },
              { value: 'values', label: 'Tylko warto\u015bci' },
              { value: '__save_template__',   label: 'Zapisz aktualny widok jako szablon\u2026' },
              { value: '__manage_templates__', label: 'Zarz\u0105dzaj szablonami widok\u00f3w\u2026' }
            ],
            onChange: function (v) {
              if (v === '__save_template__')   { showSaveViewTemplateModal(doc); return; }
              if (v === '__manage_templates__'){ showManageViewTemplatesModal(doc); return; }
              _viewMode = v; applyViewMode(doc);
            }
          } },
  ...
  waitForIframe(function (doc) {
    ...
    updateCounter(doc);
    // Background fetch children counts (non-blocking)
