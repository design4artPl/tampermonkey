...
// @version      4.0.0
// @description  Toolbar do grupowej edycji parametrow: panel-pro widget, checkboxy, zaznaczanie, rozwijanie/zwijanie, grupowe usuwanie/edycja, import CSV
// @author       SyncOffer
// @match        https://*.iai-shop.com/panel/app/parameters.php*
// @match        https://*.idosell.com/panel/app/parameters.php*
// @require      https://raw.githubusercontent.com/design4artPl/tampermonkey/panel-pro/v1.0.0/_shared/panel-pro/panel-pro.js
// @grant        none
...
`;
  ...
  function updateCounter(doc) {
    if (_counterTimer) return; // already scheduled
      ...
      toolbar: {
        ...
        sections: [
          ...
          { label: 'Widok', dropdown: {
            ...
            onChange: function (v) { _viewMode = v; applyViewMode(doc); }
          } },
          { label: 'Drzewo', buttons: [
            { id: 'expand-all',   icon: 'unfold_more',   tooltip: 'Rozwi\u0144 wszystko', variant: 'icon', onClick: function (e, api) {
            ...
            { id: 'collapse-all', icon: 'unfold_less',   tooltip: 'Zwi\u0144 wszystko',   variant: 'icon', onClick: function () { collapseAll(doc); } },
            ...
            { id: 'sort-alpha',   icon: 'sort_by_alpha', tooltip: 'Sortuj alfabetycznie', variant: 'icon', onClick: function () { sortAlphabetically(doc); } }
          ] }
        ],
        bulkSection: {
          label: 'Edycja grupowa',
          buttons: [
            { icon: 'edit',            label: 'Edytuj',   tooltip: 'Grupowa edycja zaznaczonych',          variant: 'primary', onClick: function () { if (selectedNodes.size === 0) { alert('Najpierw zaznacz elementy!'); return; } showBulkEditModal(doc); } },
            { icon: 'drive_file_move', label: 'Przenie\u015b', tooltip: 'Przenie\u015b zaznaczone warto\u015bci', variant: 'accent',  onClick: function () { if (selectedNodes.size === 0) { alert('Najpierw zaznacz elementy!'); return; } showBulkMoveModal(doc); } },
            { icon: 'delete',          label: 'Usu\u0144', tooltip: 'Usu\u0144 zaznaczone',                  variant: 'danger',  onClick: function () { if (selectedNodes.size === 0) { alert('Najpierw zaznacz elementy!'); return; } showBulkDeleteModal(doc); } }
          ]
        }
      },
      columns: [
