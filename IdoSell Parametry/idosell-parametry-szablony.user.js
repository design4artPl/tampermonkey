...
`;
  ...
  function showTranslationsModal(doc) {
    ...
    // Wstrzyknij CSS tłumaczeń (tylko raz)
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        '.tp-tr-toolbar { display:flex; align-items:center; gap:10px; padding:10px 24px; border-bottom:1px solid #f3f4f6; background:#fafbfc; flex-wrap:wrap; }',
        '.tp-tr-search { flex:1; min-width:200px; padding:7px 12px; border:1px solid #d1d5db; border-radius:6px; font-size:13px; font-family:inherit; outline:none; }',
        '.tp-tr-search:focus { border-color:#2563eb; box-shadow:0 0 0 3px rgba(37,99,235,.1); }',
        '.tp-tr-body { flex:1; overflow:auto; padding:0; }',
        ...
        '.tp-tr-table { width:100%; border-collapse:separate; border-spacing:0; font-size:13px; }',
        '.tp-tr-table thead th { position:sticky; top:0; background:#f8fafc; padding:10px 12px; text-align:left; font-weight:600; color:#374151; font-size:12px; border-bottom:1px solid #e5e7eb; white-space:nowrap; z-index:2; }',
        '.tp-tr-table thead th.tp-tr-col-elem { min-width:240px; }',
        '.tp-tr-table tbody td { padding:6px 10px; border-bottom:1px solid #f3f4f6; vertical-align:middle; }',
        '.tp-tr-table tbody tr.tp-tr-row-param { background:#f9fafb; }',
        '.tp-tr-table tbody tr.tp-tr-row-param td:first-child { font-weight:600; color:#111827; }',
        '.tp-tr-table tbody tr.tp-tr-row-value td:first-child { padding-left:32px; color:#4b5563; }',
        '.tp-tr-elem-id { color:#9ca3af; font-size:11px; margin-left:6px; font-weight:400; }',
        '.tp-tr-input { width:100%; padding:5px 8px; border:1px solid #e5e7eb; border-radius:4px; font-size:13px; font-family:inherit; background:#fff; box-sizing:border-box; outline:none; transition:border-color .12s; }',
        '.tp-tr-input:focus { border-color:#2563eb; box-shadow:0 0 0 2px rgba(37,99,235,.12); }',
        ...
        '.tp-tr-btn { padding:8px 18px; border-radius:6px; font-size:13px; font-weight:600; cursor:pointer; border:1px solid transparent; font-family:inherit; display:inline-flex; align-items:center; gap:6px; }',
        '.tp-tr-btn-cancel { background:#fff; border-color:#d1d5db; color:#374151; }',
        '.tp-tr-btn-cancel:hover { background:#f9fafb; }',
    ...
    modal.innerHTML =
      ...
      '</div>' +
      '<div class="tp-tr-toolbar"><input type="search" class="tp-tr-search" placeholder="Szukaj parametru lub wartości po nazwie / ID…"></div>' +
    ...
    function loadValuesFor(paramItem) {
      // pobierz IDs wartości
      return loadChildValues(paramItem.id).then(function (kids) {
        if (!kids || !kids.length) return [];
        totalToLoad += kids.length;
        updateProgress();
        // Per wartość — getParameterLangData. UWAGA: Prototype.js łamie Array.reduce
        // (zweryfikowane na parameters.php) → ręczna pętla step(i).
        var acc = [];
        function step(i) {
          if (i >= kids.length) return Promise.resolve(acc);
          return fetchLangData(kids[i].id).then(function (names) {
            doneCount++; updateProgress();
            acc.push({ id: kids[i].id, type: 'value', parentId: paramItem.id, names: names });
            return step(i + 1);
          }, function () { doneCount++; updateProgress(); return step(i + 1); });
        }
        return step(0);
      });
    }
    function loadParam(p) {
      return fetchLangData(p.id).then(function (names) {
        loaded.push({ id: p.id, type: 'param', parentId: null, names: names, parentName: '' });
        doneCount++; updateProgress();
        return loadValuesFor(p).then(function (valItems) {
          // Prototype.js bezpieczne: ręczna pętla zamiast forEach
          if (valItems && valItems.length) { for (var j = 0; j < valItems.length; j++) loaded.push(valItems[j]); }
        });
      }, function () { doneCount++; updateProgress(); });
    }
    // Paralelizacja: 6 jednoczesnych. Prototype.js na parameters.php psuje Array.slice/map
    ...
    updateProgress();
    chunkAll(paramList, 6, loadParam).then(function () {
      renderTable();
    });
    ...
    function renderTable() {
      ...
      var html = '<table class="tp-tr-table"><thead><tr><th class="tp-tr-col-elem">Element</th>';
      ...
      searchEl.addEventListener('input', function () {
        ...
        // Najpierw oznacz match'ujące rzędy
        var matched = {}; // paramId or valueId -> true
        rows.forEach(function (r) {
          var id = r.dataset.id;
          var name = r.querySelector('td').textContent.toLowerCase();
          if (name.indexOf(q) >= 0 || id.indexOf(q) >= 0) matched[id] = true;
        });
        rows.forEach(function (r) {
          var show = matched[r.dataset.id];
          // Pokaż też parametr jeśli któraś z jego wartości matchuje
          if (!show && r.dataset.type === 'param') {
            var pid = r.dataset.id;
            // szukaj kolejnych value rows
            var nx = r.nextElementSibling;
            while (nx && nx.dataset && nx.dataset.type === 'value') {
              if (matched[nx.dataset.id]) { show = true; break; }
              nx = nx.nextElementSibling;
            }
          }
          // Pokaż też wartość jeśli jej parametr matchuje
          if (!show && r.dataset.type === 'value') {
            var prev = r.previousElementSibling;
            while (prev && prev.dataset && prev.dataset.type === 'value') prev = prev.previousElementSibling;
            if (prev && matched[prev.dataset.id]) show = true;
          }
