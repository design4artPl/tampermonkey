...
`;
  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function copyGfxFilesToLangs(src, others) {
        ...
        ['search', 'projector'].forEach(function (ctx) {
      ...
      function copyActiveLangToAll() {
        ...
        showConfirmModal(doc, {
          ...
          message: 'Nazwa, opis ORAZ pliki grafik (lista i karta, wszystkie wielkości) zostaną przeniesione z języka „' + escapeHtml(getLangName(src)) + '" do wszystkich pozostałych języków — dla każdego sklepu, w którym dany język jest dostępny. Istniejące grafiki w językach docelowych zostaną nadpisane.',
          okLabel: 'Powiel',
          onOk: function () {
            others.forEach(function (lg) {
            ...
            if (_panel) _panel.showStatus('Powielanie języka „' + getLangName(src) + '"… (nazwa, opis, grafiki)');
            copyGfxFilesToLangs(src, others).then(function (r) {
              loadLang(src);
              if (_panel) {
                if (r.total === 0) _panel.showStatus('Powielono nazwę i opis (brak grafik do skopiowania)');
                else if (r.fail) _panel.showStatus('Powielono. Grafiki: ' + r.ok + '/' + r.total + ' OK, ' + r.fail + ' błędów', true);
                else _panel.showStatus('Powielono język „' + getLangName(src) + '": nazwa, opis i ' + r.ok + ' grafik');
              }
            }).catch(function (e) {
              loadLang(src);
