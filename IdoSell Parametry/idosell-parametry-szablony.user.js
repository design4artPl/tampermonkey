...
`;
  ...
  async function runExport(doc, opts) {
    ...
    if (extras.redirects) {
      ...
      // v4.6.159: zachowujemy surowa liste w data.redirectsByShop (diagnostyka)
      var redirectsPerShop = {};
      data.redirectsByShop = {};
      for (var rsi = 0; rsi < redirShops.length; rsi++) {
        ...
        try {
          ...
          Object.keys(redirMap).forEach(function (id) {
            var r = redirMap[id];
            if (!r || !r.uri) return;
            var entry = { id: id, uri: r.uri, destination: r.dest || '', type: r.type || '301' };
            listRaw.push(entry);
            list.push(Object.assign({ _uriN: _norm(r.uri), _destN: _norm(r.dest || '') }, entry));
          });
          redirectsPerShop[rsh] = list;
          data.redirectsByShop[rsh] = listRaw;
        } catch (e) {
          redirectsPerShop[rsh] = [];
          data.redirectsByShop[rsh] = { error: String(e.message || e) };
        }
      }
    ...
    if (opts.format === 'json') {
      ...
      // v4.6.159: surowa lista redirectow per shop (diagnostyka — user widzi czy fetch dziala)
      if (extras.redirects && data.redirectsByShop) jsonOut.redirectsByShop = data.redirectsByShop;
