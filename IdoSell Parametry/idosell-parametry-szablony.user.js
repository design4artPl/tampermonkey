  ...
  function showTranslationsModal(doc) {
    ...
    function loadValuesFor(paramItem) {
      ...
      return loadChildValues(paramItem.id).then(function (kids) {
        ...
        updateProgress();
        // Per wartość — getParameterLangData
        return kids.reduce(function (chain, kid) {
          return chain.then(function (acc) {
            return fetchLangData(kid.id).then(function (names) {
              doneCount++; updateProgress();
              acc.push({ id: kid.id, type: 'value', parentId: paramItem.id, names: names });
              return acc;
            }, function () { doneCount++; updateProgress(); return acc; });
          });
        }, Promise.resolve([]));
      });
    }
    function loadParam(p) {
      return fetchLangData(p.id).then(function (names) {
        ...
        doneCount++; updateProgress();
        return loadValuesFor(p).then(function (valItems) { valItems.forEach(function (v) { loaded.push(v); }); });
      }, function () { doneCount++; updateProgress(); });
    }
    // Paralelizacja: 6 jednoczesnych
    function chunkAll(items, concurrency, fn) {
      var i = 0;
      function next() {
        if (i >= items.length) return Promise.resolve();
        var batch = items.slice(i, i + concurrency); i += concurrency;
