...
.tp-col-drag {
  ...
  /* v4.6.103: kolumna zwężona do 0 (checkbox przy lewej krawędzi) —
     uchwyt ⠿ przy wartościach chowany przez overflow */
...
`;
  ...
  async function renderValueFiltersEditor(host, doc, nodeId, shop, lang) {
    ...
    function buildHtml() {
      ...
      html += '</div>';
      html += '<div class="tp-fv-actions"><button type="button" class="tp-fv-save">Zapisz filtry</button></div>';
      html += '</div>';
      return html;
    }
    ...
    var saveBtn = host.querySelector('.tp-fv-save');
    ...
    saveBtn.addEventListener('click', async function () {
      ...
      try {
        if (mode === 'default') {
          await saveValueFiltersDefault(nodeId, shop, lang);
        } else {
          var ids = [];
          for (var k = 0; k < stateActive.length; k++) ids.push(stateActive[k].id);
          var ps = (psInput && psInput.value) || '10';
          await saveValueFilters(nodeId, shop, lang, ids, ps, ps);
        }
        if (_panel) _panel.showStatus('Zapisano filtry');
        saveBtn.textContent = 'Zapisz filtry'; saveBtn.disabled = false;
      } catch (e) {
        alert('Błąd zapisu filtrów: ' + (e && e.message || e));
        saveBtn.textContent = 'Zapisz filtry'; saveBtn.disabled = false;
      }
    });
  }
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function buildBody(dn) {
        ...
        if (isVal) {
          ...
          if (hf) renderValueFiltersEditor(hf, d, nodeId, curShop, curLang);
        }
      }
