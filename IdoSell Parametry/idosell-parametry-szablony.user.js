  ...
  function showEditElementModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function mkUploadRow(labelTxt, cx, sh, kind) {
        ...
        btn.addEventListener('click', function () {
          var lg = curLang;
          var span = nativeSpanFor(lg, cx, sh, kind);
          if (!span) {
            stt.textContent = 'błąd — brak pola natywnego';
            alert('Nie znaleziono natywnego pola wgrywania dla tego sklepu/wariantu. Zamknij i otwórz ponownie edycję.');
            return;
          }
          var imgId = 'img_' + cx + '_' + nodeId + '_' + lg + '_' + sh;
          var imEl = GIDOC.getElementById(imgId);
          var before = imEl ? (imEl.getAttribute('src') || '') : '';
          stt.textContent = 'Wybierz plik w oknie systemowym…';
          // Klikamy ORYGINALNY natywny span — jego onclick (IAI.PictureUploader.select(this,…))
          // otwiera systemowy wybór pliku i zapisuje grafikę od razu po stronie panelu.
          try { span.click(); } catch (e) { stt.textContent = 'błąd'; alert('Błąd: ' + (e.message || e)); return; }
          var t0 = Date.now();
          var pv = setInterval(function () {
            var im2 = GIDOC.getElementById(imgId);
            var now = im2 ? (im2.getAttribute('src') || '') : '';
            if (now && now !== before) {
              clearInterval(pv);
              stt.textContent = 'wgrano i zapisano ✓';
              if (gfxData[lg]) {
                gfxData[lg].img[sh] = gfxData[lg].img[sh] || {};
                gfxData[lg].img[sh][cx] = now + (now.indexOf('?') > -1 ? '&' : '?') + 'tpv=' + Date.now();
              }
              gfxEnable[lg] = gfxEnable[lg] || {};
              gfxEnable[lg][sh] = gfxEnable[lg][sh] || {};
              gfxEnable[lg][sh][cx] = 'y_manual';
              if (_panel) _panel.showStatus('Wgrano grafikę (zapisane w panelu)');
              if (lg === curLang) renderGfxBody(sh);
            } else if (Date.now() - t0 > 90000) {
              clearInterval(pv);
              stt.textContent = 'anulowano lub brak zmiany';
            }
          }, 600);
        });
      ...
      (function initNativeGfx() {
        ...
        function finish(ok) {
          // NIE niszczymy natywnego formularza — zostaje ŻYWY offscreen (klasa
          // .tp-gfx-killed + styl tp-gfx-hide). killIv pracuje dalej, więc każde
          // okno uploadu IAI.PictureUploader też ląduje offscreen. Sprzątanie
          // dopiero przy zamknięciu modalu (cleanupNativeGfx w close()).
