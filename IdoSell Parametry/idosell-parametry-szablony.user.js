  ...
  function showTranslationsPage(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        '.tp-tr-cell-text { flex:1 1 auto; padding:8px 10px; font-size:13px; color:#1a1a2e; min-width:0; line-height:1.4; cursor:text; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }',
    ...
    // v4.6.75: CACHE per-param w localStorage. TTL 1h. Klucz: tp-tr-cache-v1:<paramId>.
    // Zawiera {ts, paramNames: {lang: name}, pvByLang: {lang: [{id, name}]}}.
    var TR_CACHE_TTL = 60 * 60 * 1000; // 1h
    ...
    function trCacheLoad(paramId) {
      try {
        ...
        if (Date.now() - obj.ts > TR_CACHE_TTL) return null;
        return obj;
      } catch (e) { return null; }
    }
    function trCacheSave(paramId, paramNames, pvByLang) {
      ...
      } catch (e) { /* quota exceeded — ignoruj */ }
    }
    ...
    function loadBatched() {
      var paramNames = {};
      // v4.6.75: spróbuj wczytać cached paramy — jeśli mamy w cache aktualne dane,
      // pomiń fetch dla nich (mocno przyspiesza kolejne otwarcia w sesji).
      var cachedParams = {}; // paramId -> {paramNames, pvByLang}
      var paramsNeedFetch = [];
      for (var ci = 0; ci < paramList.length; ci++) {
        var cp = paramList[ci];
        var cached = trCacheLoad(cp.id);
        if (cached && cached.paramNames && Object.keys(cached.paramNames).length) {
          cachedParams[cp.id] = cached;
          paramNames[cp.id] = cached.paramNames;
        } else {
          paramsNeedFetch.push(cp);
        }
      }
      var hitCount = Object.keys(cachedParams).length;
      if (hitCount > 0 && _panel && _panel.showStatus) _panel.showStatus('Cache: ' + hitCount + '/' + paramList.length + ' parametrów wczytanych z pamięci podręcznej');

      if (ptextEl) ptextEl.textContent = 'Faza 1/2: wczytywanie nazw parametrów… (' + paramsNeedFetch.length + ' do pobrania, ' + hitCount + ' z cache)';
      totalToLoad = paramsNeedFetch.length;
      doneCount = 0;
      updateProgress();
      return chunkAll(paramsNeedFetch, 30, function (p) {
