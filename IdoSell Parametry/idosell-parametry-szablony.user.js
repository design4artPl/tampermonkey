  ...
  function showTranslationsPage(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        '.tp-tr-page { display:flex; flex-direction:column; background:#fff; font-family:"DM Sans",system-ui,-apple-system,"Segoe UI",Roboto,sans-serif; animation:tp-tr-fadeIn .12s ease-out; }',
    ...
    // Schowaj kartę drzewa (.panel-pro w iframe doc) i wstaw nasz widok w jego miejsce.
    var treeCard = d.querySelector('.panel-pro');
    if (!treeCard) { alert('Nie znaleziono karty drzewa — przeładuj stronę.'); return; }
    var prevTreeDisplay = treeCard.style.display || '';
    ...
    modal.innerHTML =
      ...
      '<div class="tp-tr-footer">' +
        ...
        '</div>' +
      '</div>';
    // Wstawiamy stronę w miejsce karty drzewa (która jest schowana).
    treeCard.parentNode.insertBefore(modal, treeCard);
    ...
    function close() {
      try { treeCard.style.display = prevTreeDisplay; } catch (e) {}
      ...
      opsBar: {
        right: {
          ...
          buttons: [
            ...
            { icon: 'translate',     label: 'Tłumaczenia',          tooltip: 'Edytuj nazwy parametrów i wartości we wszystkich językach', variant: 'text', onClick: function () { showTranslationsPage(doc); } },
