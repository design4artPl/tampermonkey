...
(function (root) {
  ...
  // v4.5.26: fetch full list of sections via API — IdoSell has type=parameter/section/value
  async function getAllSections(doc) {
    try {
      ...
      var sections = resp.data.map(function (s) {
        return { id: String(s.id), name: String(s.name || '').trim(), productCount: 0, products: [] };
      });
      var BATCH = 5;
      for (var i = 0; i < sections.length; i += BATCH) {
        ...
        await Promise.all(batch.map(async function (s) {
          try {
            var r = await fetchAjax('action=numberOfOccurrence&id=' + encodeURIComponent(s.id));
            if (r && r.data) {
              s.productCount = Number(r.data.numberOfProduct || 0);
              s.products = Array.isArray(r.data.products) ? r.data.products : [];
            }
          } catch (e) {}
        }));
      }
...
`;
  ...
  async function mountSectionsPanel(doc) {
    ...
    var sections = await getAllSections(doc);
    ...
    sections.forEach(function (sec) {
      ...
      li.innerHTML =
        ...
        '<div></div>' +
        '<div style="display:flex;align-items:center;gap:6px"><span class="material-symbols-outlined" style="color:#1d4ed8;font-size:20px">folder_special</span><span class="tp-sec-name" style="font-weight:500">' + escapeHtml(sec.name) + '</span></div>' +
        '<div style="text-align:center;font-family:Roboto Mono,monospace;font-size:12px;color:#5f6368">' + sec.id + '</div>' +
        '<div style="text-align:center">' + sec.productCount + '</div>' +
      ...
      li.querySelectorAll('[data-act]').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
          ...
          if (act === 'products') {
            ...
            if (!confirm('Usun\u0105\u0107 sekcj\u0119 "' + sec.name + '"? Czynno\u015b\u0107 nieodwracalna.')) return;
            (async function () {
              var attempts = [
                'action=removeParam&id=' + encodeURIComponent(sec.id) + '&type=section',
                'action=removeParam&id=' + encodeURIComponent(sec.id),
                'action=delete&id=' + encodeURIComponent(sec.id)
              ];
              var lastErr = null;
              for (var i = 0; i < attempts.length; i++) {
                try {
                  var resp = await fetchAjax(attempts[i]);
                  if (resp && resp.errno && resp.errno !== 0) { lastErr = resp.message || ('errno ' + resp.errno); continue; }
                } catch (e) { lastErr = e.message || String(e); continue; }
                try {
                  var check = await fetchAjax('action=getList&type=section&parent=0&lang=' + LANG);
                  var still = check && check.data && check.data.some(function (x) { return String(x.id) === String(sec.id); });
                  if (!still) {
                    if (_panel) _panel.showStatus('Usuni\u0119to sekcj\u0119 "' + sec.name + '"');
                    refreshSectionsPanel(doc);
                    return;
                  }
                } catch (e) {}
              }
              alert('Nie uda\u0142o si\u0119 usun\u0105\u0107 sekcji.\n\nMo\u017cliwe przyczyny:\n\u2022 Sekcja u\u017cywana w produktach \u2014 usu\u0144 j\u0105 najpierw z kart towar\u00f3w\n\u2022 IdoSell nie wspiera globalnego usuwania sekcji przez API parameters.php\n\n' + (lastErr ? 'Ostatni b\u0142\u0105d: ' + lastErr : ''));
            })();
          }
        });
      });

      // Dblclick on name = inline rename (like parameters)
  ...
  function showSectionProductsModal(doc, sec) {
    if (!sec.products || sec.products.length === 0) {
      alert('Sekcja "' + sec.name + '" (ID: ' + sec.id + ') nie jest przypisana do \u017cadnego produktu.');
      return;
    }
