  ...
  function showTranslationsPage(doc) {
    ...
    if (!d.getElementById('tp-tr-css')) {
      ...
      st.textContent = [
        ...
        // VIRTUAL SCROLL NATYWNY (CSS content-visibility): browser pomija rendering
        // wierszy poza viewportem, zachowując zarezerwowane miejsce (contain-intrinsic-size).
        // Eliminuje lag/crash przy 1000+ rzędach z inputami w komórkach.
        '.tp-tr-grid li.panel-pro__row, .tp-tr-grid li.tp-tr-row-loading { content-visibility:auto; contain-intrinsic-size:auto 50px; }',
    ...
    function buildValueRecords(paramId, byLang, langsArr) {
      ...
      for (var li = 0; li < langsArr.length; li++) {
        var langKids = byLang[langsArr[li]] || [];
        for (var ki = 0; ki < langKids.length; ki++) {
          if (!seenIds[langKids[ki].id]) { seenIds[langKids[ki].id] = true; orderedIds.push(langKids[ki].id); }
        }
      }
      var out = [];
      for (var oi = 0; oi < orderedIds.length; oi++) {
        ...
        for (var lj = 0; lj < langsArr.length; lj++) {
          var arr2 = byLang[langsArr[lj]] || [];
          var found = null;
          for (var fi = 0; fi < arr2.length; fi++) { if (arr2[fi].id === vid) { found = arr2[fi]; break; } }
          nmsByLang[langsArr[lj]] = found ? found.name : '';
        }
    ...
    function insertValuesUnderParam(paramId, values) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        for (var lj = 0; lj < visibleLangs.length; lj++) {
          ...
          rowsHtml += '<div class="tp-tr-cell" data-id="' + esc(v.id) + '" data-lang="' + esc(lg) + '">' +
            '<span class="tp-tr-cell-check" title="Zaznacz komórkę do skopiowania"></span>' +
            '<div class="tm-input-wrap"><span class="material-symbols-outlined tm-input-icon">title</span><input class="tm-node-text-input tp-tr-input" data-id="' + esc(v.id) + '" data-lang="' + esc(lg) + '" value="' + cv + '" data-orig="' + cv + '"></div>' +
          '</div>';
        }
        rowsHtml += '</li>';
      }
    ...
    function loadBatched() {
      ...
      return chunkAll(paramList, 30, function (p) {
        ...
        renderTable(); // user widzi paramy + placeholdery „Wczytywanie wartości…"

        // Faza B: per (param, lang). Po wszystkich langs danego paramu — insert do DOM.
        var tasks = [];
        for (var i2 = 0; i2 < paramList.length; i2++) {
          for (var j2 = 0; j2 < langsArr.length; j2++) tasks.push({ paramId: paramList[i2].id, lang: langsArr[j2] });
        }
        if (ptextEl) ptextEl.textContent = 'Faza 2/2: wczytywanie wartości we wszystkich językach…';
        ...
        var pvByLang = {};
        var paramLangsRem = {};
        for (var ki = 0; ki < paramList.length; ki++) paramLangsRem[paramList[ki].id] = langsArr.length;
