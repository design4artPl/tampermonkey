...
`;
  ...
  async function saveValueFilters(nodeId, shop, lang, activeIds, pricestep, pricestepnet, perOpts) {
    ...
    // v4.6.166: diagnostyka custom_name — co user wpisal i co bedzie wyslane
    try {
      var diagCustomNames = {};
      if (perOpts) for (var fdk in perOpts) if (perOpts[fdk] && perOpts[fdk].custom_name) diagCustomNames[fdk] = perOpts[fdk].custom_name;
      console.log('[Parametry PRO] custom_name do zapisania:', diagCustomNames);
      console.log('[Parametry PRO] payload custom_name format:', 'filter_' + shop + '_active_name[<filterId>]=<name>');
    } catch (e) {}
    ...
    // v4.6.166: po zapisie sprawdz getNode response → czy filter_name jest zapisane
    try {
      ...
      var nameRelated = {};
      Object.keys(dChk).forEach(function (k) { if (/name|filter/i.test(k)) nameRelated[k] = dChk[k]; });
      console.log('[Parametry PRO] po zapisie custom_name — pola name/filter w getNode:', nameRelated);
    } catch (e) {}
    return resp;
  }
