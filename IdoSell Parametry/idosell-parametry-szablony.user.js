...
`;
  ...
  async function saveValueFilters(nodeId, shop, lang, activeIds, pricestep, pricestepnet, perOpts) {
    ...
    // v4.6.163: diagnostyka — pricestep/net otrzymane jako args
    try { console.log('[Parametry PRO] saveValueFilters args:', { nodeId: nodeId, shop: shop, lang: lang, pricestep: pricestep, pricestepnet: pricestepnet }); } catch (e) {}
    ...
    try { console.log('[Parametry PRO] setNodeFilters body:', body); } catch (e) {}
    var resp = await fetchAjax(body);
    try { console.log('[Parametry PRO] setNodeFilters response:', resp); } catch (e) {}
    // v4.6.163: diagnostyka — po zapisie pobierz node i pokaz co backend zwraca dla pricestep/net
    try {
      var rChk = await fetchAjax('action=getNode&node_id=' + encodeURIComponent(nodeId) + '&lang=' + encodeURIComponent(lang) + '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0');
      var dChk = (rChk && rChk.data) || {};
      // v4.6.164: rozwiniete klucze + wartosci + caly d.filter obj
      var filterKeys = Object.keys(dChk).filter(function (k) { return k.indexOf('filter_') === 0 || k === 'filter'; });
      var filterValues = {};
      filterKeys.forEach(function (k) { filterValues[k] = dChk[k]; });
      console.log('[Parametry PRO] getNode po zapisie — pricestep/net z backend:', {
        filter_pricestep: dChk.filter_pricestep,
        filter_pricestepnet: dChk.filter_pricestepnet,
        wszystkie_pola_filter: filterValues
      });
      // Plus szukaj pricestepnet w INNYCH polach (moze pod inna nazwa)
      var allKeys = Object.keys(dChk).filter(function (k) { return /step|net|price/i.test(k); });
      var matchVals = {};
      allKeys.forEach(function (k) { matchVals[k] = dChk[k]; });
      console.log('[Parametry PRO] wszystkie pola z step/net/price:', matchVals);
    } catch (e) {}
    return resp;
  }
