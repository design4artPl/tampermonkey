...
`;
  ...
  function showTranslationsPage(doc) {
    ...
    // v4.6.77: defer (setTimeout 0) — JSON.stringify + localStorage.setItem to
    // synchroniczne i blokują wątek; defer pozwala UI „odetchnąć" pomiędzy.
    function trCacheSave(paramId, paramNames, pvByLang) {
      setTimeout(function () {
        try { localStorage.setItem(trCacheKey(paramId), JSON.stringify({ ts: Date.now(), paramNames: paramNames || {}, pvByLang: pvByLang || {} })); } catch (e) {}
      }, 0);
    }
