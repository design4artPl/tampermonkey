...
`;
  ...
  async function renderValueFiltersEditor(host, doc, nodeId, shop, lang) {
    ...
    function badgeFor(filterId) {
      ...
      return bits.length ? '<span class="tp-fv-badge">' + escapeHtml('(' + bits.join(', ') + ')') + '</span>' : '';
    }
    function rowHtml(f, idx, isActive) {
      var rmOrAdd = isActive
        ? '<button type="button" class="tp-fv-btn-rm" title="Usuń">×</button>'
        : '<button type="button" class="tp-fv-btn-add" title="Dodaj">+</button>';
      return '<div class="tp-fv-row" data-idx="' + idx + '">' +
        '<span class="tp-fv-name">' + escapeHtml(f.name) + '</span>' +
        badgeFor(f.id) +
        '<button type="button" class="tp-fv-btn-edit" title="Edytuj nazwę">' +
    ...
    function renderLists() {
      ...
      function wire(listEl, arr, otherArr) {
        var rows = listEl.querySelectorAll('.tp-fv-row');
        for (var k = 0; k < rows.length; k++) (function (row) {
          var idx = parseInt(row.getAttribute('data-idx'), 10);
          var f = arr[idx];
          var rmAdd = row.querySelector('.tp-fv-btn-rm, .tp-fv-btn-add');
          var editBtn = row.querySelector('.tp-fv-btn-edit');
