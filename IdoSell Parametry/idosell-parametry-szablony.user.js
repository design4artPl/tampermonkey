...
`;
  ...
  function enhanceRow(li, doc) {
    ...
    // v4.6.88: kolejność pierwszych 3 cells różni się per typ żeby folder/checkbox
    // alignowały się między parametrem a wartością:
    //   Param:    [checkbox slot 1][folder slot 2][drag empty slot 3][name slot 4]…
    //   Wartość:  [drag-icon slot 1][checkbox slot 2][arrow slot 3][name slot 4]…
    // Wynik: folder param (slot 2) i checkbox wartości (slot 2) w tej samej kolumnie X.
    //        Checkbox param (slot 1) i drag wartości (slot 1) — też w spójnej.
    // columnsMenu.toggleable nie zawiera drag/check/expand, więc nth-child mechanizm
    // nie jest psowany.
    if (isPar) {
      li.insertBefore(dragCell, li.firstChild);     // slot 3 (empty)
      li.insertBefore(expandCell, li.firstChild);   // slot 2 (folder)
      li.insertBefore(checkCell, li.firstChild);    // slot 1 (checkbox)
    } else {
      li.insertBefore(expandCell, li.firstChild);   // slot 3 (arrow)
      li.insertBefore(checkCell, li.firstChild);    // slot 2 (checkbox)
      li.insertBefore(dragCell, li.firstChild);     // slot 1 (drag-icon)
    }
  }
