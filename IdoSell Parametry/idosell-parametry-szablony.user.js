...
`;
  ...
  async function saveValueFilters(nodeId, shop, lang, activeIds, pricestep, pricestepnet, perOpts) {
    ...
    // v4.6.124: per-filter custom name (jesli ustawiony)
    if (perOpts) {
      for (var fid in perOpts) if (perOpts[fid] && perOpts[fid].custom_name) {
        body += '&filter_' + shop + '_active_name[' + encodeURIComponent(fid) + ']=' + encodeURIComponent(perOpts[fid].custom_name);
      }
    }
  ...
  async function renderValueFiltersEditor(host, doc, nodeId, shop, lang) {
    ...
    // v4.6.124: per-filter options (custom_name + dalsze opcje w nastepnych iteracjach)
    // Ladujemy z getNode response: d.filter_name, d.filter_display, d.filter_default_enabled
    var perOpts = {};
    if (d.filter_name && typeof d.filter_name === 'object') {
      for (var fn in d.filter_name) if (d.filter_name[fn]) {
        if (!perOpts[fn]) perOpts[fn] = {};
        perOpts[fn].custom_name = d.filter_name[fn];
      }
    }
      ...
      function wire(listEl, arr, otherArr) {
        ...
        for (var k = 0; k < rows.length; k++) (function (row) {
          ...
          if (editBtn) editBtn.addEventListener('click', function () {
            openFilterNameDialog(doc, f.id, f.name, (perOpts[f.id] && perOpts[f.id].custom_name) || '', function (newName) {
              if (!perOpts[f.id]) perOpts[f.id] = {};
              perOpts[f.id].custom_name = newName;
              renderLists();
            });
          });
        })(rows[k]);
    ...
    return {
      ...
      // v4.6.124: per-filter options (custom_name, dalsze opcje w nastepnych iteracjach)
      getPerOpts: function () { return perOpts; }
    };
  }

  // v4.6.124: prosty dialog do edycji nazwy filtra (na poczatek tylko custom_name).
  // Pelne ustawienia (display dla Marka/Seria, default_enabled dla stocks/promotion,
  // pricerange per waluta) — kolejne iteracje (v4.6.125, 126).
  function openFilterNameDialog(doc, filterId, defaultName, currentCustom, onSave) {
    var d = doc || document;
    var overlay = d.createElement('div');
    overlay.className = 'tm-node-modal-overlay tm-overlay tm-settings-v2';
    ...
    var modal = d.createElement('div');
    modal.className = 'tm-modal-content';
    modal.style.cssText = 'max-width:520px;width:520px';
    var hasCustom = !!currentCustom;
    modal.innerHTML =
      '<div class="tm-modal-header">' +
        '<div class="tm-settings-header-left">' +
          '<div class="tm-settings-header-icon"><span class="material-symbols-outlined">edit</span></div>' +
          '<div class="tm-settings-header-text">' +
            '<div class="tm-settings-header-title">Edycja filtra</div>' +
            '<div class="tm-settings-header-subtitle">' + escapeHtml(defaultName) + '</div>' +
          '</div>' +
        '</div>' +
        '<button type="button" class="tm-modal-close" title="Zamknij"><span class="material-symbols-outlined">close</span></button>' +
      '</div>' +
      '<div class="tm-modal-body" style="padding:20px 24px">' +
        '<div class="tm-field-label" style="margin-bottom:8px">Nazwa filtra na stronie</div>' +
        '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:6px 0">' +
          '<input type="radio" name="tpfn_' + filterId + '" value="d"' + (hasCustom ? '' : ' checked') + '> Domyślna' +
          '<span style="color:#667085">(' + escapeHtml(defaultName) + ')</span>' +
        '</label>' +
        '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:6px 0">' +
          '<input type="radio" name="tpfn_' + filterId + '" value="o"' + (hasCustom ? ' checked' : '') + '> Własna' +
        '</label>' +
        '<div class="tm-input-wrap" style="margin-top:6px">' +
          '<span class="material-symbols-outlined tm-input-icon">edit</span>' +
          '<input type="text" class="tm-node-text-input tpfn-custom" placeholder="Wpisz własną nazwę filtra…" value="' + escapeHtml(currentCustom || '') + '">' +
        '</div>' +
      '</div>' +
      '<div class="tm-modal-footer" style="display:flex;gap:8px;justify-content:flex-end;padding:14px 20px;border-top:1px solid #e4e7ec">' +
        '<button type="button" class="tm-btn tm-btn-ghost tpfn-cancel">Anuluj</button>' +
        '<button type="button" class="tm-btn tm-btn-primary tpfn-save">Zapisz</button>' +
      '</div>';
    overlay.appendChild(modal);
    d.body.appendChild(overlay);
    var input = modal.querySelector('.tpfn-custom');
    var radioD = modal.querySelector('input[name="tpfn_' + filterId + '"][value="d"]');
    var radioO = modal.querySelector('input[name="tpfn_' + filterId + '"][value="o"]');
    input.addEventListener('input', function () { if (input.value.trim()) { radioO.checked = true; } });
    input.addEventListener('focus', function () { radioO.checked = true; });
    function close() { overlay.remove(); }
    modal.querySelector('.tm-modal-close').addEventListener('click', close);
    modal.querySelector('.tpfn-cancel').addEventListener('click', close);
    overlay.addEventListener('click', function (e) { if (e.target === overlay) close(); });
    modal.querySelector('.tpfn-save').addEventListener('click', function () {
      var useCustom = radioO.checked && input.value.trim();
      var val = useCustom ? input.value.trim() : '';
      if (onSave) onSave(val);
      close();
    });
    setTimeout(function () { if (hasCustom) input.focus(); }, 50);
  }
