...
`;
  ...
  function showExportModal(targetDoc, defaultScope, kind) {
    ...
    var stateExtras = isSec ? [
      ...
      { key: 'icon_files', label: 'Pliki grafik (ikony search/projector)', desc: 'Eksportuje URLe ikon per shop+lang (oraz RWD desktop/tablet/mobile)', on: false, mode: 'url' }
    ];
    var savedApiKey = '';
  ...
  async function runExport(doc, opts) {
    ...
    if (extras.redirects) {
      ...
      // 2) Per element x (shop, lang): pobierz seolink (jesli shopSettings off — przez getNode),
      //    znajdz pasujace redirecty i zapisz w element.shopSettings[shop][lang].redirects
      var allElements = [];
      data.parameters.forEach(function (p) {
      ...
      allElements.forEach(function (el) {
        redirShops.forEach(function (sh) {
          opts.langs.forEach(function (lg) {
            matchJobs.push({ el: el, shop: sh, lang: lg });
          });
        });
      });

      var BATCH_RM = 10;
      for (var rmi = 0; rmi < matchJobs.length; rmi += BATCH_RM) {
        var batchRM = matchJobs.slice(rmi, rmi + BATCH_RM);
        await Promise.all(batchRM.map(async function (job) {
          try {
            // Pobierz seolink (z shopSettings cache lub osobnym getNode)
            var seo = null;
            if (extras.shopSettings && job.el.shopSettings[job.shop] && job.el.shopSettings[job.shop][job.lang]) {
              seo = job.el.shopSettings[job.shop][job.lang].seolink;
            } else {
              var rsn = await fetchAjax('action=getNode&node_id=' + encodeURIComponent(job.el.id) + '&lang=' + encodeURIComponent(job.lang) + '&shop=' + encodeURIComponent(job.shop) + '&tree=parameters&parent=0');
              var dsn = (rsn && rsn.data) || {};
              seo = (dsn.seolink && dsn.seolink[0]) || null;
            }
            if (!seo) return;
            var seoN = _norm(seo);
            // Znajdz pasujace redirecty
            var matches = (redirectsPerShop[job.shop] || []).filter(function (r) {
              return r._uriN === seoN || r._destN === seoN;
            }).map(function (r) {
              return { id: r.id, uri: r.uri, destination: r.destination, type: r.type };
            });
            if (matches.length) {
              if (!job.el.shopSettings[job.shop]) job.el.shopSettings[job.shop] = {};
              if (!job.el.shopSettings[job.shop][job.lang]) job.el.shopSettings[job.shop][job.lang] = {};
              job.el.shopSettings[job.shop][job.lang].seolink = seo;
              job.el.shopSettings[job.shop][job.lang].redirects = matches;
            }
          } catch (e) {}
        }));
        if (_panel) _panel.showStatus('Eksport: dopasowywanie przekierowań (' + Math.min(rmi + BATCH_RM, matchJobs.length) + '/' + matchJobs.length + ')');
      }
    }

    // v4.6.147: pliki grafik — URLe ikon per shop+lang (search/projector + RWD desktop/tablet/mobile)
    if (extras.icon_files) {
      var gfxTargets = [];
      data.parameters.forEach(function (p) {
        p.icons = {};
      ...
      var ICON_KEYS = ['icon_search', 'icon_search_desktop', 'icon_search_tablet', 'icon_search_mobile', 'icon_projector', 'icon_projector_desktop', 'icon_projector_tablet', 'icon_projector_mobile'];
      function _absUrl(u) {
        if (!u) return '';
        u = String(u);
    ...
    if (extras.shopSettings) {
      ...
      data.parameters.forEach(function (p) {
        p.shopSettings = {};
        ssTargets.push({ ref: p, id: p.id });
        p.values.forEach(function (v) {
          v.shopSettings = {};
          ssTargets.push({ ref: v, id: v.id });
        });
      });
      var ssTasks = [];
        ...
        await Promise.all(batchSS.map(async function (job) {
          try {
            ...
            try {
              ...
              var rdj; try { rdj = JSON.parse(rd); } catch (e) { rdj = {}; }
              ss.node_desc = (rdj && rdj.data && (rdj.data.desc || rdj.data.description || rdj.data.value)) || null;
            } catch (e) {}
    ...
    function buildNode(n, isValue) {
      ...
      // v4.6.152: shopSettings eksportowane gdy shopSettings lub redirects on (bo redirecty siedzia tam)
      if ((extras.shopSettings || extras.redirects) && n.shopSettings) obj.shopSettings = n.shopSettings;
      if (extras.icon_files && n.icons) obj.icons = n.icons;
      if (extras.products) {
        if (n._productCodes) {
    ...
    if (opts.format === 'json') {
      ...
      var jsonOut = { exportedAt: new Date().toISOString(), langs: data.langs, extras: extras, parameters: jsonParams };
      ...
      if (anyTexts) {
        try {
          ...
          if (what.texts.listDesc) {
            // Pobierz opis listy ze srcShop, zapisz dla tgt
            ...
            var rdj; try { rdj = JSON.parse(rDesc); } catch (e) { rdj = {}; }
            var srcListHtml = (rdj && rdj.data && (rdj.data.desc || rdj.data.description || rdj.data.value)) || '';
          ...
          (async function () {
            ...
            if (opts.listDesc) {
              try {
                ...
                var rdj; try { rdj = JSON.parse(rDs); } catch (e) { rdj = {}; }
                srcListDesc = (rdj && rdj.data && (rdj.data.desc || rdj.data.description || rdj.data.value)) || '';
