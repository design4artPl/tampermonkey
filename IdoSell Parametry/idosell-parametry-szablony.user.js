...
`;
  ...
  async function runExport(doc, opts) {
    ...
    // v4.6.151: przekierowania 301 per sklep — parsuje HTML strony config-system-services.php
    if (extras.redirects) {
      data.redirects = {};
      var redirShops;
      ...
      for (var rsi = 0; rsi < redirShops.length; rsi++) {
        ...
        if (_panel) _panel.showStatus('Eksport: przekierowania (sklep ' + (rsi + 1) + '/' + redirShops.length + ')');
        try {
          ...
          Object.keys(redirMap).forEach(function (id) {
            var r = redirMap[id];
            if (r && r.uri) list.push({ id: id, uri: r.uri, destination: r.dest || '', type: r.type || '301' });
          });
          data.redirects[rsh] = list;
        } catch (e) {
          data.redirects[rsh] = { error: String(e.message || e) };
        }
      }
    }
    ...
    function buildNode(n, isValue) {
      ...
      if (extras.shopSettings && n.shopSettings) obj.shopSettings = n.shopSettings;
    ...
    if (opts.format === 'json') {
      ...
      // v4.6.151: redirects per shop (top-level, niezalezne od parametrow)
      if (extras.redirects && data.redirects) jsonOut.redirects = data.redirects;
