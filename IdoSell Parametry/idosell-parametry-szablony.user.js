  ...
  function showValueSeoModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function subtitleHtml() {
        return 'ID: <span class="tp-seo-copyid" title="Kliknij, aby skopiować ID" style="cursor:pointer;border-bottom:1px dashed rgba(0,0,0,.35);">' + esc(nodeId) + '</span>' +
          ' · wartość parametru „' + esc(elName) + '"' +
          ' · Sklep: ' + esc(shopLabel(curShop)) +
          ' · Język: ' + esc(getLangName(curLang));
      }
      var header = d.createElement('div');
      ...
      header.addEventListener('click', function (ev) {
        ...
        function done() {
          ...
          setTimeout(function () { t.textContent = prev; }, 1100);
