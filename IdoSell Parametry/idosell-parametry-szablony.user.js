    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function buildBody(dn) {
        ...
        var contentCard = card('edit_note', 'Treść',
          '<div class="tm-field-row tm-field-row--stacked"><span class="tm-field-label">Nazwa (' + esc(getLangName(curLang)) + ')</span>' + mkInp('f_name', st[curLang].name, 'title') + '</div>' +
          ...
          rowStacked('Opis', 'host_desc') +
          (isVal ? rowStacked('Opis wyświetlany na liście towarów', 'host_list') : ''));
        ...
        var dispCard = card('grid_view', 'Wyświetlanie',
          ...
          '<div id="disp_own"' + (view === 'own' ? '' : ' style="display:none"') + '>' +
            ...
            rowF('Pozwól klientom zmieniać sortowanie', mkSel('f_asort', [['n', 'tak'], ['y', 'nie']], aSort, 'swap_vert')) +
            rowF('Pozwól klientom zmieniać ilość', mkSel('f_alim', [['n', 'tak'], ['y', 'nie']], aLim, 'format_list_numbered')) +
          '</div>');
      ...
      function renderGfx() {
        ...
        var origin = ''; try { var GW2 = (typeof getIframeWin === 'function') ? getIframeWin() : window; origin = (GW2 && GW2.location && GW2.location.origin) || location.origin || ''; } catch (e) { origin = location.origin || ''; }
        function abs(p) { if (!p) return ''; p = String(p).split('?')[0]; if (/^https?:\/\//i.test(p)) return p; return origin + '/' + p.replace(/^\/+/, ''); }
        [['search', 'Grafika na liście towarów'], ['projector', 'Grafika na karcie towaru']].forEach(function (pair) {
          ...
          gfxTypeState[cx] = typ;
          var sec = d.createElement('div'); sec.style.cssText = 'border:1px solid #e8ebf0;border-radius:10px;margin:8px 0;overflow:hidden;';
          var hd2 = d.createElement('div'); hd2.style.cssText = 'padding:8px 12px;background:#fafbfc;border-bottom:1px solid #eef0f4;font-size:12px;font-weight:700;color:#e65100;text-transform:uppercase;letter-spacing:.04em;'; hd2.textContent = title;
          sec.appendChild(hd2);
          var bd = d.createElement('div'); bd.style.cssText = 'padding:10px 12px;'; sec.appendChild(bd);
          // typ grafiki
          var trow = d.createElement('div'); trow.className = 'tm-field-row';
          trow.innerHTML = '<span class="tm-field-label">Typ grafiki</span>' + mkSel('gtype_' + cx, [['img_rwd', 'Trzy wielkości (RWD)'], ['img', 'Jedna wielkość']], typ, 'aspect_ratio');
          bd.appendChild(trow);
          var sizes = (typ === 'img_rwd') ? [['desktop', 'Komputer'], ['tablet', 'Tablet'], ['mobile', 'Smartfon']] : [['single', 'Grafika']];
          sizes.forEach(function (sz) {
            var kind = sz[0];
            var key = (kind === 'single') ? ('icon_' + cx) : ('icon_' + cx + '_' + kind);
            var cur = (L[key] && L[key][curShop]) ? abs(L[key][curShop]) : '';
            var row = d.createElement('div'); row.style.cssText = 'display:flex;align-items:center;gap:10px;padding:6px 0;flex-wrap:wrap;';
            var lab = d.createElement('span'); lab.style.cssText = 'font-size:12.5px;color:#344054;min-width:90px;'; lab.textContent = sz[1];
            row.appendChild(lab);
            if (cur) {
              var im = d.createElement('img'); im.src = cur + (cur.indexOf('?') > -1 ? '&' : '?') + 't=' + Date.now();
              im.style.cssText = 'max-height:42px;max-width:120px;border:1px solid #e0e3ea;border-radius:6px;background:#fff;';
              row.appendChild(im);
              var del = d.createElement('button'); del.type = 'button'; del.className = 'tm-btn-cancel'; del.style.cssText = 'font-size:11px;padding:4px 10px;'; del.textContent = 'Usuń';
              del.addEventListener('click', function () {
                if (!confirm('Usunąć grafikę?')) return;
                del.disabled = true;
                gfxRemove(cx, curShop, curLang, kind === 'single' ? null : kind).then(function () { if (_panel) _panel.showStatus('Usunięto grafikę'); if (dt.langData[curLang] && dt.langData[curLang][key]) dt.langData[curLang][key][curShop] = ''; renderGfx(); }).catch(function (e) { del.disabled = false; alert('Błąd: ' + (e.message || e)); });
              });
              row.appendChild(del);
            } else {
              var no = d.createElement('span'); no.style.cssText = 'font-size:12px;color:#98a2b3;'; no.textContent = 'brak'; row.appendChild(no);
            }
            var fin = d.createElement('input'); fin.type = 'file'; fin.accept = 'image/*'; fin.style.display = 'none';
            var br = d.createElement('button'); br.type = 'button'; br.className = 'tm-btn-cancel'; br.style.cssText = 'font-size:11px;padding:4px 10px;'; br.textContent = 'Wgraj…';
            br.addEventListener('click', function () { fin.value = ''; fin.click(); });
            fin.addEventListener('change', function () {
              var f = fin.files && fin.files[0]; if (!f) return;
              br.disabled = true; br.textContent = 'Wgrywanie…';
              gfxUpload(f, cx, curShop, kind, curLang).then(function (res) {
                if (_panel) _panel.showStatus('Wgrano grafikę');
                var pth = (res && res.path) || '';
                if (pth) { dt.langData[curLang] = dt.langData[curLang] || {}; dt.langData[curLang][key] = dt.langData[curLang][key] || {}; dt.langData[curLang][key][curShop] = pth.replace(/^\//, ''); }
                renderGfx();
              }).catch(function (e) { br.disabled = false; br.textContent = 'Wgraj…'; alert('Błąd wgrywania: ' + (e.message || e)); });
            });
            row.appendChild(br); row.appendChild(fin);
            bd.appendChild(row);
          });
          host.appendChild(sec);
          var tsel = bd.querySelector('[name=gtype_' + cx + ']');
          if (tsel) tsel.addEventListener('change', function () { gfxTypeState[cx] = tsel.value; renderGfx_typeChange(cx, tsel.value); });
        });
      }
      function renderGfx_typeChange(cx, v) {
        // zapis typu nastąpi przy „Zapisz" (setSettings icon_<cx>_type); odśwież sloty
        var L = (dt.langData && dt.langData[curLang]) || {};
        L['icon_' + cx + '_type'] = L['icon_' + cx + '_type'] || {};
        L['icon_' + cx + '_type'][curShop] = v;
        renderGfx();
      }
      ...
      function doSave() {
        ...
        fetchAjax(nameBody).then(function (r1) { if (r1 && r1.errno && r1.errno !== 0) settingsErr = (r1.message || ('errno ' + r1.errno)); }, function (e) { settingsErr = (e && e.message) || String(e); }).then(function () {
          ...
          return navGetNode(curShop, curLang).then(function (r) {
            ...
            var asc = (bodyEl.querySelector('[name=f_asort]') || {}).value || 'n';
            var alc = (bodyEl.querySelector('[name=f_alim]') || {}).value || 'n';
