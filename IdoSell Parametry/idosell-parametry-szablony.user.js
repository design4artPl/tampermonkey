...
(function (root) {
  ...
  function getAllSections(doc) {
    var out = [];
    doc.querySelectorAll('div[id^="showMenuSub_"].section').forEach(function (el) {
      var id = el.id.replace('showMenuSub_', '');
      if (!doc.getElementById('m_' + id)) return;
...
`;
  ...
  function mountSectionsPanel(doc) {
    ...
    mount.style.cssText = 'margin: 0 0 24px 0;';
    // Insert after the main mountTarget
