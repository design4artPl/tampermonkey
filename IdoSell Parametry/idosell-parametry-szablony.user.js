...
`;
  ...
  function showValueSeoModal(doc, nodeId) {
    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      function seoFlag(lg) { try { return (typeof getLangFlag === 'function') ? getLangFlag(lg, 14) : ''; } catch (e) { return ''; } }
      function subtitleHtml() {
        return 'ID: ' + esc(nodeId) + ' · wartość parametru „' + esc(elName) + '"' +
          ' · Sklep: ' + esc(shopLabel(curShop)) +
          ' · Język: <span style="display:inline-flex;align-items:center;gap:4px;vertical-align:-2px;">' + seoFlag(curLang) + '<span>' + esc(getLangName(curLang)) + '</span></span>';
      }
      var header = d.createElement('div');
