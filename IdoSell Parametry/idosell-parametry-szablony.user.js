...
`;
  ...
  function showTranslationsPage(doc) {
    ...
    function loadBatched() {
      var paramNames = {}; // paramId -> {lang: name}
      ...
      return chunkAll(paramList, 20, function (p) {
        ...
        // Faza B: getTreeForLang per (param, lang)
        var tasks = [];
        for (var i = 0; i < paramList.length; i++) {
          for (var j = 0; j < langsArr.length; j++) tasks.push({ paramId: paramList[i].id, lang: langsArr[j] });
        }
        ...
        var pvByLang = {}; // paramId -> {lang: [{id, name}]}
        return chunkAll(tasks, 20, function (t) {
          return getTreeForLang(t.paramId, t.lang).then(function (kids) {
            pvByLang[t.paramId] = pvByLang[t.paramId] || {};
            pvByLang[t.paramId][t.lang] = kids;
            doneCount++; updateProgress();
          }, function () {
            pvByLang[t.paramId] = pvByLang[t.paramId] || {};
            pvByLang[t.paramId][t.lang] = [];
            doneCount++; updateProgress();
          });
        }).then(function () {
          // Składanie loaded[]: params + values
          for (var ip = 0; ip < paramList.length; ip++) {
            var p = paramList[ip];
            loaded.push({ id: p.id, type: 'param', parentId: null, names: paramNames[p.id] || {} });
          }
          for (var iq = 0; iq < paramList.length; iq++) {
            var pp = paramList[iq];
            var byLang = pvByLang[pp.id] || {};
            // Union IDs po wszystkich językach (niektóre wartości mogą nie mieć tłumaczenia we wszystkich)
            var seenIds = {};
            var orderedIds = [];
            ...
            for (var oi = 0; oi < orderedIds.length; oi++) {
              ...
              loaded.push({ id: vid, type: 'value', parentId: pp.id, names: nmsByLang });
            }
          }
        });
      });
    ...
    updateProgress();
    loadBatched().then(function () { renderTable(); }, function (e) { renderTable(); if (_panel) _panel.showStatus('Błąd ładowania: ' + (e && e.message || e), true); });
