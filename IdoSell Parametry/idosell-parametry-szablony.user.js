...
(function (root) {
  ...
  function getAllSections(doc) {
    var out = [];
    // Try multiple selectors (IdoSell markup varies)
    var selectors = [
      'div[id^="showMenuSub_"].section',
      'div[id^="showMenuSub_"][class*="section"]',
      'li[class*="section"] [id^="showMenuSub_"]'
    ];
    var found = new Set();
    selectors.forEach(function (sel) {
      doc.querySelectorAll(sel).forEach(function (el) {
        var id = (el.id || '').replace('showMenuSub_', '');
        if (id && !found.has(id) && doc.getElementById('m_' + id)) {
          found.add(id);
          var childBlock = doc.getElementById('block_group' + id);
          var paramCount = childBlock ? childBlock.querySelectorAll(':scope > li[id^="m_"]').length : 0;
          out.push({ id: id, name: (el.textContent || '').trim(), paramCount: paramCount });
        }
      });
    });
    // Diagnostic when nothing found — log to console so we know which selector to adjust
    if (out.length === 0) {
      var uniq = new Set();
      doc.querySelectorAll('div[id^="showMenuSub_"]').forEach(function (el) {
        (el.className || '').toString().split(/\s+/).forEach(function (c) { if (c) uniq.add(c); });
      });
      console.warn('[parametry] No sections matched. showMenuSub class names seen:', Array.from(uniq));
      doc.querySelectorAll('div[id^="showMenuSub_"]').forEach(function (el, i) {
        if (i < 3) console.log('[parametry] sample showMenuSub:', el.outerHTML.substring(0, 300));
      });
    }
    out.sort(function (a, b) { return a.name.localeCompare(b.name, 'pl'); });
    return out;
  }
...
`;
  ...
  function mountSectionsPanel(doc) {
    ...
    var sections = getAllSections(doc);

    var sectionsPanel = PanelPro.mount(doc, {
      mountTarget: cardTarget,
      opsBar: {
        left: { label: 'Sekcje', buttons: [] },
        right: { label: 'Operacje na sekcjach', buttons: [
          { icon: 'sort_by_alpha', label: 'Sortuj alfabetycznie', variant: 'text',    onClick: function () { /* TODO: resort by name */ refreshSectionsPanel(doc); } },
      ...
      columns: [
        ...
        { id: 'params',  label: 'Parametr\u00f3w', width: '110px' },
    ...
    sections.forEach(function (sec) {
      ...
      li.innerHTML =
        ...
        '<div></div>' +
        '<div style="display:flex;align-items:center;gap:6px"><span class="material-symbols-outlined" style="color:#1d4ed8;font-size:20px">folder_special</span><span style="font-weight:500">' + escapeHtml(sec.name) + '</span></div>' +
        '<div style="text-align:center;font-family:Roboto Mono,monospace;font-size:12px;color:#5f6368">' + sec.id + '</div>' +
        '<div style="text-align:center">' + sec.paramCount + '</div>' +
        '<div style="display:flex;align-items:center;justify-content:center;gap:6px">' +
          '<span class="material-symbols-outlined" data-act="goto" title="Przejd\u017a do sekcji" style="cursor:pointer;color:#5f6368;font-size:20px">visibility</span>' +
          '<span class="material-symbols-outlined" data-act="edit" title="Edytuj" style="cursor:pointer;color:#5f6368;font-size:20px">settings</span>' +
          '<span class="material-symbols-outlined" data-act="delete" title="Usu\u0144" style="cursor:pointer;color:#dc2626;font-size:20px">delete</span>' +
        '</div>';
      ...
      li.querySelectorAll('[data-act]').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
          ...
          if (act === 'goto') {
            var target = doc.getElementById('m_' + sec.id);
            if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
          } else if (act === 'edit') {
            // reuse existing inline rename flow: trigger double-click on the section name in main tree
            var label = doc.querySelector('#m_' + sec.id + ' .tp-row-label');
            if (label) label.dispatchEvent(new Event('dblclick', { bubbles: true }));
          } else if (act === 'delete') {
            if (!confirm('Usun\u0105\u0107 sekcj\u0119 "' + sec.name + '"? (tylko lokalnie z widoku; do usuni\u0119cia trwa\u0142ego u\u017cyj natywnego)')) return;
          }
        });
      });
    ...
    _sectionsMount = { mount: mount, panel: sectionsPanel };
  }
  ...
  waitForIframe(function (doc) {
    ...
    // v4.5.22: sections panel below parameters list
    setTimeout(function () { mountSectionsPanel(doc); }, 1200);
