...
// @version      3.4.0
// @description  Toolbar do grupowej edycji parametrow: checkboxy, zaznaczanie, rozwijanie/zwijanie, grupowe usuwanie/edycja, import CSV
// @author       SyncOffer
...
`;
  ...
  function enhanceRow(li, doc) {
    ...
    if (selectedNodes.has(nodeId)) li.classList.add('tp-row--selected');
  ...
  function applyPagination(doc) {
    ...
    if (perPage === 0) {
      ...
      renderPaginationControls(doc, 1);
      updateCounter(doc);
      return;
    }
    ...
    _paginationState.totalVisible = filteredItems.length;
    renderPaginationControls(doc, totalPages);
    updateCounter(doc);
  }
  ...
  function deselectAll(doc) {
    ...
    const items = doc.querySelectorAll('.tp-row--selected');
    for (const li of items) li.classList.remove('tp-row--selected');
    updateCounter(doc);
  }
