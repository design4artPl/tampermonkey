    ...
    fetchAjax('action=getParameterLangData&id=' + encodeURIComponent(nodeId)).then(function (resp) {
      ...
      if (isVal) {
        ...
        function renderNavBody(shop) {
          ...
          // Nazwa w nagłówku (read-only Faza 1a)
          var rowHl = d.createElement('div'); rowHl.className = 'tp-se-row';
          rowHl.innerHTML = '<span class="tp-se-lbl">Nazwa w nagłówku</span><div class="tp-se-ctl"><span style="color:#475467;">' +
            (dnode.headline_name ? navEsc(dnode.headline_name) : ('automatyczna („' + navEsc(dnode.title_name || dnode.name || '') + '")')) + '</span></div>';
          navHost.appendChild(rowHl);
          // Meta (read-only Faza 1a)
          var rowMeta = d.createElement('div'); rowMeta.className = 'tp-se-row';
          rowMeta.innerHTML = '<span class="tp-se-lbl">Meta</span><div class="tp-se-ctl" style="font-size:12.5px;color:#475467;">' +
            'title: ' + (dnode.meta_title ? navEsc(dnode.meta_title) : '<i style="color:#98a2b3;">auto</i>') +
            '<br>robots: index=' + navEsc(dnode.meta_robots_index || 'default') + ', follow=' + navEsc(dnode.meta_robots_follow || 'default') + '</div>';
          navHost.appendChild(rowMeta);
          ...
          navSave.style.cssText = 'font-size:12px;padding:6px 14px;'; navSave.textContent = 'Zapisz opis listy (sklep ' + shop + ')';
          ...
          navSave.addEventListener('click', function () {
            var html = '';
            try { html = navEd.getValue(); } catch (e) {}
            navSave.disabled = true; navState.textContent = 'Zapisywanie…';
            navPost('action=setNodeDesc&desc=' + encodeURIComponent(html) + '&node=' + encodeURIComponent(nodeId) +
              '&lang=' + encodeURIComponent(curLang) + '&shop=' + encodeURIComponent(shop) + '&tree=parameters&parent=0').then(function (r) {
              navSave.disabled = false;
              if (r && r.status === 'ok') { navState.textContent = 'Zapisano ✓'; if (_panel) _panel.showStatus('Zapisano opis listy (sklep ' + shop + ', ' + getLangName(curLang) + ')'); }
              else { navState.textContent = 'błąd zapisu'; }
            }).catch(function (e) { navSave.disabled = false; navState.textContent = 'błąd: ' + (e.message || e); });
          });
        }
