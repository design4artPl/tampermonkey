...
(function() {
    ...
    // File System Access API - folder zapisu eksportu (Chrome/Edge)
    // Po wybraniu folderu raz - zapis bez promptow.
    ...
    // ========== FOLDER ZAPISU (File System Access API) ==========
    ...
    async function saveFolderHandleToDB(handle) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            // FileSystemDirectoryHandle jest structured-cloneable
            var req = store.put({ key: 'saveFolderHandle', handle: handle, name: handle.name });
    ...
    async function loadFolderHandleFromDB() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            var store = tx.objectStore('metadata');
            var req = store.get('saveFolderHandle');
    ...
    async function clearFolderHandleFromDB() {
        ...
        return new Promise(function(resolve, reject) {
            var tx = db.transaction(['metadata'], 'readwrite');
            var store = tx.objectStore('metadata');
            var req = store.delete('saveFolderHandle');
            req.onsuccess = function() { resolve(); };
            req.onerror = function() { reject(req.error); };
        });
    }

    // Zwraca obiekt window-like ktory ma showDirectoryPicker (window / unsafeWindow / globalThis / top)
    // albo null. Tampermonkey z @grant none teoretycznie daje native window, ale czasem
    // wrapper sandboxa moze ukryc nowe API. Sprawdzamy kilka scope'ow.
    function getFileSystemAccessHost() {
        var hosts = [];
        try { hosts.push(window); } catch(e) {}
        try { if (typeof unsafeWindow !== 'undefined') hosts.push(unsafeWindow); } catch(e) {}
        try { if (typeof globalThis !== 'undefined') hosts.push(globalThis); } catch(e) {}
        try { hosts.push(window.top); } catch(e) {}
        for (var i = 0; i < hosts.length; i++) {
            try {
                if (hosts[i] && typeof hosts[i].showDirectoryPicker === 'function') {
                    return hosts[i];
                }
            } catch(e) {}
        }
    ...
    function isFileSystemAccessSupported() {
        return getFileSystemAccessHost() !== null;
    }

    // Sprawdz czy mamy uprawnienie readwrite na handle (bez promptu)
    ...
    // Wymus prosbe o uprawnienie - moze prompt - wymaga user gesture
    ...
    // Zapis blobu do pliku w wybranym folderze. Throws on error.
    ...
    // Update UI po zmianie statusu folderu
    function updateSaveFolderUI() {
        var nameEl = document.getElementById('save-folder-name');
        var clearBtn = document.getElementById('clear-save-folder');
        if (!nameEl) return;
        if (saveFolderHandle) {
            nameEl.textContent = saveFolderName || saveFolderHandle.name || '(folder)';
            nameEl.style.color = '#27ae60';
            if (clearBtn) clearBtn.classList.remove('hidden');
        } else {
            nameEl.textContent = 'Pobrane (domyslnie)';
            nameEl.style.color = '#95a5a6';
            if (clearBtn) clearBtn.classList.add('hidden');
        }
    }

    // Try to restore folder handle from IDB on script load (silent if permission persisted)
    async function restoreSaveFolderIfGranted() {
        if (!isFileSystemAccessSupported()) return;
        try {
            var record = await loadFolderHandleFromDB();
            if (!record || !record.handle) return;
            var perm = await checkFolderPermission(record.handle);
            if (perm === 'granted') {
                saveFolderHandle = record.handle;
                saveFolderName = record.name || record.handle.name || '';
                updateSaveFolderUI();
                log('info', 'Folder zapisu przywrocony: ' + saveFolderName);
            } else if (perm === 'prompt') {
                // Mamy handle ale przegladarka chce ponownego potwierdzenia.
                // Nie mozemy poprosic bez user-gesture - pokaz info, ale NIE uzywaj
                // handle (inaczej kazdy zapis cicho failuje). User musi kliknac Wybierz.
                var nameEl = document.getElementById('save-folder-name');
                if (nameEl) {
                    nameEl.textContent = (record.name || record.handle.name) + ' (wybierz ponownie aby uaktywnic)';
                    nameEl.style.color = '#f39c12';
                }
            }
        } catch (e) {
            console.warn('Restore folder handle failed:', e);
        }
    }

    async function pickSaveFolder() {
        var host = getFileSystemAccessHost();
        if (!host) {
            ...
            log('error', 'File System Access API niedostepne. Wymaga Chrome/Edge 86+, HTTPS, top-level page.');
            log('info', 'Diag: UA="' + ua + '" secure=' + isSecure + ' window=' + hasWin + ' unsafeWindow=' + hasUW);
            log('info', 'Sprawdz w konsoli (F12): typeof window.showDirectoryPicker - powinno byc "function"');
            return;
        }
        try {
            var handle = await host.showDirectoryPicker({ mode: 'readwrite' });
            var perm = await requestFolderPermission(handle);
            if (perm !== 'granted') {
                log('error', 'Brak uprawnienia readwrite do folderu');
                return;
            }
            ...
            updateSaveFolderUI();
            log('success', 'Wybrano folder zapisu: ' + saveFolderName);
        } catch (e) {
            if (e.name === 'AbortError') return; // user anulowal
            log('error', 'Blad wyboru folderu: ' + e.message);
        }
    }
    ...
    async function clearSaveFolder() {
        ...
        saveFolderName = '';
        try { await clearFolderHandleFromDB(); } catch(e) {}
        updateSaveFolderUI();
        log('info', 'Folder zapisu wyczyszczony - eksport idzie do Downloads');
    }
    ...
    function createUI() {
        ...
        ui.innerHTML = `
            ...
            <label>Folder zapisu eksportu:</label>
            <div style="display:flex;gap:5px;align-items:center;">
                <button id="pick-save-folder" style="padding:6px 10px;margin:0;background:#3498db;color:white;border:none;border-radius:4px;cursor:pointer;font-size:11px;">Wybierz</button>
                <span id="save-folder-name" style="flex:1;font-size:11px;color:#95a5a6;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Pobrane (domyslnie)</span>
                <button id="clear-save-folder" class="hidden" style="padding:6px 10px;margin:0;background:#7f8c8d;color:white;border:none;border-radius:4px;cursor:pointer;font-size:11px;">X</button>
            </div>
            <div class="info">Bez wybierania: pliki ida do Downloads. Wybrany folder: cichy zapis bez promptow (Chrome/Edge).</div>
            ...
            <div id="log-panel-content" style="margin-top:5px; padding:5px; background:#1a252f; border-radius:4px; height:120px; overflow-y:auto; overflow-x:hidden; word-break:break-all; font-family:monospace;">
        ...
        document.getElementById('pick-save-folder').addEventListener('click', pickSaveFolder);
        document.getElementById('clear-save-folder').addEventListener('click', clearSaveFolder);
        ...
        // Przywroc folder zapisu jesli wczesniej wybrany i uprawnienie wciaz wazne
        restoreSaveFolderIfGranted();
    }
    ...
    async function retryEmptyProducts() {
        ...
        try {
            for (var i = 0; i < emptyIds.length && !shouldStop; i += parallelCount) {
                ...
                // Pobierz partie rownolegle z timeout (skala z parallel)
                var batchStartTime = Date.now();
                var batchTimeoutBuffer = Math.max(30000, parallelCount * 10000);
    ...
    async function collectProductIds(mode, count, specificIds) {
        ...
        if (window.location.href.indexOf('/admin/product/edit') === -1) {
            ...
            // Logujemy totalna liczbe produktow w sklepie raz na poczatku
            var totalEl = document.querySelector('.GF_Datagrid_records_total');
            var totalInShop = totalEl ? parseInt(totalEl.textContent) : 0;
            if (totalInShop) {
                log('info', 'Lacznie produktow w sklepie: ' + totalInShop);
            }
            ...
            while (collecting && !shouldStop) {
                pageNum++;
                tickWatchdog(); // reset watchdog na kazdej stronie

                var prevCount = ids.length;
                var rows = document.querySelectorAll('.GF_Datagrid .body tbody tr');
                ...
                var added = ids.length - prevCount;
                log('info', 'Strona ' + pageNum + ': +' + added + ' ID (lacznie ' + ids.length + (totalInShop ? '/' + totalInShop : '') + ')');
                ...
                if (mode === 'count' && ids.length >= count) {
                    ...
                    if (recordsTo < recordsTotal) {
                        ...
                        if (nextBtn) {
                            ...
                            await sleep(2000);
                            tickWatchdog();
                            updateStatus('Zbieranie ID: ' + ids.length + (totalInShop ? '/' + totalInShop : '') + '...');
                        } else {
                            collecting = false;
                        }
                    } else {
    ...
    function loadProductInIframe(productId, loadTime) {
        return new Promise(function(resolve, reject) {
            ...
            // Adaptacyjny timeout: skaluje sie z liczba rownoleglych iframow.
            // Przy parallel=5: 5 iframow x 4 zakladki AJAX = 20 requestow naraz,
            // event loop zatkany, timery odpalaja z opoznieniem. Buffer min 15s, max parallel*8s.
            var parallelCount = parseInt((document.getElementById('parallel-count') || {}).value) || 1;
            var timeoutBuffer = Math.max(15000, parallelCount * 8000);
            var timeoutLimit = loadTime + timeoutBuffer;
            var timeout = setTimeout(function() {
                if (resolved) return;
                resolved = true;
                var elapsed = Date.now() - startTime;
                timeoutCount++;
                log('error', 'TIMEOUT po ' + elapsed + 'ms (limit: ' + timeoutLimit + 'ms)', productId);
                cleanup();
                resolve(null);
            }, timeoutLimit);
            ...
            iframe.onload = function() {
                ...
                setTimeout(async function() {
                    ...
                    try {
                        ...
                        // Kategorie: drzewo checkboxow laduje sie lazy po klikieciu zakladki
                        var tabsToClick = ['Kategorie', 'Zdjęcia', 'Akcesoria', 'Produkty podobne', 'Sprzedaż krzyżowa'];
    ...
    function parseCategories(doc, data) {
        var categories = [];
        // Checkboxy kategorii maja pusty atrybut name - identyfikowane przez value (ID kategorii).
        // Sa w drzewie #category_pane (label.item-name > input[type=checkbox] + tekst nazwy).
        var pane = doc.querySelector('#category_pane');
        var checkedBoxes = pane
            ? pane.querySelectorAll('input[type="checkbox"]:checked')
            : doc.querySelectorAll('#category_pane input[type="checkbox"]:checked');
    ...
    function buildCategoryPath(checkbox) {
        ...
        while (currentLi) {
            ...
            if (label) {
                ...
                // Usun licznik produktow doklejany przez ShopWay na koncu nazwy, np. "Pościel (1)" -> "Pościel"
    ...
    function stripHtml(html) {
        ...
        // Zwin wszystkie biale znaki (\s obejmuje newline, tab, NBSP  ) w jedna spacje,
        // trim koncow. Daje "krotki opis" w jednej linii bez pustych pol po pustych <p>/<br>.
    ...
    function exportToCSV(products, suffix) {
        ...
        var filename = 'produkty_szczegoly_' + new Date().toISOString().slice(0,10) + suffix + '.csv';
        ...
        if (saveFolderHandle) {
            // Zapis do wybranego folderu (cichy, bez promptu). Fire-and-forget z fallbackiem.
            writeBlobToFolder(saveFolderHandle, filename, blob).then(function() {
                log('success', 'Zapisano: ' + filename + ' -> ' + (saveFolderName || 'folder'));
            }).catch(function(e) {
                log('error', 'Blad zapisu do folderu: ' + e.message + ' - pobieram do Downloads');
                fallbackDownload();
            });
        } else {
            fallbackDownload();
    ...
    async function startDetailExport() {
        ...
        log('info', 'Tryb: ' + mode + (mode === 'count' ? ', limit=' + count : '') + (mode === 'ids' ? ', ids="' + ids + '"' : ''));
        log('info', 'Sciezka: ' + window.location.pathname);

        updateStatus('Zbieranie listy produktow...');
        log('info', 'Zbieram liste produktow...');

        var productIds;
        try {
            productIds = await collectProductIds(mode, count, ids);
        } catch (e) {
            log('error', 'WYJATEK w collectProductIds: ' + e.message);
            updateStatus('Blad zbierania ID: ' + e.message);
            isExporting = false;
            document.getElementById('start-export').classList.remove('hidden');
            document.getElementById('stop-export').classList.add('hidden');
            return;
        ...
        if (productIds.length === 0) {
            log('error', 'Nie znaleziono produktow do eksportu (mode=' + mode +
                (mode === 'ids' ? ', wpisane ids="' + ids + '"' : '') +
                '). Sprawdz: 1) czy jestes na liscie produktow, 2) tryb eksportu, 3) pole ID jesli wybrany tryb "po ID"');
        ...
        try {
            for (var i = 0; i < productIds.length && !shouldStop; i += parallelCount) {
                ...
                // Pobierz partie rownolegle z timeout zabezpieczeniem (skala z parallel)
                var batchStartTime = Date.now();
                var batchTimeoutBuffer = Math.max(30000, parallelCount * 10000);
                var batchPromises = batch.map(function(productId) {
                    var timerId;
