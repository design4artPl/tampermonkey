...
(function() {
        ...
        try {
            for (var i = 0; i < emptyIds.length && !shouldStop; i += parallelCount) {
                ...
                var batchPromises = batch.map(function(productId) {
                    return Promise.race([
                        fetchProductData(productId, loadTime),
    ...
    function parseBasicInfo(doc, data) {
        data.nazwa = getInputValue(doc, 'basic_pane[language_data][1][name]');
        var seo = getInputValue(doc, 'basic_pane[language_data][1][seo]');
        data.url = seo ? window.location.origin + '/produkt/' + seo : '';
    ...
    function parseMetaInfo(doc, data) {
        data.seo_tytul = getInputValue(doc, 'meta_data[language_data][1][keywordtitle]');
        data.seo_opis = getTextareaValue(doc, 'meta_data[language_data][1][keyworddescription]');
        data.seo_slowa_kluczowe = getTextareaValue(doc, 'meta_data[language_data][1][keyword]');
    }
    ...
    function parseDescription(doc, data) {
        var shortDescHtml = getTextareaValue(doc, 'description_pane[language_data][1][shortdescription]');
        data.opis_krotki = stripHtml(shortDescHtml);

        data.opis_dlugi_html = getTextareaValue(doc, 'description_pane[language_data][1][description]');
        data.opis_dodatkowe_info_html = getTextareaValue(doc, 'description_pane[language_data][1][longdescription]');
        data.opis_rozszerzony_html = data.opis_dlugi_html + (data.opis_dodatkowe_info_html ? '\n' + data.opis_dodatkowe_info_html : '');

        data.slowa_wyszukiwarki = getTextareaValue(doc, 'description_pane[language_data][1][search_words]');
        data.youtube_id = getInputValue(doc, 'description_pane[language_data][1][yt1]');
        data.youtube_id_2 = getInputValue(doc, 'description_pane[language_data][1][yt2]');
    }
    ...
    function parsePhotos(doc, data) {
        ...
        photoInputs.forEach(function(input) {
            ...
            if (/photos_pane\[photo\]\[\d+\]/.test(name)) {
                ...
                if (photoId) {
                    var url = window.location.origin + '/design/_gallery/_orginal/' + photoId + '.jpg';
        ...
        if (mainPhotoId) {
            mainPhoto = window.location.origin + '/design/_gallery/_orginal/' + mainPhotoId + '.jpg';
    ...
    function exportToCSV(products, suffix) {
        suffix = suffix || '';
        var headers = [
            'ID', 'STATUS_POBRANIA', 'Nazwa', 'URL', 'Wyswietlany', 'Zgodny ze zdjeciem', 'EAN', 'Kod dostawcy', 'Producent', 'Dostawca',
            'SEO Tytul', 'SEO Opis', 'SEO Slowa kluczowe',
            'Stan magazynowy', 'Dostepnosc',
            'Kategorie',
            'Grupa cenowa', 'Stawka VAT', 'Waluta sprzedazy', 'Waluta zakupu',
            'Cena zakupu netto', 'Cena zakupu brutto', 'Cena sprzedazy netto', 'Cena sprzedazy brutto',
            'Cena promocyjna netto', 'Cena promocyjna brutto', 'Data rozpoczecia promocji', 'Data zakonczenia promocji',
            'Waga (kg)', 'Waga (g)', 'Szerokosc', 'Wysokosc', 'Glebokosc', 'Jednostka miary', 'Ilosc w opakowaniu',
            'Opis krotki', 'Opis dlugi (HTML)', 'Opis dodatkowe info (HTML)', 'Opis rozszerzony (HTML)',
            'Slowa wyszukiwarki', 'YouTube ID', 'YouTube ID 2',
            'Galeria', 'Ikona',
            'Akcesoria', 'Produkty podobne', 'Sprzedaz krzyzowa',
            'Dane techniczne'
        ];
        ...
        products.forEach(function(p) {
            var row = [
                ...
                p.cena_promocyjna_netto,
                p.cena_promocyjna_brutto,
                p.data_rozpoczecia_promocji,
                p.data_zakonczenia_promocji,
                p.waga_kg,
                p.waga_g,
                p.szerokosc,
                p.wysokosc,
                ...
                p.opis_krotki,
                p.opis_dlugi_html,
                p.opis_dodatkowe_info_html,
                p.opis_rozszerzony_html,
                p.slowa_wyszukiwarki,
                p.youtube_id,
                p.youtube_id_2,
                p.galeria,
                p.ikona,
