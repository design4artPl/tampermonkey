...
(function () {
  ...
  // =========================================================================
  // MODULE: OPINIONS (OPINIE)
  // =========================================================================

  async function fetchAllOpinions(domain, apiKey, shopId, log) {
    const all = [];
    let page = 0;
    while (true) {
      const url = buildUrl(domain, `/api/admin/v7/products/opinions/opinions?opinion%5BshopId%5D=${shopId}&resultsPage=${page}&resultsLimit=100`);
      log(`Pobieranie opinii: strona ${page + 1}...`);
      const data = await apiRequest('GET', url, apiKey);
      const results = data.results || [];
      all.push(...results);
      const total = data.resultsNumberAll || 0;
      log(`Pobrano ${all.length} / ${total} opinii`);
      if ((page + 1) * 100 >= total || !results.length) break;
      page++;
    }
    return all;
  }

  async function importOpinions(domain, apiKey, opinions, targetShopId, log) {
    let successCount = 0, failCount = 0;
    const errors = [];
    log(`Importowanie ${opinions.length} opinii do shopId=${targetShopId}...`);

    for (let i = 0; i < opinions.length; i++) {
      const o = opinions[i];
      const productId = o.product?.id;
      if (!productId) { failCount++; errors.push(`Op ${o.id}: brak productId`); continue; }

      const body = {
        params: {
          confirmed: o.confirmed ? 'confirmed' : 'not_confirmed',
          rating: parseInt(o.rating, 10) || 5,
          content: o.content || '',
          language: o.language || 'pol',
          shopAnswer: o.answer || '',
          opinionConfirmedByPurchase: !!o.confirmedByPurchase,
          shopId: targetShopId,
          product: { id: productId },
        },
      };
      if (o.client?.login) body.params.author = o.client.login;
      if (o.createDate) body.params.createDate = o.createDate;

      try {
        const url = buildUrl(domain, '/api/admin/v7/products/opinions/opinions');
        const res = await apiRequest('PUT', url, apiKey, body);
        if (res.errors && res.errors.faultCode) {
          failCount++;
          errors.push(`Op ${o.id} (prod ${productId}): ${res.errors.faultString}`);
        } else {
          successCount++;
        }
      } catch (err) {
        failCount++;
        errors.push(`Op ${o.id}: ${err.message}`);
      }

      if ((i + 1) % 25 === 0) log(`Postep: ${successCount} OK, ${failCount} bledow / ${opinions.length}`);
    }
    return { imported: successCount, failed: failCount, errors };
  }

  // =========================================================================
  // MODULE: ENTRIES (BLOGI)
  // =========================================================================

  async function fetchEntryIdsFromPanel(domain, shopId, log) {
    const url = buildUrl(domain, `/panel/app/entries.php?mode=blog&action=items&shop=${shopId}`);
    log(`Pobieranie listy blogow z panelu: ${url}`);
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        method: 'GET',
        url,
        onload(res) {
          ...
          if (res.responseText.includes('panel_login') || res.responseText.includes('Logowanie')) {
            reject(new Error('Brak sesji na panelu zrodlowym - zaloguj sie na ' + domain));
            return;
          }
          const ids = new Set();
          const patterns = [
            ...
            /id_entry[=:"\s]+(\d{6,})/g,
            /data-entry-id="(\d{6,})"/g,
            /entries\.php\?[^"']*[?&]id=(\d{6,})/g,
          ];
          for (const p of patterns) {
            let m;
            while ((m = p.exec(res.responseText)) !== null) ids.add(parseInt(m[1], 10));
          }
          resolve([...ids]);
        },
        onerror(e) { reject(new Error('Blad sieci: ' + (e.statusText || ''))); },
      });
    });
  }

  async function fetchEntryDetails(domain, apiKey, entryId, log) {
    const url = buildUrl(domain, `/api/admin/v7/entries/entries?entryId=${entryId}`);
    const data = await apiRequest('GET', url, apiKey);
    return data.result?.entry || data.result || null;
  }
  ...
  function downloadImageBase64(url) {
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        ...
        onload(res) {
          if (res.status >= 400 || !res.response) { resolve(null); return; }
  ...
  function pictureFormatFromUrl(url) {
    const m = (url || '').toLowerCase().match(/\.(png|jpg|jpeg|gif)(\?|$)/);
    if (!m) return 'jpg';
    return m[1] === 'jpeg' ? 'jpg' : m[1];
  }

  async function importEntries(srcDomain, srcKey, tgtDomain, tgtKey, entries, srcShopId, tgtShopId, log) {
    let successCount = 0, failCount = 0;
    const errors = [];
    log(`Importowanie ${entries.length} blogow do shopId=${tgtShopId}...`);

    for (let i = 0; i < entries.length; i++) {
      const eid = entries[i];
      try {
        const detail = await fetchEntryDetails(srcDomain, srcKey, eid, log);
        if (!detail) { failCount++; errors.push(`Entry ${eid}: brak danych`); continue; }

        const langs = (detail.langs || detail.langData || []).map(l => ({
          langId: l.langId || l.lang_id || 'pol',
          title: l.title || '',
        ...
        const body = {
          params: {
            ...
            visibleOnSitesList: [{ siteId: tgtShopId }],
            products: (detail.products || []).map(p => ({ productId: p.productId || p.id })),
            langs,
            titleLinkType: detail.titleLinkType || false,
            link: detail.link || '',
          },
        };

        if (detail.picture && !detail.picture.endsWith('/.')) {
          const picUrl = detail.picture.startsWith('http') ? detail.picture : `${srcDomain.replace(/\/$/, '')}/${detail.picture.replace(/^\//, '')}`;
          const b64 = await downloadImageBase64(picUrl);
          if (b64) {
            body.params.pictureData = {
              pictureBase64: b64,
              pictureFormat: pictureFormatFromUrl(picUrl),
            };
          }
        }

        const url = buildUrl(tgtDomain, '/api/admin/v7/entries/entries');
        const res = await apiRequest('PUT', url, tgtKey, body);
        if (res.errors && res.errors.faultCode) {
          failCount++;
          errors.push(`Entry ${eid}: ${res.errors.faultString}`);
        } else {
          successCount++;
        }
      } catch (err) {
        failCount++;
        errors.push(`Entry ${eid}: ${err.message}`);
      }

      if ((i + 1) % 5 === 0) log(`Postep: ${successCount} OK, ${failCount} bledow / ${entries.length}`);
    }
  ...
  const MODULES = [
    ...
    // --- SEPARATOR: CONTENT ---
    { id: '_sep_content', label: null, section: 'Tresci' },
    {
      id: 'opinions',
      label: 'Opinie (Opinions)',
      icon: '\u2B50',
      run: async (cfg, log) => {
        log('--- Start migracji opinii ---');
        const srcShopId = cfg.sourceShopId || 1;
        const tgtShopId = parseInt((cfg.targetShopIds || '1').split(',')[0].trim(), 10) || 1;
        log(`Source shopId=${srcShopId}, Target shopId=${tgtShopId}`);

        const opinions = await fetchAllOpinions(cfg.sourceDomain, cfg.sourceApiKey, srcShopId, log);
        log(`Pobrano ${opinions.length} opinii ze zrodla.`);
        if (opinions.length === 0) {
          log('Brak opinii.');
          return { ok: true, matchCount: 0, total: 0, issues: [] };
        }

        const result = await importOpinions(cfg.targetDomain, cfg.targetApiKey, opinions, tgtShopId, log);
        log(`--- Zakonczono: ${result.imported} OK, ${result.failed} bledow ---`);
        if (result.errors.length > 0) log('Bledy (max 20):\n' + result.errors.slice(0, 20).join('\n'));

        return { ok: result.failed === 0, matchCount: result.imported, total: opinions.length, issues: result.errors };
      },
    },
    {
      id: 'entries',
      ...
      run: async (cfg, log) => {
        ...
        let entryIds;
        try {
          entryIds = await fetchEntryIdsFromPanel(cfg.sourceDomain, srcShopId, log);
        } catch (e) {
          log(`BLAD scrape: ${e.message}`);
          return { ok: false, matchCount: 0, total: 0, issues: [e.message] };
        }
        log(`Znaleziono ${entryIds.length} ID blogow.`);
        if (entryIds.length === 0) {
          log('Brak blogow w panelu zrodlowym (lub regex nie znalazl ID - sprawdz strukture HTML).');
          return { ok: true, matchCount: 0, total: 0, issues: [] };
        }

        const result = await importEntries(cfg.sourceDomain, cfg.sourceApiKey, cfg.targetDomain, cfg.targetApiKey, entryIds, srcShopId, tgtShopId, log);
        log(`--- Zakonczono: ${result.imported} OK, ${result.failed} bledow ---`);
        if (result.errors.length > 0) log('Bledy (max 20):\n' + result.errors.slice(0, 20).join('\n'));

        return { ok: result.failed === 0, matchCount: result.imported, total: entryIds.length, issues: result.errors };
      },
    },
  ];
