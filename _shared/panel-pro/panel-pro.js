/* ============================================================================
 * panel-pro v1.0.0
 * Reusable list/tree panel widget for Tampermonkey userscripts on IdoSell-style
 * admin pages. Renders one bordered container with toolbar (search, sections,
 * dropdown, bulk actions), grid table header, slot for the existing tree/list,
 * footer and pagination.
 *
 * Loaded via @require with a versioned tag URL.
 *
 * Usage:
 *   const panel = PanelPro.mount(doc, {
 *     mountTarget: doc.querySelector('#some_container'),
 *     treeRoot:    doc.querySelector('ul#tree_root'),
 *     toolbar: { ... },
 *     columns: [ ... ],
 *     pagination: { perPage: 50, perPageOptions: [10,25,50,100,0] },
 *   });
 *
 * Public API:
 *   panel.root, .toolbar, .tableHeader, .body, .footer
 *   panel.applyRowGrid(li)  -> add classes to <li> for grid layout
 *   panel.showStatus(msg, isError)
 *   panel.renderPagination(state, opts)
 *   panel.setBulkVisible(bool), .findToolbarBtn(id), .destroy()
 * ==========================================================================*/
(function (root) {
  'use strict';
  if (root.PanelPro) return;

  var VERSION = '1.0.0';
  var STYLE_ID = 'panel-pro-styles-' + VERSION.replace(/\./g, '_');
  ...
  var CSS = [
    ...
    '}',
    '.panel-pro__group--bulk { margin-left: auto; padding-top: 6px; }',
    '.panel-pro__search {',
    ...
    '  width: 200px; padding: 4px 0 !important; margin: 0 !important;',
    ...
    '  padding: 10px 14px; border-bottom: 2px solid #dadce0;',
    '  background: #f1f3f4;',
    ...
    'li.panel-pro__row {',
    '  display: grid !important; column-gap: 8px; align-items: center;',
  ...
  function buildToolbar(doc, cfg, ctx) {
    ...
    (cfg.sections || []).forEach(function (section, idx) {
      if (idx > 0 || cfg.search) toolbar.appendChild(el(doc, 'div', { className: 'panel-pro__separator' }));
    ...
    if (cfg.bulkSection) {
      toolbar.appendChild(el(doc, 'div', { className: 'panel-pro__break' }));
      var bulk = el(doc, 'div', { className: 'panel-pro__group panel-pro__group--bulk' });
      if (cfg.bulkSection.label) bulk.appendChild(el(doc, 'span', { className: 'panel-pro__label', textContent: cfg.bulkSection.label }));
      (cfg.bulkSection.buttons || []).forEach(function (btn) { bulk.appendChild(buildButton(doc, btn, ctx)); });
  ...
  function buildDropdown(doc, cfg, ctx) {
    ...
    (cfg.options || []).forEach(function (opt) {
      ...
      item.addEventListener('click', function (e) {
        e.stopPropagation();
        wrap.querySelectorAll('.panel-pro__dropdown__item').forEach(function (i) { i.classList.remove('panel-pro--active'); });
        item.classList.add('panel-pro--active');
  ...
  function mount(doc, options) {
    ...
    var api = {
      ...
      setBulkVisible: function (visible) {
        var bulkEl = rootEl.querySelector('.panel-pro__group--bulk');
        if (bulkEl) bulkEl.style.display = visible ? '' : 'none';
      },
