 ...
 *   - removed outer border/radius/shadow (panel sits flush with page)
 *   - white toolbar bg, wider padding
 *   - new opsBar: top row with left/right button groups (Tools | Tree ops)
 *   - new selectionBar: replaces tableHeader when items selected, with
 *     parametrized label and actions (Edytuj/Przenies/Usun/...)
 *   - search box: border-radius 6px (rectangular)
 *   - removed leading separator before first toolbar section
 *   - thead bg #efefef, padding 12px 0
 *   - DRZEWO icons defaults expected: keyboard_double_arrow_up/down
 *   - panel.setSelection(n) toggles selection bar visibility
 *
 * Usage:
 *   const panel = PanelPro.mount(doc, {
 *     mountTarget, treeRoot,
 *     opsBar: { left: [...], right: [...] },        // or null/false to hide
 *     toolbar: { search, sections },                  // bulkSection removed
 *     selectionBar: { selectedLabel, actions },       // optional
 *     columns, pagination,
 *   });
 *
 * Public API: panel.root, .toolbar, .opsBar, .tableHeader, .selectionBar,
 *   .body, .footer, .applyRowGrid(li), .showStatus(msg, isError),
 *   .renderPagination(state, opts), .setSelection(count), .destroy()
 * ==========================================================================*/
(function (root) {
  'use strict';
  if (root.PanelPro && root.PanelPro.VERSION === '1.1.0') return;

  var VERSION = '1.1.0';
  var STYLE_ID = 'panel-pro-styles-v1_1_0';
  ...
  var CSS = [
    ...
    '}',
    '.panel-pro__opsbar__group {',
    '  display: flex; align-items: center; gap: 6px; flex-wrap: wrap;',
    '}',
    '.panel-pro__opsbar__label {',
    '  font-size: 11px; color: #80868b;',
    '  text-transform: uppercase; letter-spacing: 0.5px;',
    '  font-weight: 500; padding-right: 4px;',
    '}',
    ...
    '.panel-pro__separator { width: 1px; height: 24px; background: #dadce0; margin: 0 6px; }',
    '.panel-pro__spacer { flex: 1; }',
    '.panel-pro__break { flex-basis: 100%; height: 0; }',
    '.panel-pro__label {',
    '  font-size: 11px; color: #80868b;',
    '  text-transform: uppercase; letter-spacing: 0.5px;',
    '  font-weight: 500; padding-right: 4px;',
    ...
    '}',
    '.panel-pro__search .material-symbols-outlined { font-size: 18px; color: #5f6368; }',
    '.panel-pro__search input {',
    '  border: none !important; outline: none !important;',
    '  background: transparent !important; font-size: 13px !important;',
    '  width: 220px; padding: 4px 0 !important; margin: 0 !important;',
    '  box-shadow: none !important; color: #202124 !important;',
    '  font-family: inherit;',
    '}',
    ...
    '}',
    '.panel-pro__btn:hover:not(:disabled) { background: #f1f3f4; box-shadow: 0 1px 3px rgba(0,0,0,.1); }',
    '.panel-pro__btn:disabled { opacity: 0.5; cursor: not-allowed; }',
    '.panel-pro__btn .material-symbols-outlined { font-size: 18px; }',
    '.panel-pro__btn--icon { padding: 6px; border-color: transparent; background: transparent; }',
    '.panel-pro__btn--icon:hover:not(:disabled) { background: #e8eaed; }',
    '.panel-pro__btn--icon .material-symbols-outlined { font-size: 20px; }',
    '.panel-pro__btn--text { border-color: transparent; background: transparent; }',
    '.panel-pro__btn--text:hover:not(:disabled) { background: #e8eaed; }',
    '.panel-pro__btn--primary { background: #1a73e8; color: #fff; border-color: #1a73e8; }',
    '.panel-pro__btn--primary:hover:not(:disabled) { background: #1765cc; }',
    '.panel-pro__btn--danger { color: #d93025; }',
    '.panel-pro__btn--danger:hover:not(:disabled) { background: #fce8e6; }',
    ...
    '.panel-pro__dropdown__toggle {',
    '  display: inline-flex; align-items: center; gap: 4px;',
    '  padding: 6px 10px; border: 1px solid #dadce0; border-radius: 4px;',
    '  background: #fff; cursor: pointer;',
    '  font-size: 13px; color: #202124; font-family: inherit;',
    '}',
    '.panel-pro__dropdown__toggle:hover { background: #f1f3f4; }',
    '.panel-pro__dropdown__toggle .material-symbols-outlined { font-size: 18px; color: #1a73e8; }',
    '.panel-pro__dropdown__menu {',
    '  position: absolute; top: calc(100% + 4px); left: 0;',
    '  min-width: 200px; background: #fff;',
    '  border: 1px solid #dadce0; border-radius: 6px;',
    '  box-shadow: 0 2px 8px rgba(0,0,0,0.15);',
    '  z-index: 10000; display: none; padding: 4px 0;',
    '}',
    '.panel-pro__dropdown__menu.panel-pro--open { display: block; }',
    '.panel-pro__dropdown__item {',
    '  display: flex; align-items: center;',
    '  padding: 8px 12px; cursor: pointer;',
    '  font-size: 13px; color: #202124; gap: 6px;',
    '}',
    '.panel-pro__dropdown__item:hover { background: #f1f3f4; }',
    '.panel-pro__dropdown__item.panel-pro--active { background: #e8f0fe; color: #1a73e8; font-weight: 500; }',
    ...
    '  padding: 12px 16px; background: #efefef;',
    '  border-bottom: 1px solid #dadce0;',
    '  font-size: 12px; font-weight: 500; color: #5f6368;',
    '  text-transform: uppercase; letter-spacing: 0.5px;',
    '}',
    '.panel-pro__thead > div { padding: 0 4px; text-align: center; }',
    '.panel-pro__thead .panel-pro__col--name { text-align: left; }',
    '.panel-pro__thead .panel-pro__col-hint {',
    '  font-size: 10px; color: #9aa0a6;',
    '  font-weight: 400; letter-spacing: 0;',
    '  text-transform: none; margin-left: 6px;',
    '}',

    /* Selection bar — replaces table header when items selected */
    '.panel-pro__selection-bar {',
    ...
    '}',
    '.panel-pro__selection-bar.panel-pro--visible { display: flex; }',
    '.panel-pro__selection-bar__label {',
    '  display: inline-flex; align-items: center; gap: 8px;',
    '  font-weight: 500;',
    '}',
    '.panel-pro__selection-bar__count {',
    '  display: inline-flex; align-items: center; justify-content: center;',
    '  min-width: 22px; height: 22px; padding: 0 6px;',
    '  background: #1a73e8; color: #fff;',
    '  border-radius: 11px; font-weight: 600; font-size: 12px;',
    '}',
    '.panel-pro__selection-bar__actions { display: flex; align-items: center; gap: 6px; }',
    '.panel-pro__selection-bar__close {',
    '  display: inline-flex; align-items: center; justify-content: center;',
    '  width: 28px; height: 28px; border: none; background: transparent;',
    '  cursor: pointer; color: #5f6368; border-radius: 50%;',
    '}',
    '.panel-pro__selection-bar__close:hover { background: rgba(0,0,0,0.05); color: #202124; }',
    ...
    'li.panel-pro__row {',
    '  display: grid !important; column-gap: 10px; align-items: center;',
    ...
    '}',
    'li.panel-pro__row:hover { background: #f8f9fa; }',
    'li.panel-pro__row.panel-pro--selected { background: #e8f0fe; }',
    'li.panel-pro__row.panel-pro--selected:hover { background: #d2e3fc; }',
    'li.panel-pro__row.panel-pro--hidden,',
    'li.panel-pro__row.panel-pro--filter-hidden,',
    'li.panel-pro__row.panel-pro--view-hidden { display: none !important; }',
    'li.panel-pro__row > div { padding: 0 4px; }',
    ...
    '  animation: panel-pro-fade-in 0.2s ease;',
    '}',
    '.panel-pro__status--error { background: #d93025; }',
    '@keyframes panel-pro-fade-in {',
    ...
    '.panel-pro__pagination__btn,',
    '.panel-pro__pagination__page {',
    '  display: inline-flex; align-items: center; justify-content: center;',
    ...
    '  font-size: 13px; color: #202124; font-family: inherit;',
    '}',
    '.panel-pro__pagination__btn:hover:not(:disabled),',
    '.panel-pro__pagination__page:hover:not(.panel-pro--active) { background: #f1f3f4; }',
    '.panel-pro__pagination__page--active { background: #1a73e8 !important; color: #fff !important; border-color: #1a73e8 !important; }',
    '.panel-pro__pagination__btn:disabled { opacity: 0.4; cursor: not-allowed; }',
    '.panel-pro__pagination__per-page select {',
  ...
  function buildOpsBar(doc, cfg, ctx) {
    ...
    function buildSide(side) {
      var group = el(doc, 'div', { className: 'panel-pro__opsbar__group' });
      if (side.label) group.appendChild(el(doc, 'span', { className: 'panel-pro__opsbar__label', textContent: side.label }));
      (side.buttons || []).forEach(function (b) { group.appendChild(buildButton(doc, b, ctx)); });
      return group;
    }
    bar.appendChild(cfg.left ? buildSide(cfg.left) : el(doc, 'div'));
    bar.appendChild(cfg.right ? buildSide(cfg.right) : el(doc, 'div'));
    return bar;
  }
  ...
  function buildToolbar(doc, cfg, ctx) {
    ...
    if (cfg.search) {
      var sg = el(doc, 'div', { className: 'panel-pro__group' });
      ...
      sg.appendChild(sb);
      toolbar.appendChild(sg);
      toolbar.appendChild(el(doc, 'div', { className: 'panel-pro__spacer' }));
    }
    (cfg.sections || []).forEach(function (section, idx) {
      // No leading separator before first section anymore (v1.1.0)
      if (idx > 0) toolbar.appendChild(el(doc, 'div', { className: 'panel-pro__separator' }));
  ...
  function buildSelectionBar(doc, cfg, ctx) {
    var bar = el(doc, 'div', { className: 'panel-pro__selection-bar' });
    var labelWrap = el(doc, 'div', { className: 'panel-pro__selection-bar__label' });
    var checkIcon = doc.createElement('input');
    ...
    labelWrap.appendChild(checkIcon);
    var labelTpl = cfg.selectedLabel || 'Wybrano {n} obiektow';
    var labelText = doc.createElement('span');
    labelText.dataset.ppLabel = labelTpl;
    labelWrap.appendChild(labelText);
    var countChip = el(doc, 'span', { className: 'panel-pro__selection-bar__count', textContent: '0' });
    bar.appendChild(labelWrap);
    bar.dataset.ppCountChip = '1';

    var actions = el(doc, 'div', { className: 'panel-pro__selection-bar__actions' });
    (cfg.actions || []).forEach(function (a) { actions.appendChild(buildButton(doc, a, ctx)); });
    ...
    // Inject count chip into label text
    var labelHtml = labelTpl.replace(/\{n\}/, '__COUNT__');
    labelText.innerHTML = labelHtml.replace('__COUNT__', '<span class="panel-pro__selection-bar__count">0</span>');
    return bar;
  }
  ...
  function mount(doc, options) {
    ...
    var rootEl = el(doc, 'div', { className: 'panel-pro' });
    var ctx = { api: null };
    ...
    var selectionBar = options.selectionBar ? buildSelectionBar(doc, options.selectionBar, ctx) : null;
    ...
    if (opsBar) rootEl.appendChild(opsBar);
    if (toolbar) rootEl.appendChild(toolbar);
    rootEl.appendChild(header);
    if (selectionBar) rootEl.appendChild(selectionBar);
    rootEl.appendChild(body);
    rootEl.appendChild(footer);

    options.mountTarget.appendChild(rootEl);
    ...
    var api = {
      ...
      findToolbarBtn: function (id) { return rootEl.querySelector('.panel-pro__btn[data-pp-id="' + id + '"]'); },
      destroy: function () {
        rootEl.remove();
