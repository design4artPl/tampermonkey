 ...
 *   - Style restored to 1:1 match with reference categories-list UI:
 *     * card panel: border 1px #dadce0, radius 8px, overflow:hidden
 *     * opsBar: TWO separate floating cards above the panel
 *       (NarzedziaMenu | OperacjeNaDrzewie+Add) with their own border/shadow
 *     * selection bar: absolute overlay over the table header, with grid
 *       template (24px 40px 1fr) so checkbox aligns with row check column
 *     * animated search box (focus expands width)
 *     * row min-height 37px, padding 4px 0, border #e2e8f0
 *     * thead bg #efefef, padding 12px 0
 *     * dropdown wrapper renamed to .panel-pro__dropdown (unchanged)
 *     * width:100% + min-width:0 + box-sizing:border-box on container
 *       to prevent horizontal scrollbar in narrow mount targets
 *   - panel.setSelection(count) now toggles overlay class (no display flip)
 *
 * Usage unchanged. Public API unchanged.
 * ==========================================================================*/
(function (root) {
  'use strict';
  ...
  var CSS = [
    /* OpsBar — floating cards ABOVE the main panel */
    '.panel-pro__opsbar {',
    '  display: flex; flex-wrap: wrap; align-items: center;',
    '  justify-content: space-between; gap: 12px;',
    '  margin: 0 0 12px 0;',
    '}',
    '.panel-pro__opsbar-group {',
    '  display: flex; flex-wrap: wrap; gap: 8px; align-items: center;',
    '  background: #fff; padding: 10px 16px;',
    '  border: 1px solid #e2e8f0; border-radius: 6px;',
    '  box-shadow: 0 1px 2px rgba(0,0,0,0.04);',
    '}',
    '.panel-pro__opsbar-label {',
    '  color: #64748b; font-size: 11px; font-weight: 700;',
    '  text-transform: uppercase; letter-spacing: 0.5px;',
    '  padding-right: 4px;',
    '}',
    '.panel-pro__opsbar-btn {',
    ...
    /* Toolbar (search left, sections right) */
    '.panel-pro__toolbar {',
    ...
    '  border-radius: 6px; padding: 7px 12px; width: 260px;',
    ...
    '  font-size: 13px; font-weight: 500; font-family: inherit; color: #334155;',
    ...
    '  padding: 5px 10px; border: 1px solid transparent; border-radius: 4px;',
    '  background: transparent; cursor: pointer;',
    '  font-size: 13px; color: #334155; font-family: inherit; font-weight: 500;',
    '}',
    ...
    '  font-size: 13px; color: #334155; gap: 8px;',
    '}',
    ...
    '  padding: 12px 0; background: #efefef;',
    '  border-bottom: 1px solid #dadce0;',
    ...
    /* Selection bar — absolute overlay over header */
    ...
    '  box-sizing: border-box;',
    '}',
    '.panel-pro__selection-bar.panel-pro--visible { opacity: 1; pointer-events: auto; transform: translateY(0); }',
    ...
    '.panel-pro__selection-bar__content {',
    '  display: flex; align-items: center; justify-content: space-between;',
    '  gap: 12px; flex-wrap: wrap; min-width: 0;',
    '}',
    '.panel-pro__selection-bar__left {',
    '  display: flex; align-items: center; gap: 8px;',
    ...
    '.panel-pro__selection-bar__btn {',
    '  display: inline-flex; align-items: center; gap: 5px;',
    '  padding: 4px 12px; background: #fff; color: #334155;',
    ...
    '  cursor: pointer; color: #64748b; border-radius: 4px; margin-left: 4px;',
    ...
    '  min-height: 37px; transition: background 0.1s;',
    ...
    '.panel-pro__pagination__page:hover:not(.panel-pro--active) { background: #f8fafc; border-color: #cbd5e1; }',
  ...
  function buildSelectionBar(doc, cfg, ctx, columns) {
    // Grid: drag(24) + check(40) + 1fr (rest), matching first 3 columns of row
    ...
    (cfg.actions || []).forEach(function (a, i) {
      // Insert separator before non-primary actions if mixed
      rightCol.appendChild(buildSelectionBarBtn(doc, a, ctx));
    });

    // Optional separator + extra actions placed AFTER first actions
  ...
  function mount(doc, options) {
    ...
    // OpsBar lives OUTSIDE the card panel
