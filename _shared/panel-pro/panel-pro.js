...
(function (root) {
  'use strict';
  if (root.PanelPro && root.PanelPro.VERSION === '1.2.1') return;

  var VERSION = '1.2.1';
  var STYLE_ID = 'panel-pro-styles-v1_2_1';
  ...
  var CSS = [
    ...
    '.panel-pro { background: #fff; border: 1px solid #dadce0; border-radius: 8px; display: flex; flex-direction: column; overflow: visible; width: 100%; min-width: 0; box-sizing: border-box; font-family: "Google Sans", Roboto, "Segoe UI", Arial, sans-serif; font-size: 14px; color: #202124; margin: 0; position: relative; }',
    ...
    '.panel-pro__col--hidden { display: none !important; }',
  ...
  function buildHiddenColsCss(mountId, columns) {
    ...
    columns.forEach(function (c, idx) {
      if (c.hidden) {
        rules.push('#' + mountId + ' li.panel-pro__row > *:nth-child(' + (idx + 1) + ') { display: none !important; }');
      }
    });
    return rules.join('\n');
  }
